...
const getLinkIcon = (name: string) => {
  ...
  if (n.includes('ai')) return <Bot {...props} />;
  return <Layout {...props} />;
};
...
const getThemeStyles = (title: string) => {
  ...
  if (t.includes('setting')) return createStyles('#e68313', '10,116,139', 'from-pink-500 via-pink-400 to-pink-300');
  ...
  if (t.includes('ai')) return createStyles('#a855f7', '168,85,247', 'from-violet-600 via-purple-500 to-indigo-600');
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0 relative z-10 ml-2">
                  <Star
                    size={18}
                    onClick={(e) => {
                      e.stopPropagation(); // 1. THIS PREVENTS NAVIGATION

                      // 2. LOGIC TO TOGGLE PIN
                      const isStarred = pinned.includes(link.name);
                      let updatedPinned;

                      if (isStarred) {
                        updatedPinned = pinned.filter(name => name !== link.name);
                      } else {
                        updatedPinned = [...pinned, link.name];
                      }

                      // Update state and storage
                      setPinned(updatedPinned);
                      const storageFormat = updatedPinned.map(name => ({ name }));
                      localStorage.setItem('pinned_modules', JSON.stringify(storageFormat));

                      // Optional: Dispatch event if other components need to know
                      window.dispatchEvent(new Event('recent_visits_updated'));
                    }}
                    className={`transition-all duration-300 cursor-pointer hover:scale-125 active:scale-90 ${isStarred
