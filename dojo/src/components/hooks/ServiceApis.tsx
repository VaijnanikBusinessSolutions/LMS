...
export const humanBodyCheckService = {
  ...
  async updateQuestion(id: number, questionText: string): Promise<Question> {
    try {
      // We must append the ID and a trailing slash for Django REST Framework
      // Result: http://api.../questions/5/
      const baseUrl = `${API_ENDPOINTS.BASE_URL}${API_ENDPOINTS.HUMAN_BODY_CHECKS.QUESTIONS}`;
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.question_text ? errorData.question_text[0] : 'Failed to update question');
      }
  ...
  async deleteQuestion(id: number): Promise<void> {
    try {
      ...
      if (!response.ok) {
        // Attempt to read error text, fallback to generic
        let errorMsg = 'Failed to delete question';
        try {
          const errorData = await response.json();
          errorMsg = errorData.detail || errorMsg;
        } catch (e) { /* ignore json parse error on delete */ }

        throw new Error(errorMsg);
      }

      // DELETE usually returns 204 No Content, so we don't return JSON
      return;
    } catch (error) {
  ...
  async reorderQuestions(orderedIds: number[]): Promise<void> {
    try {
      // Construct the URL using your existing constants
      const baseUrl = `${API_ENDPOINTS.BASE_URL}${API_ENDPOINTS.HUMAN_BODY_CHECKS.QUESTIONS}`;
      
      // Append 'reorder/' to the base URL. 
      // This handles cases whether the base url ends with '/' or not.
      const url = baseUrl.endsWith('/') ? `${baseUrl}reorder/` : `${baseUrl}/reorder/`;
...
// }; // End of humanBodyCheckService object
  ...
  async updateCheckData(payload: CheckDataPayload): Promise<void> {
    try {
      // --- THIS IS THE CORRECTED URL ---
      // Manually construct the correct RESTful URL for the update.
      // It should look like: /api/human-body-checks/TEMP12345/
      const url = `${API_ENDPOINTS.BASE_URL}/human-body-checks/${payload.temp_id}/`;
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.detail || 'Failed to update check data');
      }
    } catch (error) {
      console.error('Failed to update check data', error);
      throw error;
    }
  },
  ...
  // Fetch existing check data by tempId
  async fetchCheckDataByTempId(tempId: string): Promise<any> { // Changed return type to any to allow raw response
    try {
      // Ensure URL creates: /human-body-checks/?temp_id=TEMP...
      // If API_ENDPOINTS.HUMAN_BODY_CHECKS.GET_BY_TEMP_ID is a function, ensure it adds the query param.
      // Safest way is to construct it manually like this:
      const url = `${API_ENDPOINTS.BASE_URL}/human-body-checks/?temp_id=${tempId}`;
      ...
      if (response.ok) {
        ...
        // The Hook (useCheckData) will handle parsing 'sheet_answers'.
        return data;
      }
      return null;
    } catch (error) {
      console.error('Failed to fetch existing check data', error);
      // Return null or empty array so the UI doesn't crash
      return [];
    }
  },
  ...
  async fetchUserById(userId: any) {
    // ⚠️ IMPORTANT: Adjust this URL to match your backend API endpoint for getting one user.
    // It's likely similar to your temp user list URL but with an ID at the end.
    const url = `${API_ENDPOINTS.BASE_URL}${API_ENDPOINTS.TEMP_USER_INFO}${userId}/`;
...
export const getHierarchyData = async () => {
  const response = await fetch(`${API_ENDPOINTS.BASE_URL}/hierarchy-simple/`);
  if (!response.ok) {
