...
//     e.preventDefault();
//     if (!formData.topic || !formData.subtopic || !formData.date) {
...
//     try {
//       if (editingId) {
//         await axios.put(`http://localhost:8000/api/actions/${editingId}/`, formData);
//       } else {
//         await axios.post("http://localhost:8000/api/actions/", formData);
//       }
//       fetchData();
//       closeModal();
//     } catch (error) {
...
//       try {
//         await axios.delete(`http://localhost:8000/api/actions/${editingId}/`);
//         fetchData();
//         closeModal();
//       } catch (error) {
...
//   // When a row is clicked, we find the data by index
...
//   // Only map the raw text data (No buttons here anymore)
...
//       {/* CLICKABLE HEADER TO ADD NEW */}
//       <div 
//         onClick={openAddModal} 
//         style={styles.headerClickable}
//         title="Click here to Add New Plan"
//       >
//         <h2>Action Planned Rejection <span style={{fontSize:'0.6em', verticalAlign:'middle'}}>➕</span></h2>
//         <p style={{fontSize: '12px', color: '#666', marginTop: '-10px'}}>Click here to add new • Click a row to edit</p>
//       </div>
...
//   // Header Styling to look interactive
//   headerClickable: { 
//     cursor: "pointer", 
...
const MyTable = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("http://localhost:8000/api/actionsrejection/");
      setTableData(response.data);
    } catch (error) {
      console.error("Error fetching data", error);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`http://localhost:8000/api/actionsrejection/${editingId}/`, formData);
      } else {
        await axios.post("http://localhost:8000/api/actionsrejection/", formData);
      }
      fetchData();
      closeModal();
    } catch (error) {
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this?")) {
      try {
        await axios.delete(`http://localhost:8000/api/actionsrejection/${editingId}/`);
        fetchData();
        closeModal();
      } catch (error) {
        console.error("Error deleting", error);
