...
// Helper: read an "R G B" css variable and return rgb()/rgba()
function cssRgbVar(varName: string, alpha = 1) {
  const root = document.documentElement;
  const raw = getComputedStyle(root).getPropertyValue(varName).trim(); // e.g. "226 232 240"
  if (!raw) return alpha === 1 ? 'rgb(0 0 0)' : `rgb(0 0 0 / ${alpha})`;
...
const MonthlySkillDisplay: React.FC<MonthlySkillDisplayProps> = ({
  ...
  React.useEffect(() => {
    ...
    const trackColor = cssRgbVar('--canvas-track', 0.6);   // slightly translucent
    const strokeColor = cssRgbVar('--canvas-stroke', 0.9);
    const textColor = cssRgbVar('--canvas-text', 1);
    ...
    if (skillLevel > 0) {
      ...
      ctx.arc(centerX, centerY, radius, -Math.PI / 2, endAngle);
      ctx.closePath();
      ctx.fillStyle = colors[skillLevel] || cssRgbVar('--text-muted', 1);
      ctx.fill();
    }
  ...
  return (
    <div className="flex items-center justify-center">
      <canvas
        ...
        height={size}
        className="rounded-full"
        title={title || `Station ${stationId}${stationName ? ` (${stationName})` : ''} - Level ${skillLevel}`}
      />
    </div>
  );
};
