...
// src/components/pages/Level1SettingsPro.tsx
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted mt-1">Manage training structure, content, and materials in a guided flow.</p>
          </div>
          <button
            className="text-sm px-4 py-2 rounded-lg bg-surface border border-border text-muted hover:text-text hover:shadow-sm transition-all duration-200 flex items-center gap-2"
            onClick={() => setShowTips(v => !v)}
          >
            <Info size={16} />
            {showTips ? "Hide" : "Show"} Instructions
        ...
        {showTips && (
          <div className="bg-surface border border-border rounded-xl p-5 text-sm">
            <div className="font-semibold mb-2 text-text flex items-center gap-2">
              <AlertCircle size={18} className="text-primary" />
              How to use
            </div>
            <ol className="list-decimal ml-5 space-y-1.5 text-muted">
              <li>Select a Day (chip) to filter lessons by day.</li>
              <li>Click "+ Add Lesson" to create a lesson under the selected Day.</li>
              <li>Select a Lesson card → click "+ Add Content" to add textual content.</li>
              <li>Click a Content item → add Materials (upload file or add a link).</li>
        ...
        <div className="bg-surface rounded-xl shadow-sm border border-border p-5">
          <div className="flex flex-wrap gap-4">
            {steps.map((s, i) => (
              <div key={s.key} className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300
                  ${s.done ? "bg-primary text-white shadow-md scale-110" : "bg-muted/20 text-muted"}`}>
                  {s.done ? <Check size={16} /> : i + 1}
                </div>
                <span className={`text-sm font-medium ${s.done ? "text-primary" : "text-muted"}`}>{s.label}</span>
                {i < steps.length - 1 && (
        ...
        {/* Filters row (updated to remove topic) */}
        <div className="bg-surface rounded-xl shadow-sm p-5 space-y-4 border border-border">
          {/* Day chips */}
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {days.map(d => (
                <div key={d.days_id} className={`group flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-200 cursor-pointer border
                  ${selectedDay === d.days_id 
                    ? "bg-primary text-white border-primary shadow-md" 
                    : "bg-background border-border text-muted hover:text-text hover:border-primary/50"}`}>
                  <button className="font-medium focus:outline-none" onClick={() => setSelectedDay(selectedDay === d.days_id ? null : d.days_id)}>
                    {d.day}
                  </button>
                  <button 
                    className={`opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:scale-110 ${selectedDay === d.days_id ? 'text-white/80 hover:text-white' : 'text-muted hover:text-primary'}`}
                    title="Rename" 
                    onClick={(e) => { e.stopPropagation(); renameDay(d); }}
                  >
                    <Edit size={14} />
                  </button>
                  <button 
                    className={`opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:scale-110 ${selectedDay === d.days_id ? 'text-white/80 hover:text-white' : 'text-muted hover:text-red-500'}`}
                    title="Delete" 
                    onClick={(e) => { e.stopPropagation(); deleteDay(d.days_id); }}
                  >
                    <X size={14} />
                  </button>
                </div>
              ...
              <div className="flex items-center gap-2">
                <input 
                  className="input py-2 h-auto text-sm bg-background" 
                  placeholder="Add Day (e.g., Day 1)" 
                  value={newDay} 
                  onChange={e => setNewDay(e.target.value)}
                  onKeyPress={e => e.key === 'Enter' && addDay()}
                />
                <button 
                  className="btn btn-primary h-auto py-2"
                  onClick={addDay} 
                  disabled={isBusy}
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Lessons */}
          <div className="bg-surface rounded-xl shadow-sm p-5 border border-border">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-semibold text-lg text-text mb-4 flex items-center gap-2">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-3 ml-auto">
                  <div className="relative">
                    ...
                    <input
                      className="input pl-10 py-2 h-auto text-sm bg-background"
                      placeholder="Search lessons..."
                      value={search}
                      onChange={e => setSearch(e.target.value)}
                    />
                  ...
                  <button
                    onClick={() => setShowLessonModal(true)}
                    className="btn btn-primary h-auto py-2 disabled:opacity-50"
                    disabled={!selectedDay}
                    title={!selectedDay ? "Pick a Day first" : "Add Lesson"}
                  >
                    <Plus size={16} className="mr-1" />
            ...
            {filteredLessons.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {filteredLessons.map(l => (
                  <div
                    key={l.id}
                    className={`group border rounded-xl p-4 transition-all duration-200 cursor-pointer relative overflow-hidden
                      ${selectedLesson === l.id 
                  ...
                  >
                    ...
                    <div className="font-semibold text-text truncate mb-3">{l.title}</div>
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 absolute bottom-3 right-3">
                      <button 
                        className="p-1.5 bg-background border border-border text-muted hover:text-primary rounded-lg shadow-sm hover:bg-primary/5 transition-colors" 
                        onClick={(e) => { e.stopPropagation(); renameLesson(l); }}
                        title="Edit"
                      >
                        <Edit size={14} />
                      ...
                      <button 
                        className="p-1.5 bg-background border border-border text-muted hover:text-red-500 rounded-lg shadow-sm hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" 
                        onClick={(e) => { e.stopPropagation(); deleteLesson(l.id); }}
                        title="Delete"
                      >
                        <Delete size={14} />
            ...
            {!selectedLesson ? (
              ...
              <div className="space-y-3">
                ...
                {contents.map(c => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="font-medium text-text flex-1 mr-2">{c.content}</div>
                      <button 
                        className="text-sm p-1.5 text-muted hover:text-red-500 rounded-lg transition-colors" 
                        onClick={(e) => { e.stopPropagation(); deleteContent(c.subtopiccontent_id); }}
                      >
                        <Delete size={14} />
                      </button>
                    ...
                    {selectedContent === c.subtopiccontent_id && (
                      <div className="mt-4 pt-4 border-t border-border">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <button 
                            className="text-sm px-3 py-1 bg-primary/10 text-primary rounded-lg hover:bg-primary/20 transition-colors duration-200 flex items-center gap-1" 
                            onClick={() => setShowMaterialModal(true)}
                          >
                            <Plus size={14} />
                            Add
                        ...
                        {materials.length === 0 ? (
                          ...
                          <div className="mt-2 space-y-2">
                            {materials.map(m => (
                              <div key={m.id} className="p-3 bg-background border border-border rounded-lg flex items-center justify-between hover:shadow-sm transition-all duration-200">
                                <div className="min-w-0 flex-1">
                                  <div className="font-medium text-text truncate">{m.description}</div>
                                ...
                                <div className="flex gap-2 ml-2">
                                  {m.url_link && (
                                    <a 
                                      className="px-3 py-1 bg-primary/10 text-primary rounded-lg hover:bg-primary/20 transition-colors duration-200 text-sm font-medium" 
                                      href={m.url_link} 
                                      target="_blank" 
                                      rel="noreferrer"
                                    >
        ...
        {showLessonModal && (
          <Modal title="Create Lesson" onClose={() => setShowLessonModal(false)}>
            <p className="text-sm text-muted mb-4">This lesson will be created under the selected Day.</p>
            <input 
        ...
        {showContentModal && (
          <Modal title="Add Content" onClose={() => setShowContentModal(false)}>
            <textarea 
              className="input w-full bg-background resize-none" 
              rows={4} 
              placeholder="Write your content text here..." 
              value={contentText} 
              onChange={(e) => setContentText(e.target.value)}
        ...
        {showMaterialModal && (
          <Modal title="Add Material" onClose={() => setShowMaterialModal(false)}>
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <button 
                  className={`flex-1 px-4 py-2 rounded-lg transition-all duration-200 flex items-center justify-center gap-2 border font-medium
                    ${matType === "file" 
                      ? "bg-primary text-white border-primary shadow-sm" 
                      : "bg-surface text-muted border-border hover:bg-muted/10"}`} 
                  onClick={() => setMatType("file")}
                >
                  <Upload size={16} />
                  File Upload
              ...
              {matType === "file" ? (
                <div
                  className="border-2 border-dashed border-border rounded-lg p-6 text-center bg-background transition-all duration-200 hover:border-primary hover:bg-primary/5"
                  onDragOver={(e) => e.preventDefault()}
                  onDrop={(e) => { e.preventDefault(); if (e.dataTransfer.files?.[0]) setMatFile(e.dataTransfer.files[0]); }}
                >
                  <Upload size={32} className="mx-auto mb-3 text-muted" />
                  <p className="mb-3 text-muted">Drag & drop a file here, or</p>
                  <input 
                    ref={fileRef} 
                    type="file" 
                    className="hidden" 
                  ...
                  <div className="mt-3 text-sm text-text font-medium">
                    {matFile?.name ?? "No file selected"}
                  </div>
                </div>
              ) : (
        ...
        {toast && (
          <div className="fixed right-4 bottom-4 z-50 animate-slide-in">
            <div className={`px-5 py-3 rounded-lg shadow-lg text-white flex items-center gap-3 border
              ${toast.type === "success" ? "bg-green-600 border-green-700" : 
                toast.type === "error" ? "bg-red-600 border-red-700" : 
                "bg-surface text-text border-border dark:bg-gray-800"}`}>
              {toast.type === "success" && <Check size={20} />}
              {toast.type === "error" && <X size={20} />}
              {toast.type === "info" && <Info size={20} />}
              {toast.message}
        ...
        {isBusy && (
          <div className="fixed inset-0 z-40 bg-black/20 backdrop-blur-sm pointer-events-none">
            <div className="absolute right-4 top-4 bg-surface shadow-xl border border-border px-4 py-3 rounded-lg text-sm flex items-center gap-3 text-text">
...
function EmptyState({ title, subtitle, actionText, onAction, compact }: { title: string; subtitle?: string; actionText?: string; onAction?: () => void; compact?: boolean; }) {
  return (
    <div className={`text-center ${compact ? "py-6" : "py-12"} border-2 border-dashed border-border rounded-xl bg-background/50`}>
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode; }) {
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-surface rounded-xl shadow-2xl border border-border w-full max-w-md animate-scale-in">
        <div className="p-5 border-b border-border flex items-center justify-between">
          ...
          <button 
            onClick={onClose} 
            className="text-muted hover:text-text transition-colors duration-200 p-1 hover:bg-muted/10 rounded-lg"
          >
            <X size={20} />
          </button>
        </div>
...
function ConfirmDialog({ text, onCancel, onConfirm }: { text: string; onCancel: () => void; onConfirm: () => void; }) {
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-surface rounded-xl shadow-2xl border border-border w-full max-w-sm animate-scale-in">
        <div className="p-5">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center">
              <AlertCircle size={24} className="text-red-600 dark:text-red-400" />
            </div>
            <div className="font-semibold text-lg text-text">Please confirm</div>
          </div>
          <div className="text-muted mb-6">{text}</div>
          <div className="flex justify-end gap-3">
            <button 
              className="btn btn-secondary" 
              onClick={onCancel}
