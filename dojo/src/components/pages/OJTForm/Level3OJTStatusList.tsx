...
const Level3OJTStatusList: React.FC = () => {
  ...
  const [searchTerm, setSearchTerm] = useState(""); // Actual filter logic
  const [statusFilter, setStatusFilter] = useState<"All" | "Complete" | "Incomplete" | "Pass" | "Fail">("All");
  const [departmentFilter, setDepartmentFilter] = useState<string>("All");
  const [selectedView] = useState<"OJT" | "TenCycle">("OJT");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError(err.message || "Failed to load Level 3 OJT data");
      } finally {
        setLoading(false);
      }
    };
   ...
   const filtered = useMemo(() => {
     return data.filter(item => {
       ...
       const matchesSearch =
         ...
         item.trainer_name.toLowerCase().includes(term) ||
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("OJT Level 3", {
    ...
    titleCell.value = "OJT Level 3 Status Report";
    ...
    saveAs(blob, `OJT_Level3_Report_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-lg p-8 max-w-sm w-full text-center">
          ...
          <p className="text-gray-600 font-medium">Loading Level 3 OJT Data...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                <h1 className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-white via-white/90 to-white/70 bg-clip-text text-transparent drop-shadow-lg">
                  {selectedView === "OJT" ? "OJT Level 3 Status Dashboard" : "TenCycle Status"}
                </h1>
                {/* <p className="text-white/80 mt-2 text-sm sm:text-base font-medium drop-shadow">
                  {selectedView === "OJT"
                    ? "Real-time trainee progress tracking with advanced analytics"
          ...
          <>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search by ID, Name, Trainer, Line, or Station..."
...
interface StatCardProps {
  ...
  label: string;
  value: number;
  color: "indigo" | "emerald" | "amber" | "emerald" | "rose";
}
...
export default Level3OJTStatusList;