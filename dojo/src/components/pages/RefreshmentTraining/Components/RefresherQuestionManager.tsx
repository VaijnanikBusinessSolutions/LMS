...
const SkeletonCard: React.FC = () => (
  <div className="border border-border-200 rounded-xl p-4 animate-pulse">
...
const LivePreview: React.FC<{
  ...
  return (
    <div className="bg-background-to-br from-background-50 to-background border-2 border-dashed border-border-200 rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-4 text-text-600">
        <Eye className="w-4 h-4" />
        <span className="text-xs font-bold uppercase tracking-wide">Live Preview</span>
      </div>
      
      {!hasContent ? (
        <div className="flex flex-col items-center justify-center h-64 text-text-300">
          <Sparkles className="w-16 h-16 mb-4 opacity-50" />
          <p className="text-sm font-medium">Start typing to see preview</p>
        </div>
      ) : (
        <div className="bg-surface rounded-xl shadow-lg border border-border-100 p-5 transform transition-all duration-300">
          {/* Question */}
          <div className="mb-4 pb-4 border-b border-border-100">
              ...
              <div className="flex-1">
                ...
                {previews.question && (
                  <img 
                    ...
                    alt="Question" 
                    className="mt-3 max-h-32 rounded-lg border border-border-200 object-contain"
                  />
                )}
              </div>
            </div>
                ...
                <div 
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg transition-all duration-200 ${
                    isCorrect 
                      ...
                      : 'bg-background border border-border-200'
                  }`}
                >
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                    isCorrect ? 'bg-green-500 text-white' : 'bg-gray-300 text-text'
                  }`}>
                    {opt}
                  </div>
                  {image && (
                  ...
                  <span className={`text-sm ${isCorrect ? 'font-semibold text-green-800' : 'text-text'}`}>
                    {text || '(Image only)'}
                  </span>
          ...
          {/* Marks Badge */}
          <div className="mt-4 pt-4 border-t border-border-100 flex justify-end">
...
const EmptyState: React.FC<{
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-8">
      ...
      <h3 className="text-2xl font-bold text-text mb-2">{config.title}</h3>
      <p className="text-text mb-8">{config.subtitle}</p>
      ...
      {config.showActions && (
        <div className="flex gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-surface border-2 border-border-200 text-text font-bold rounded-xl hover:border-purple-300 hover:shadow-md transition-all duration-200"
          >
...
const ImageInput: React.FC<{
  ...
  return (
    <div className={`border rounded-xl p-3 transition-all duration-200 ${preview ? 'border-purple-300 bg-purple-50' : 'border-border-200 bg-surface hover:border-purple-200'}`}>
      {label && <div className="text-xs font-bold text-text uppercase mb-2">{label}</div>}
      
      {preview ? (
        <div className="relative mb-3 group">
          <img src={preview} alt="Preview" className={`w-full object-contain rounded-lg bg-background ${compact ? 'h-20' : 'h-32'}`} />
          <button 
            type="button"
        ...
        <div className={`w-full bg-background-to-br from-background-50 to-background-100 rounded-lg border-2 border-dashed border-border-300 flex flex-col items-center justify-center text-text-400 transition-all hover:border-purple-300 hover:bg-background-50 ${compact ? 'h-20' : 'h-28'}`}>
          <ImageIcon className="w-5 h-5 mb-1" />
          <span className="text-[10px] font-bold uppercase">No Image</span>
        </div>
      )}

      <div className="flex bg-background p-1 rounded-lg mb-2">
        <button 
          type="button"
          onClick={() => setMode('file')}
          className={`flex-1 py-1.5 text-xs font-bold rounded-md flex items-center justify-center gap-1 transition-all ${mode === 'file' ? 'bg-surface shadow text-purple-700' : 'text-text hover:text-text'}`}
        >
        ...
        <button 
          type="button"
          onClick={() => setMode('url')}
          className={`flex-1 py-1.5 text-xs font-bold rounded-md flex items-center justify-center gap-1 transition-all ${mode === 'url' ? 'bg-surface shadow text-purple-700' : 'text-text hover:text-text'}`}
        >
      ...
      {mode === 'file' ? (
        <input 
          type="file" 
          accept="image/*" 
          className="block w-full text-xs text-text file:mr-2 file:py-1.5 file:px-3 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-purple-100 file:text-purple-700 hover:file:bg-purple-200 file:cursor-pointer file:transition-colors"
        ...
        <input 
          ...
          className="w-full bg-background p-2 text-xs border border-border-200 rounded-lg focus:ring-2 focus:ring-purple-200 focus:border-purple-400 outline-none transition-all"
...
const QuestionCard: React.FC<{
  ...
  if (viewMode === 'compact') {
    return (
      <div
        className={`flex items-center gap-4 p-3 border-b border-border-100 hover:bg-background transition-all duration-200 cursor-pointer group animate-fade-in`}
      ...
      >
        <button
          ...
          className="text-text-400 hover:text-purple-600 transition-colors"
        >
          {isSelected ? (
        ...
        <span className="text-xs font-bold text-text-400 w-6">{index + 1}</span>
  ...
  if (viewMode === 'table') {
    return (
      <tr
        className={`hover:bg-background transition-all duration-200 animate-fade-in ${
          isSelected ? 'bg-purple-50' : ''
        }`}
        style={{ animationDelay: `${animationDelay}ms` }}
      >
        <td className="p-3">
          <button onClick={onSelect} className="text-text-400 hover:text-purple-600">
            {isSelected ? (
              <CheckSquare className="w-4 h-4 text-purple-600" />
            ) : (
              <Square className="w-4 h-4" />
        ...
        <td className="p-3 text-sm font-medium text-text-500">{index + 1}</td>
        <td className="p-3">
          <div className="flex items-center gap-2">
            {q.question_image_url && (
              <ImageIcon className="w-4 h-4 text-purple-400" />
            )}
            <span className="text-sm text-text max-w-md truncate">
        ...
        <td className="p-3">
          <div className="flex gap-1">
            <button
              onClick={onPreview}
              className="p-1.5 text-text-500 hover:bg-background rounded-lg transition-colors"
              title="Preview"
            >
              <Eye className="w-4 h-4" />
            </button>
    ...
    <div
      className={`relative border rounded-xl p-4 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 cursor-pointer group animate-fade-in ${
        isSelected
          ...
          : 'border-border-200 bg-surface hover:border-purple-200'
      }`}
      style={{ animationDelay: `${animationDelay}ms` }}
      onClick={onPreview}
    >
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              {q.question_text && (
                <p className="font-medium text-text text-sm leading-relaxed line-clamp-2">
                  {q.question_text}
                </p>
              )}
              {q.question_image_url && (
                <img
                  ...
                  alt="Q"
                  className="h-16 rounded-lg mt-2 border border-border-200 object-contain bg-surface shadow-sm"
                />
              )}
            </div>
          </div>
...
const PreviewModal: React.FC<{
  ...
  return (
    ...
    >
      <div
        className="bg-surface rounded-2xl shadow-2xl max-w-2xl w-full mx-4 overflow-hidden animate-scale-in"
      ...
      >
        ...
        <div className="p-6">
              ...
              <div className="flex-1">
                ...
                {question.question_image_url && (
                  <img
                    ...
                    alt="Question"
                    className="mt-3 max-h-48 rounded-lg border border-border-200 object-contain"
                  />
                )}
              </div>
            </div>
            ...
            {['A', 'B', 'C', 'D'].map(opt => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`flex-1 ${
                      isCorrect ? 'font-semibold text-green-800' : 'text-text'
                    }`}
                  >
                    {text || '(Image only)'}
                  </span>
          ...
          {/* Marks */}
          <div className="mt-6 pt-4 border-t border-border-100 flex items-center justify-between">
            <span className="text-text-500 text-sm">Marks</span>
        ...
        {/* Modal Footer */}
        <div className="bg-background px-6 py-4 flex justify-between items-center border-t">
          <div className="flex gap-2">
            <button
              onClick={onPrev}
              disabled={!hasPrev}
              className="p-2 text-text-500 hover:bg-gray-200 rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            >
...
const RefresherQuestionManager: React.FC<Props> = ({
  ...
  return (
    ...
    <div className="bg-surface text-text rounded-2xl shadow-2xl border border-border overflow-hidden flex flex-col h-[90vh] max-h-[900px]">
      {/* HEADER */}
      <div className="bg-gradient-to-r from-purple-600 via-purple-700 to-pink-600 p-5 flex-shrink-0">
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:flex items-center gap-2 bg-surface/10 rounded-full px-4 py-2">
        ...
        <div className="mt-4 flex items-center gap-4">
          <div className="flex items-center gap-2 text-purple-200 text-xs">
            ...
            <span>Press</span>
            <kbd className="bg-surface/20 px-2 py-0.5 rounded text-white font-mono">
              Ctrl+N
            </kbd>
      ...
      {/* TOOLBAR */}
      {/* <div className="px-6 py-4 border-b border-border-100 flex flex-wrap items-center gap-4 bg-gray-50 flex-shrink-0"> */}
      <div className="px-6 py-4 border-b border-border/60 flex flex-wrap items-center gap-4 bg-background flex-shrink-0">
        <div className="relative flex-1 min-w-[200px] max-w-md">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-text-400 w-4 h-4" />
          <input
            type="text"
            placeholder="Search questions..."
            value={searchQuery}
            onChange={e => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2.5 border border-border-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent focus:outline-none text-sm bg-surface shadow-sm"
          />
        </div>

        <div className="flex bg-surface border border-border-200 p-1 rounded-xl shadow-sm">
          <button
            onClick={() => setViewFilter('active')}
            className={`px-4 py-2 text-xs font-bold rounded-lg transition-all flex items-center gap-2 ${
              viewFilter === 'active'
                ? 'bg-green-100 text-green-700 shadow-sm'
                : 'text-text-500 hover:text-text'
            }`}
          >
        ...
        <div className="flex gap-2 ml-auto">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-surface border border-border-200 text-text font-medium rounded-xl hover:bg-background hover:border-border-300 transition-all text-sm shadow-sm"
          >
      ...
      {message && (
        ...
        >
          ...
          <button
            ...
            className="ml-auto text-text-400 hover:text-text"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
            ...
            <>
              <div className="flex items-center justify-between mb-4 pb-3 border-b border-border-100">
                <button
                  ...
                  className="flex items-center gap-2 text-sm text-text-500 hover:text-purple-600 transition-colors"
                >
              ...
              {viewMode === 'table' ? (
                <div className="bg-surface rounded-xl border border-border-200 overflow-hidden shadow-sm">
                  <table className="w-full">
                    <thead className="bg-background border-b border-border-200">
                      <tr>
                        <th className="p-3 text-left w-10"></th>
                        <th className="p-3 text-left text-xs font-bold text-text-500 uppercase">
                          #
                        </th>
                        <th className="p-3 text-left text-xs font-bold text-text-500 uppercase">
                          Question
                ...
                <div className="bg-surface rounded-xl border border-border-200 overflow-hidden shadow-sm divide-y divide-border-100">
                  {filteredQuestions.map((q, i) => (
                    <QuestionCard
                      key={q.id}
                      question={q}
        ...
        {showRightPanel && (
          ...
          <div className="w-1/2 border-l border-border bg-background overflow-y-auto animate-slide-in-right">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  ...
                  className="p-2 text-text-400 hover:text-text hover:bg-gray-200 rounded-lg transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 relative">
                <form onSubmit={handleManualSubmit} className="space-y-5">
                  {/* Question */}
                  <div className="bg-background rounded-xl p-4 border border-border-200 shadow-sm">
                    ...
                    <textarea
                      ...
                      className="w-full p-3 bg-background border border-border-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent focus:outline-none text-sm resize-none"
                      rows={3}
                  ...
                  <div className="space-y-3 bg-background">
                    {['a', 'b', 'c', 'd'].map(opt => (
                      <div
                        key={opt}
                        className={`bg-background rounded-xl p-4 border-2 transition-all ${
                      ...
                      >
                        <div className="bg-background flex items-center justify-between mb-3">
                          <label className="font-bold text-text uppercase text-sm flex items-center gap-2">
                            <span
                              className={`w-6 h-6 bg-background rounded-full flex items-center justify-center text-xs ${
                                manualForm.correct_answer.toLowerCase() === opt
                                  ? 'bg-green-500 text-white'
                                  : 'bg-gray-500 text-text'
                              }`}
                            >
                              {opt.toUpperCase()}
                            </span>
                          ...
                          <label className="flex items-center bg-background gap-2 text-xs text-text-500 cursor-pointer">
                            <input
                              type="radio"
                              name="correct"
                              checked={
                              ...
                              className="w-4 h-4 bg-background text-green-600 focus:ring-green-500 cursor-pointer"
                            />
                            Correct
                          </label>
                        </div>
                        ...
                        <input
                          ...
                          className="w-full bg-background p-2.5 border border-border-200 rounded-lg text-sm focus:ring-2 focus:ring-purple-200 focus:border-purple-400 outline-none"
                  ...
                  {/* Marks */}
                  <div className="bg-surface rounded-xl p-4 border border-border-200 shadow-sm">
                    <label className="block text-sm font-bold text-text mb-2">
                      Marks
                    </label>
                    <div className="flex gap-2">
                      {[1, 2, 3, 4, 5].map(m => (
                        <button
                          ...
                          className={`w-10 h-10 rounded-lg font-bold transition-all ${
                            manualForm.marks === m
                              ? 'bg-purple-600 text-text shadow-lg scale-110'
                              : 'bg-background text-text hover:bg-background-200'
                          }`}
                        >
                          {m}
                        </button>
                  ...
                  <div className="flex gap-3 pt-4">
                    <button
                      ...
                      className="flex-1 py-3 text-text font-bold hover:bg-gray-200 rounded-xl transition-colors"
                    >
                      Cancel
                    </button>
                    <button
      ...
      {selectedIds.length > 0 && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-6 animate-slide-up z-20">
          ...
          <button
            ...
            className="p-2 text-text-400 hover:text-white hover:bg-gray-700 rounded-lg transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      {showUploadModal && (
        ...
        >
          <div
            className="bg-surface rounded-2xl shadow-2xl max-w-lg w-full mx-4 overflow-hidden animate-scale-in"
            onClick={e => e.stopPropagation()}
          >
            ...
            <div className="p-6">
              ...
              <div
                className={`relative border-2 border-dashed rounded-2xl p-8 text-center transition-all cursor-pointer hover:bg-background ${
                  excelFile ? 'border-green-400 bg-green-50' : 'border-border-300'
                }`}
              >
                <input
                  type="file"
                ...
                {excelFile ? (
                  <>
                    ...
                    <p className="text-text-500 text-sm">Ready to upload</p>
                  </>
                ) : (
                  <>
                    <Upload className="w-16 h-16 text-text-300 mx-auto mb-4" />
                    <h4 className="text-lg font-bold text-text">
