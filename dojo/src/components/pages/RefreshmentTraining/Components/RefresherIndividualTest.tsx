...
const RefresherIndividualTest: React.FC<Props> = ({ 
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-background text-center p-4">
        <div className="bg-surface border border-border rounded-3xl shadow-xl p-8 max-w-md w-full">
          <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <AlertCircle className="w-8 h-8 text-red-500" />
          </div>
          <h3 className="text-xl font-bold text-text mb-2">{error}</h3>
          <button 
            onClick={onExit} 
            className="mt-6 px-6 py-3 bg-surface border-2 border-border text-text hover:bg-background rounded-2xl font-bold transition-colors w-full"
          >
            Return to Lobby
          </button>
        </div>
  ...
  if (submitted && scoreData) {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        ...
        <div className="bg-surface border border-border rounded-3xl shadow-xl p-8 text-center max-w-md w-full animate-fade-in">
          {scoreData.passed ? (
            <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
              <Trophy className="w-10 h-10 text-green-600" />
            </div>
          ...
          <div className="flex items-center justify-center gap-2 mb-6">
            <Avatar name={employeeName} size="sm" />
            <span className="text-muted font-medium">{employeeName}</span>
          </div>

          <div className="grid grid-cols-2 gap-3 mb-8">
            <div className="bg-background border border-border p-4 rounded-2xl">
              <div className="text-xs text-muted font-bold uppercase mb-1">Score</div>
              <div className={`text-3xl font-bold ${scoreData.passed ? 'text-green-600' : 'text-orange-500'}`}>
                {percentage}%
              </div>
            </div>
            <div className="bg-background border border-border p-4 rounded-2xl">
              ...
              <div className="text-3xl font-bold text-text">
                {scoreData.score}<span className="text-base text-muted">/{scoreData.total}</span>
              </div>
            </div>
          </div>
          ...
          <button 
            onClick={onExit} 
            className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all"
          >
            Continue
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* Header */}
      <StickyHeader className="bg-surface border-b border-border shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              <button 
                onClick={() => { if(confirm("Exit test? Progress may be lost.")) onExit(); }} 
                className="p-2 hover:bg-background text-muted hover:text-text rounded-xl transition-colors"
              >
      ...
      <div className="flex-1 flex items-center justify-center p-4 pb-24">
        <div className="max-w-5xl w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Question Card */}
            <div className="bg-surface border border-border rounded-3xl shadow-lg p-8 flex flex-col">
              {/* Question Header */}
              <div className="flex items-center justify-between mb-4">
                <span className="text-sm font-bold text-purple-600 bg-purple-50 px-3 py-1.5 rounded-lg">
                  Question {currentIndex + 1} of {questions.length}
              ...
              <div className="mt-4 pt-4 border-t border-border">
                <div className="flex items-center justify-between text-sm mb-2">
                  ...
                  <span className="font-bold text-text">{answeredCount}/{questions.length} answered</span>
                </div>
            ...
            <div className="flex flex-col gap-3">
              {['A', 'B', 'C', 'D'].map((letter) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => selectOption(letter)}
                    className={`
                      w-full text-left rounded-2xl border-2 overflow-hidden flex items-stretch transition-all duration-200 group
                      ${isSelected 
                        ? 'border-purple-500 bg-purple-50/50 shadow-md' 
                        : 'border-border bg-surface hover:border-purple-300 hover:bg-background'
                      }
                    `}
                  >
                    {/* Letter Badge */}
                    <div className={`
                      ...
                      ${isSelected 
                        ? 'bg-purple-600 text-white' 
                        : 'bg-background text-muted border-r border-border group-hover:text-purple-600'
                      }
                    `}>
                      {letter}
                    </div>
                    ...
                    <div className="flex-1 p-4 flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <span className={`text-base font-medium block ${
                          isSelected ? 'text-purple-900' : 'text-text'
                        }`}>
                          {text}
                        </span>
                      </div>
              ...
              <div className="bg-surface rounded-xl border border-border p-4 mt-2">
                <div className="text-xs font-bold text-muted uppercase mb-2">Keyboard Shortcuts</div>
                <div className="flex flex-wrap gap-2 text-xs text-muted">
                  <span className="flex items-center gap-1">
                    <kbd className="px-1.5 py-0.5 bg-background border border-border rounded font-mono text-text">A</kbd>
                    <kbd className="px-1.5 py-0.5 bg-background border border-border rounded font-mono text-text">B</kbd>
                    <kbd className="px-1.5 py-0.5 bg-background border border-border rounded font-mono text-text">C</kbd>
                    <kbd className="px-1.5 py-0.5 bg-background border border-border rounded font-mono text-text">D</kbd>
                    Select
                  </span>
                  <span className="flex items-center gap-1">
                    <kbd className="px-1.5 py-0.5 bg-background border border-border rounded font-mono text-text">←</kbd>
                    <kbd className="px-1.5 py-0.5 bg-background border border-border rounded font-mono text-text">→</kbd>
                    Navigate
                  </span>
                </div>
      ...
      {/* Fixed Footer Navigation */}
      <div className="fixed bottom-0 left-0 w-full bg-surface border-t border-border shadow-2xl">
        <div className="max-w-5xl mx-auto px-4 py-4 flex justify-between items-center">
          <button
            onClick={goToPrevious}
            disabled={currentIndex === 0}
            className="px-6 py-3 font-bold text-muted hover:text-text hover:bg-background rounded-xl disabled:opacity-30 disabled:hover:bg-transparent transition-colors flex items-center gap-1.5"
          >
            <ArrowLeft className="w-5 h-5" /> Previous
          </button>
          ...
          <div className="flex items-center gap-2">
            {currentIndex === questions.length - 1 ? (
              <button
                onClick={handleSubmit}
                disabled={isSubmitting}
                className="px-8 py-3 bg-gradient-to-r from-green-600 to-green-500 text-white rounded-2xl font-bold shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center gap-2 disabled:opacity-50"
              >
                {isSubmitting ? (
                  <>
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
              ...
              <button
                onClick={goToNext}
                className="px-8 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-2xl font-bold shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center gap-2"
              >
                Next <ArrowRight className="w-5 h-5" />
              </button>
            )}
