...
import {
  Users, Plus, Trash2, Play, ArrowLeft, AlertCircle, Wifi, CheckCircle,
} from 'lucide-react';
import { Avatar, ProgressStepper, StickyHeader } from '../Components/shared/UIComponents';
...
const EasyTestRemote: React.FC<EasyTestRemoteProps> = ({
  ...
  return (
    <div className={`flex justify-center select-none ${isActive ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
      <div className="w-64 flex-shrink-0">
        <div className="relative bg-surface rounded-3xl p-3 shadow-2xl border-4 border-border">
          <div className="absolute -top-1 -right-1 flex items-center gap-1">
            <div className={`w-3 h-3 rounded-full ${isActive ? 'bg-green-500 animate-pulse' : 'bg-gray-400'} shadow-lg`} />
          </div>
...
const EmployeeSelectionModal: React.FC<EmployeeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface text-text rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-scale-up overflow-hidden border border-border">
        {/* Header */}
        <div className="bg-gradient-to-r from-purple-600 to-purple-700 p-6 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
        ...
        {/* Toolbar */}
        <div className="px-6 py-3 bg-background border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="text-sm text-muted">
              <span className="font-semibold text-text">{availableSessions.length}</span> available
              {alreadyAssignedSessions.length > 0 && (
                <span className="ml-2 text-orange-600">
                  • {alreadyAssignedSessions.length} already assigned
                </span>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {filteredSessions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <h3 className="text-xl font-semibold text-text mb-2">No Employees Found</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {availableSessions.map((session) => (
                <button
                  ...
                  className={`
                    ...
                    ${session.session_id === currentSessionId
                      ? 'border-green-400 bg-green-50 ring-2 ring-green-200'
                      : 'border-border hover:border-purple-400 hover:bg-purple-50/70 dark:hover:bg-purple-500/10 hover:shadow-lg'
                    }
                  `}
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <p className="font-semibold text-text truncate">{session.employee_name}</p>
                        {session.session_id === currentSessionId && (
                          <span className="px-2 py-0.5 bg-green-500 text-white text-xs rounded-full">Current</span>
                        )}
                      </div>
                      <p className="text-sm text-muted truncate">{session.employee_code}</p>
                    </div>
                  </div>
                </button>
              ))}
              ...
              {alreadyAssignedSessions.map((session) => (
                <div
                  key={session.session_id}
                  className="p-4 rounded-2xl border-2 border-border bg-background/60 opacity-60 cursor-not-allowed"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-border text-white text-xl font-bold flex-shrink-0">
                      {session.employee_name?.charAt(0)?.toUpperCase() || '?'}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <p className="font-semibold text-muted truncate">{session.employee_name}</p>
                        <span className="px-2 py-0.5 bg-orange-400 text-white text-xs rounded-full">Assigned</span>
                      </div>
                      <p className="text-sm text-muted/70 truncate">{session.employee_code}</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-2">
              {availableSessions.map((session) => (
                <button
                  ...
                  className={`
                    ...
                    ${session.session_id === currentSessionId
                      ? 'border-green-400 bg-green-50'
                      : 'border-border hover:border-purple-400 hover:bg-purple-50/70 dark:hover:bg-purple-500/10'
                    }
                  `}
                >
                  <div className={`
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-text">{session.employee_name}</p>
                    <p className="text-sm text-muted">{session.employee_code}</p>
                  </div>
                  {session.session_id === currentSessionId && (
                    <span className="px-3 py-1 bg-green-500 text-white text-sm rounded-full">Current</span>
                  )}
              ...
              {alreadyAssignedSessions.map((session) => (
                <div
                  key={session.session_id}
                  className="w-full p-4 rounded-xl border-2 border-border bg-background/60 flex items-center gap-4 opacity-60"
                >
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-border text-white text-lg font-bold flex-shrink-0">
                    {session.employee_name?.charAt(0)?.toUpperCase() || '?'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-muted">{session.employee_name}</p>
                    <p className="text-sm text-muted/70">{session.employee_code}</p>
                  </div>
                  <span className="px-3 py-1 bg-orange-400 text-white text-sm rounded-full">Already Assigned</span>
                </div>
              ))}
        ...
        <div className="p-4 bg-background border-t border-border flex items-center justify-between">
          <p className="text-sm text-muted">
            Press <kbd className="px-2 py-1 bg-surface border border-border rounded text-xs font-mono">ESC</kbd> to close
          </p>
          <button
            onClick={onClose}
            className="px-6 py-2 bg-surface hover:bg-background text-text font-medium rounded-xl transition-colors border border-border"
          >
            Cancel
          </button>
        </div>
...
const RemoteStationCard: React.FC<RemoteStationCardProps> = ({
  ...
  return (
    <div
      className={`
        group relative bg-surface rounded-2xl border-2 overflow-hidden
        transition-all duration-300 hover:shadow-2xl cursor-pointer
        ${isAssigned ? 'border-green-300 hover:border-green-400' : 'border-orange-300 hover:border-orange-400'}
        animate-slide-up
      `}
    ...
    >
      ...
      <div className="p-5">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h4 className="font-bold text-text text-lg">Remote #{remoteId}</h4>
            <p className={`text-sm font-medium ${isAssigned ? 'text-green-600' : 'text-orange-600'}`}>
              {isAssigned ? '✓ Employee Assigned' : '⚠ Awaiting Assignment'}
            </p>
          </div>
...
const Toast: React.FC<ToastProps> = ({ message, type, onClose }) => {
  ...
  return (
    <div className={`${styles[type]} text-white px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-3 animate-slide-up`}>
      {type === 'success' && <CheckCircle className="w-5 h-5" />}
      {type === 'error' && <AlertCircle className="w-5 h-5" />}
      {type === 'warning' && <AlertCircle className="w-5 h-5" />}
      {type === 'info' && <Wifi className="w-5 h-5" />}
...
const RefresherRemoteLobby: React.FC<Props> = ({
  ...
  const handleStartClick = () => {
    ...
    for (const remoteId of remoteInputs) {
      const sessionId = assignments[remoteId];
      if (sessionId) finalMap[remoteId] = sessionId;
    }
  ...
  return (
    <div className="min-h-screen bg-background text-text">
        ...
        <div className="max-w-6xl mx-auto px-6 py-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <button
                onClick={onBack}
                className="flex items-center text-muted hover:text-purple-600 mb-2 font-bold transition-colors text-sm"
              >
                <ArrowLeft className="w-4 h-4 mr-2" /> Back
              </button>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        {/* Stats Bar */}
        <div className="flex flex-wrap items-center gap-4 mb-8">
          <div className="bg-surface px-6 py-4 rounded-2xl shadow-lg flex items-center gap-4 border border-border">
            <div className="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center">
              <Wifi className="w-6 h-6 text-purple-600" />
            </div>
            <div>
              <div className="text-2xl font-bold text-text">{stats.total}</div>
              <div className="text-xs text-muted uppercase font-bold">Remotes</div>
            </div>
          </div>
          ...
          <div className="bg-surface px-6 py-4 rounded-2xl shadow-lg flex items-center gap-4 border border-border">
            ...
            <div>
              <div className="text-2xl font-bold text-text">{stats.assigned}/{stats.total}</div>
              <div className="text-xs text-muted uppercase font-bold">Assigned</div>
            </div>
          </div>
          ...
          <div className="bg-surface px-6 py-4 rounded-2xl shadow-lg flex items-center gap-4 border border-border">
            ...
            <div>
              <div className="text-2xl font-bold text-text">{pendingSessions.length}</div>
              <div className="text-xs text-muted uppercase font-bold">Available</div>
            </div>
          </div>
          ...
          {stats.pending > 0 && (
            <div className="bg-orange-50 px-6 py-4 rounded-2xl shadow-lg flex items-center gap-4 border border-orange-200">
              ...
              <div>
                ...
                <div className="text-xs text-orange-500 uppercase font-bold">Pending</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
            ...
            <div className="bg-surface rounded-2xl shadow-lg p-6 border border-border">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-text">Remote Stations</h2>
                    <p className="text-sm text-muted">Click on a station to assign employee</p>
                  </div>
                </div>
              ...
              {remoteInputs.length > 0 ? (
                ...
                <div className="text-center py-20 bg-gradient-to-br from-background to-border/30 rounded-2xl border-2 border-dashed border-border">
                  <div className="w-24 h-24 bg-surface rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-lg border border-border">
                    <Wifi className="w-12 h-12 text-muted" />
                  </div>
                  <h3 className="text-2xl font-bold text-text mb-2">No Remote Stations</h3>
          ...
          {/* RIGHT */}
          <div className="xl:col-span-1">
            <div className="bg-surface rounded-2xl shadow-lg p-6 sticky top-32 border border-border">
              <EasyTestRemote
                value={newRemote}
                onChange={setNewRemote}
                onSubmit={handleAddRemote}
