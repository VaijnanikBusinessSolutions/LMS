...
const RescheduleList: React.FC = () => {
  ...
  const handleSaveReschedule = async (rescheduleId: number) => {
    if (!editingForm.reschedule_date || !editingForm.reschedule_time || !editingForm.reschedule_reason.trim()) {
      setErrorMessage('Please fill in all reschedule details (date, time, and reason)');
      return;
    }
  ...
  return (
    <div className="space-y-6 text-text">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-text">Reschedule List</h2>
          <p className="text-muted mt-1">Manage reschedule details and track attendance history</p>
        </div>
        <button
          onClick={() => {
            fetchAllReschedules();
      ...
      {/* Stats cards: neutral wrapper themed */}
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="bg-surface rounded-lg shadow-md p-6 border-l-4 border-purple-600 border border-border">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-muted text-sm font-medium">Total Pending</p>
              <p className="text-3xl font-bold text-purple-600">{pendingCount}</p>
            </div>
            <AlertCircle className="w-12 h-12 text-purple-200" />
          </div>
        ...
        <div className="bg-surface rounded-lg shadow-md p-6 border-l-4 border-red-600 border border-border">
          <div className="flex items-center justify-between">
            <div>
        ...
        <div className="bg-surface rounded-lg shadow-md p-6 border-l-4 border-yellow-600 border border-border">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-muted text-sm font-medium">Rescheduled</p>
              <p className="text-3xl font-bold text-yellow-600">{rescheduledCount}</p>
            </div>
            <Calendar className="w-12 h-12 text-yellow-200" />
          </div>
        ...
        <div className="bg-surface rounded-lg shadow-md p-6 border-l-4 border-green-600 border border-border">
          <div className="flex items-center justify-between">
            <div>
        ...
        <div className="bg-surface rounded-lg shadow-md p-6 border-l-4 border-blue-600 border border-border">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-muted text-sm font-medium">History Records</p>
              <p className="text-3xl font-bold text-blue-600">{historyCount}</p>
            </div>
            <Check className="w-12 h-12 text-blue-200" />
          </div>
      ...
      {/* Filters: neutral wrapper themed */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="relative">
            <input
              ...
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Search by employee name, ID, or training..."
              className="w-full px-5 py-4 pl-12 border-2 border-border rounded-2xl shadow-sm focus:outline-none focus:ring-4 focus:ring-purple-100 focus:border-purple-500 bg-background text-base font-medium transition-all text-text placeholder:text-muted"
            />
          ...
          <div>
            <select
              value={filterStatus}
              onChange={(e) => setFilterStatus(e.target.value as any)}
              className="w-full px-5 py-4 border-2 border-border rounded-2xl focus:outline-none focus:ring-4 focus:ring-purple-100 focus:border-purple-500 bg-background shadow-sm transition-all text-base font-medium text-text"
            >
              <option value="all">All Records</option>
              <option value="pending">Pending Only</option>
              <option value="completed">Rescheduled Only</option>
          ...
          <button
            ...
            className="w-full px-5 py-4 border-2 border-border rounded-2xl focus:outline-none focus:ring-4 focus:ring-purple-100 focus:border-purple-500 bg-background shadow-sm transition-all text-base font-medium text-text hover:bg-surface"
          >
            Reset Filters
          </button>
        </div>
      ...
      {loading ? (
        ...
        filteredHistory.length === 0 ? (
          <div className="bg-surface rounded-lg shadow-md p-12 text-center border border-border">
            <CheckCircle className="w-16 h-16 text-muted mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-text mb-2">No History Records</h3>
            <p className="text-muted">No employees have been marked present after rescheduling yet.</p>
          </div>
        ) : (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-border">
                  {filteredHistory.map((history) => (
                    <tr key={history.id} className="hover:bg-background transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="font-semibold text-text">{getEmployeeName(history.employee)}</div>
                        <div className="text-sm text-muted">ID: {history.employee?.emp_id || '-'}</div>
                      </td>

                      <td className="px-6 py-4">
                        <div className="font-semibold text-text">{getTrainingName(history.original_schedule)}</div>
                      ...
                      <td className="px-6 py-4">
                        <div className="space-y-1">
                          ...
                          <div className="mt-2">
                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(history.original_status)}`}>
                              Original:{' '}
                              {history.original_status === 'absent' ? '❌ Absent' : '📅 Rescheduled'}
                            </span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="space-y-2">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold border ${getStatusColor(history.final_status)}`}>
                            {history.final_status === 'present'
                              ? '✅ Present'
                              : history.final_status === 'absent'
                                ? '❌ Absent'
        ...
        <div className="bg-surface rounded-lg shadow-md overflow-hidden border border-border">
          {/* (rest of your table remains the same; neutrals already adjusted above where needed) */}
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-background border-b border-border">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-muted uppercase tracking-wider">Employee</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-muted uppercase tracking-wider">Training</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-muted uppercase tracking-wider">Original Date</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-muted uppercase tracking-wider">Rescheduled Date</th>
              ...
              <tbody className="divide-y divide-border">
                {filteredReschedules.map((reschedule) => (
                  <React.Fragment key={reschedule.id}>
                    <tr className={`hover:bg-background ${editingId === reschedule.id ? 'bg-blue-50/30 dark:bg-blue-500/10' : ''}`}>
                      {/* your existing row content unchanged except neutral tokens where already applied above */}
                      {/* NOTE: You can keep the rest as-is; it will render correctly */}
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="font-semibold text-text">{getEmployeeName(reschedule.employee)}</div>
                        <div className="text-sm text-muted">ID: {reschedule.employee?.emp_id || '-'}</div>
                      </td>

                      <td className="px-6 py-4">
                        <div className="font-semibold text-text">{getTrainingName(reschedule.schedule)}</div>
                        <div className="text-sm text-muted">
                          {reschedule.schedule?.training_category?.name || 'Unknown Category'}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="space-y-1">
                          <div className="flex items-center text-sm text-text font-medium">
                            <Calendar className="w-4 h-4 mr-2 text-muted" />
                            {formatDate(reschedule.schedule?.date)}
                          </div>
                          <div className="flex items-center text-sm text-muted">
                      ...
                      <td className="px-6 py-4">
                        {reschedule.reschedule_date ? (
                          ...
                          <span className="text-muted/70 text-sm">Not yet rescheduled</span>
                        )}
                      </td>

                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(reschedule.status)}`}>
                          {reschedule.status === 'absent'
                            ? '❌ Absent'
                            : reschedule.status === 'present'
                              ? '✅ Present'
                      ...
                      {showActionsColumn && (
                        <td className="px-6 py-4">
                          ...
                          {/* (kept as-is to avoid any behavior changes) */}
                        </td>
                      )}
                    </tr>
                  </React.Fragment>
