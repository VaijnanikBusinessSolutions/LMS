...
const TrainingEffectivenessReport: React.FC = () => {
  ...
  const fetchDetails = async (id: number) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setDetails(data.details);
        setSelectedCategoryName(data.topic);
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const overallEffectiveness =
    summaryData.reduce((acc, item) => acc + item.effectiveness, 0) / (summaryData.length || 1);

  if (loading) {
    return (
      <div className="flex items-center justify-center h-96 bg-background text-text">
    ...
    return (
      <div className="animate-fade-in text-text">
        <button
          ...
          className="mb-6 flex items-center text-muted hover:text-purple-600 font-bold transition-colors"
        >
          <ArrowLeft className="w-5 h-5 mr-2" /> Back to Summary
        </button>
          ...
          <div className="px-8 py-6 bg-gradient-to-r from-purple-600 to-indigo-600 text-white flex justify-between items-center">
            ...
            <div className="relative">
              ...
              <input
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="pl-10 pr-4 py-2 rounded-xl bg-white text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 w-64"
              />
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-background border-b border-border">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-bold text-muted uppercase">
                      Employee
                    </th>
                    <th className="px-6 py-4 text-center text-xs font-bold text-muted uppercase">
                      Pre-Test
                    </th>
                    <th className="px-6 py-4 text-center text-xs font-bold text-muted uppercase">
                      Post-Test
                ...
                <tbody className="divide-y divide-border">
                  {filteredDetails.map((emp, idx) => (
                    <tr key={idx} className="hover:bg-purple-50/60 dark:hover:bg-purple-500/10 transition-colors">
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className="bg-background text-text px-3 py-1 rounded-lg font-bold text-sm border border-border">
                          {emp.pre_score}%
                        </span>
                      </td>
  ...
  if (summaryData.length === 0) {
    return (
      <div className="p-12 text-center bg-surface rounded-3xl shadow-lg border border-border text-text">
        <BarChart3 className="w-24 h-24 text-muted mx-auto mb-4" />
        <h3 className="text-2xl font-bold text-text">No Data Available</h3>
        <p className="text-muted">
          Complete both Pre-Tests and Post-Tests to see effectiveness reports.
        </p>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-3xl p-10 text-white shadow-2xl relative overflow-hidden">
          <div className="relative z-10 flex items-center justify-between">
            ...
            <div className="bg-white/20 backdrop-blur-md p-6 rounded-2xl border border-white/30 text-center min-w-[200px]">
              <div className="text-5xl font-bold">{Math.round(overallEffectiveness)}%</div>
              <div className="text-xs font-bold uppercase tracking-wider text-indigo-100 mt-1">
                Overall Impact
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {summaryData.map((item, index) => (
            <button
              key={index}
              onClick={() => setSelectedCategoryId(item.topic_id)}
              className="text-left bg-surface rounded-2xl shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 border border-border overflow-hidden group w-full"
            >
              <div className="p-6">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <h3 className="text-xl font-bold text-text leading-tight line-clamp-2 h-14">
                      {item.topic}
                    </h3>
                  </div>
                ...
                <div className="space-y-4 mb-6">
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-muted">Pre-Test Avg</span>
                      <span className="font-bold text-text/80">{item.pre_avg}%</span>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-muted">Post-Test Avg</span>
                      <span className="font-bold text-purple-600">{item.post_avg}%</span>
                    </div>
                    <div className="h-2 bg-background rounded-full overflow-hidden border border-border">
                      <div
                        className="h-full bg-gradient-to-r from-purple-500 to-indigo-500 rounded-full"
                        style={{ width: `${item.post_avg}%` }}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border">
                  ...
                  <div className="flex items-center gap-2 text-muted text-sm group-hover:text-purple-600 transition-colors">
                    <Users className="w-4 h-4" />
                    {item.employee_count} employees
                  </div>
                </div>
