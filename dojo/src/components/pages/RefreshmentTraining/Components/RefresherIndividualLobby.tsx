  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <button
                onClick={onExit}
                className="flex items-center text-muted hover:text-text mb-2 font-bold transition-colors text-sm"
              >
                <ArrowLeft className="w-4 h-4 mr-2" /> Back to Dashboard
              </button>
              <h1 className="text-2xl font-bold text-text">{topicName}</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div className="flex gap-4">
            <div className="bg-surface px-6 py-4 rounded-2xl shadow-lg border border-border flex items-center gap-4">
              <div className="w-12 h-12 bg-orange-100 rounded-xl flex items-center justify-center">
                <Users className="w-6 h-6 text-orange-600" />
              </div>
              <div>
            ...
            <div
              className={`px-6 py-4 rounded-2xl shadow-lg border border-border flex items-center gap-4 ${
                allDone ? 'bg-green-50' : 'bg-surface'
              }`}
            >
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center ${
              ...
              <div>
                <div
                  className={`text-2xl font-bold ${
                    allDone ? 'text-green-600' : 'text-text'
                  }`}
                >
                  {totalCompleted}/{totalPresent}
                </div>
          ...
          <button
            onClick={fetchSessions}
            disabled={loading}
            className="p-3 bg-surface rounded-xl shadow-lg border border-border text-muted hover:text-purple-600 hover:bg-background transition-colors disabled:opacity-50"
          >
            <RefreshCw className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} />
          </button>
        </div>
        ...
        {loading && sessions.length === 0 ? (
          ...
          <div className="text-center py-20 bg-surface rounded-3xl shadow-xl border border-border animate-fade-in">
            <div className="w-24 h-24 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6">
              <Smile className="w-12 h-12 text-green-600" />
            </div>
            ...
            <button
              onClick={onExit}
              className="px-10 py-4 bg-gradient-to-r from-green-600 to-green-500 text-white font-bold rounded-2xl hover:shadow-xl transition-all hover:-translate-y-0.5 shadow-lg flex items-center gap-3 mx-auto"
            >
              <CheckCircle className="w-5 h-5" /> Continue
            </button>
          </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {pendingSessions.map(session => (
                    <button
                      key={session.session_id}
                      onClick={() => setActiveSessionId(session.session_id)}
                      className="bg-surface p-6 rounded-2xl shadow-lg border border-border hover:shadow-xl transition-all text-left group hover:border-purple-300 relative overflow-hidden"
                    >
                      ...
                      <h4 className="font-bold text-text text-lg mb-1 truncate relative z-10">
                        {session.employee_name}
                      </h4>
                      <p className="text-sm text-muted font-medium relative z-10">
                        {session.employee_code}
                      </p>
                      ...
                      <div className="absolute inset-0 bg-gradient-to-br from-purple-50/0 to-purple-50/30 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {completedSessions.map(session => (
                    <div
                      key={session.session_id}
                      className="bg-background p-6 rounded-2xl border border-border opacity-70"
                    >
                      <div className="flex items-start justify-between mb-4">
                        <Avatar name={session.employee_name} size="lg" className="opacity-70 grayscale" />
                        <div className="w-10 h-10 bg-green-100/50 rounded-xl flex items-center justify-center">
                          <CheckCircle className="w-5 h-5 text-green-600" />
                        </div>
                      </div>
                      <h4 className="font-bold text-muted text-lg mb-1 truncate">
                        {session.employee_name}
                      </h4>
