...
const RefresherRemoteTest: React.FC<Props> = ({
  ...
  const handleHardwareEvent = (evt: any) => {
    ...
    const remoteId = String(evt.key_id);
    const action = KEY_MAP[evt.info] || evt.info;
    const assignedSessionId = stateRef.current.assignments[remoteId];
  ...
  if (submitted && resultData) {
    ...
    return (
      <div className="min-h-screen bg-background text-text flex items-center justify-center p-4">
        ...
        <div className="bg-surface rounded-3xl shadow-lg p-10 text-center max-w-lg w-full border border-border">
          {resultData.passed ? (
            <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
              <Trophy className="w-10 h-10 text-green-600" />
            </div>
          ...
          <p className="text-muted mb-6">Group test results submitted</p>

          <div className="grid grid-cols-3 gap-3 mb-8">
            <div className="bg-background p-4 rounded-xl border border-border">
              <div className="text-xs text-muted font-bold uppercase mb-1">Participants</div>
              <div className="text-2xl font-bold text-text">{resultData.participants}</div>
            </div>

            <div className="bg-background p-4 rounded-xl border border-border">
              <div className="text-xs text-muted font-bold uppercase mb-1">Avg Score</div>
              <div className={`text-2xl font-bold ${resultData.passed ? 'text-green-600' : 'text-orange-500'}`}>
                {percentage}%
              </div>
            </div>
            ...
            <div className="bg-background p-4 rounded-xl border border-border">
              <div className="text-xs text-muted font-bold uppercase mb-1">Correct</div>
              <div className="text-2xl font-bold text-text">{resultData.totalScore}</div>
            </div>
          </div>
    ...
    return (
      <div className="min-h-screen bg-background text-text flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted font-medium">No questions available</p>
          <button
            onClick={onExit}
            className="mt-4 px-6 py-2 bg-surface border border-border rounded-xl font-bold hover:bg-background"
          >
            Go Back
          </button>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-text">{topicName}</h1>
                <div className="flex items-center gap-2 text-xs">
                  <span className="bg-purple-100 text-purple-700 px-2 py-0.5 rounded font-bold uppercase">
                    {testType}-test
                  </span>
                  ...
                  {batchId && (
                    <span className="bg-background border border-border text-muted px-2 py-0.5 rounded font-bold uppercase">
                      Batch {batchId}
                    </span>
                  )}
                </div>
            ...
            <div className="flex items-center gap-4">
              ...
              {isActive && (
                <div className="flex items-center gap-3 bg-purple-50/70 dark:bg-purple-500/10 px-4 py-2 rounded-xl border border-purple-200/50 dark:border-purple-500/20">
                  <Users className="w-5 h-5 text-purple-600" />
                  <div className="text-right">
                    <div className="text-lg font-bold text-purple-700">
                      {votesReceived}/{totalParticipants}
        ...
        {!isActive ? (
          <div className="text-center max-w-md w-full">
            <div className="bg-surface rounded-2xl shadow-lg p-8 border border-border">
              ...
              <h2 className="text-2xl font-bold text-text mb-1">Ready to Begin?</h2>
              <p className="text-muted text-sm mb-6">Group test for {totalParticipants} participants</p>

              <div className="grid grid-cols-2 gap-3 mb-6">
                <div className="bg-purple-50/70 dark:bg-purple-500/10 p-4 rounded-xl border border-purple-200/50 dark:border-purple-500/20">
                  <div className="text-xs text-purple-500 font-bold uppercase mb-1">Participants</div>
                  <div className="text-3xl font-bold text-purple-600">{totalParticipants}</div>
                </div>
                <div className="bg-background p-4 rounded-xl border border-border">
                  <div className="text-xs text-muted font-bold uppercase mb-1">Questions</div>
                  <div className="text-3xl font-bold text-text">{questions.length}</div>
                </div>
              </div>
              ...
              <p className="text-xs text-muted mt-4">
                Press <kbd className="px-1.5 py-0.5 bg-background border border-border rounded text-[10px] font-mono">→</kbd> or use remote to start
              </p>
            </div>
          </div>
        ) : (
          <div className="max-w-6xl w-full">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {/* Question Card */}
              <div className="bg-surface rounded-2xl shadow-lg p-6 flex flex-col border border-border">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-sm font-bold text-purple-600 bg-purple-50/70 dark:bg-purple-500/10 px-3 py-1.5 rounded-lg border border-purple-200/50 dark:border-purple-500/20">
                    Question {currentIndex + 1} of {questions.length}
                  </span>
                ...
                <h2 className="text-xl lg:text-2xl font-bold text-text mb-4 leading-relaxed flex-1">
                  {currentQ.question_text}
                </h2>
                ...
                <div className="mt-4 pt-4 border-t border-border">
                  <div className="flex items-center justify-between text-sm mb-2">
                    <span className="text-muted font-medium">Response Progress</span>
                    <span className="font-bold text-text/80">{votesReceived}/{totalParticipants}</span>
                  </div>
                  <div className="h-2 bg-background rounded-full overflow-hidden border border-border">
                    <div
                      className={`h-full transition-all duration-500 ${
                        votesReceived === totalParticipants ? 'bg-green-500' : 'bg-purple-500'
                      }`}
                      style={{ width: `${(votesReceived / totalParticipants) * 100}%` }}
                    />
                  </div>
              ...
              <div className="flex flex-col gap-2">
                {['A', 'B', 'C', 'D'].map((opt) => {
                  ...
                  return (
                    <div
                      key={opt}
                      className="bg-surface rounded-xl shadow-sm border border-border overflow-hidden flex items-stretch hover:border-border/80 transition-colors"
                    >
                      <div className="w-14 flex items-center justify-center text-xl font-bold flex-shrink-0 bg-background text-muted border-r border-border">
                        {opt}
                      </div>
                      ...
                      <div className="flex-1 p-4 flex items-center">
                        ...
                        <span className="text-base font-medium text-text/80">{txt}</span>
                      </div>
                    </div>
                  );
                })}
                ...
                {/* Participant Grid */}
                <div className="bg-surface rounded-xl shadow-sm border border-border p-4 mt-2">
                  <div className="text-xs font-bold text-muted uppercase mb-3">Participant Status</div>
                  <div className="grid grid-cols-5 gap-2">
                    {Object.entries(assignments).map(([remoteId, sessionId]) => {
                      const hasAnswered = answers[sessionId]?.[currentIndex];
                      return (
                        <div
                          ...
                          className={`
                            ...
                            ${hasAnswered
                              ? 'bg-green-100 text-green-700 border-2 border-green-300'
                              : 'bg-background text-muted border-2 border-transparent'
                            }
                          `}
                        >
                          {remoteId}
        ...
        <div className="fixed bottom-0 left-0 w-full bg-surface border-t border-border shadow-lg">
          <div className="max-w-6xl mx-auto px-4 py-3 flex justify-between items-center">
            <button
              onClick={() => changeQuestion(-1)}
              disabled={currentIndex === 0}
              className="px-5 py-2.5 font-bold text-muted hover:text-text hover:bg-background rounded-lg disabled:opacity-30 disabled:hover:bg-transparent transition-colors flex items-center gap-1.5"
            >
              <ChevronLeft className="w-5 h-5" /> Previous
            </button>
