...
const Status: React.FC = () => {
  ...
  const availableTopics = useMemo(() => {
    if (selectedCategory === 'all') return trainingTopics;
    return trainingTopics.filter(
      (topic) => topic.category && topic.category.id === parseInt(selectedCategory)
    );
  }, [selectedCategory, trainingTopics]);
  ...
  const categoryTopicStats = useMemo(() => {
    ...
    filteredSessions.forEach((session) => {
      ...
      session.topics.forEach((topic) => {
        ...
        if (selectedCategory !== 'all' && categoryId !== parseInt(selectedCategory)) return;
        if (selectedTopic !== 'all' && topicId !== parseInt(selectedTopic)) return;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted font-medium">Analyze training performance and impact</p>
          </div>

          <div className="bg-surface p-1.5 rounded-xl shadow-sm border border-border flex gap-2">
            <button
              onClick={() => setActiveTab('completion')}
              className={`flex items-center gap-2 px-6 py-3 rounded-lg font-bold transition-all ${
                activeTab === 'completion'
                  ? 'bg-purple-600 text-white shadow-md'
                  : 'text-muted hover:bg-purple-50/70 dark:hover:bg-purple-500/10 hover:text-purple-600'
              }`}
            >
              <PieChart className="w-5 h-5" /> Completion Status
            </button>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-lg font-bold transition-all ${
                activeTab === 'effectiveness'
                  ? 'bg-pink-600 text-white shadow-md'
                  : 'text-muted hover:bg-pink-50/70 dark:hover:bg-pink-500/10 hover:text-pink-600'
              }`}
            >
              <BarChart2 className="w-5 h-5" /> Effectiveness Report
            </button>
        ...
        {activeTab === 'effectiveness' ? (
          ...
          <div className="animate-fade-in space-y-6">
            {/* Filters */}
            <div className="bg-surface rounded-3xl shadow-xl p-8 border border-border">
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
                <div>
                  <label className="block text-sm font-semibold text-text mb-2">Month</label>
                  <select
                    value={selectedMonth}
                    onChange={(e) => setSelectedMonth(parseInt(e.target.value))}
                    disabled={showYearOnly}
                    className="w-full px-4 py-3 border-2 border-purple-200 rounded-xl bg-background text-text focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent disabled:bg-background/60 disabled:cursor-not-allowed transition-all font-medium"
                  >
                    {monthNames.map((month, index) => (
                ...
                <div>
                  ...
                  <select
                    value={selectedYear}
                    onChange={(e) => setSelectedYear(parseInt(e.target.value))}
                    className="w-full px-4 py-3 border-2 border-purple-200 rounded-xl bg-background text-text focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all font-medium"
                  >
                    {Array.from({ length: 5 }, (_, i) => new Date().getFullYear() - 2 + i).map((year) => (
                ...
                <div>
                  <label className="block text-sm font-semibold text-text mb-2">View Mode</label>
                  <div className="flex gap-2 h-12">
                    <button
                      onClick={() => setShowYearOnly(false)}
                      className={`flex-1 px-4 rounded-xl font-semibold transition-all ${
                        !showYearOnly
                          ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg shadow-purple-500/30'
                          : 'bg-background text-muted hover:bg-purple-50/70 dark:hover:bg-purple-500/10'
                      }`}
                    >
                      Monthly
                    </button>
              ...
              <div className="flex items-center gap-2 text-sm">
                <span className="font-semibold text-text">Active Period:</span>
                <span className="px-3 py-1 bg-purple-100 text-purple-700 rounded-lg font-medium">
                  {showYearOnly ? `Year ${selectedYear}` : `${monthNames[selectedMonth]} ${selectedYear}`}
                </span>
                {selectedCategory !== 'all' && (
            ...
            {/* Statistic Cards (neutral wrapper themed, gradients kept) */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {[
                {
                  label: 'Training Categories',
                ...
                <div
                  key={label}
                  className="bg-surface rounded-2xl shadow-lg p-6 border border-border hover:shadow-xl transition-all transform hover:scale-[1.02]"
                >
                  <div className={`inline-flex p-3 rounded-xl bg-gradient-to-br ${bg} mb-4`}>
                    <div className={`bg-gradient-to-r ${gradient} bg-clip-text text-transparent`}>
                      {icon}
                  ...
                  <p className="text-xs text-muted/80 font-medium">{subtitle}</p>
                </div>
              ))}
            </div>

            {/* Data Table wrapper themed */}
            <div className="bg-surface rounded-3xl shadow-xl overflow-hidden border border-border">
              <div className="px-8 py-6 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-950/30 dark:to-pink-950/20 border-b border-border">
                <h3 className="text-2xl font-bold text-text mb-2">Training Completion Details</h3>
                    ...
                    {categoryTopicStats.length === 0 ? (
                      ...
                      categoryTopicStats.map((stat) =>
                        stat.sessions.map((session, sessionIndex) => {
                          ...
                          const completionRate =
                            session.totalEmployees > 0
                              ? Math.round((session.completedEmployees / session.totalEmployees) * 100)
                              : 0;

                          return (
                            <tr
                              key={`${stat.categoryId}-${stat.topicId}-${session.sessionId}`}
                              className="hover:bg-purple-50/70 dark:hover:bg-purple-500/10 transition-colors"
                            >
                              {isFirstRow && (
                                <td className="px-6 py-4 whitespace-nowrap align-top" rowSpan={rowSpan}>
                                  <div className="text-sm font-bold text-text">{stat.categoryName}</div>
                                </td>
                              )}

                              {isFirstRow && (
                                <td className="px-6 py-4 align-top" rowSpan={rowSpan}>
                                  <div className="text-sm font-bold text-text">{stat.topicName}</div>
                                  <div className="text-xs text-muted font-medium mt-1 px-2 py-1 bg-purple-50 dark:bg-purple-500/10 rounded-md inline-block">
                                    {stat.sessions.length} session(s)
                                  </div>
                                </td>
                              )}
                              ...
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-2">
                                  <Calendar className="w-4 h-4 text-purple-500" />
                                  <span className="text-sm font-semibold text-text">{formatDate(session.date)}</span>
                                </div>
                              </td>
