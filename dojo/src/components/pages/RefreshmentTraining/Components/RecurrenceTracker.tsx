...
const RecurrenceTracker: React.FC = () => {
  ...
  // Updated to return border/bg styles compatible with the theme
  const getStatusStyles = (recurrence: RecurrenceInfo) => {
    if (recurrence.recurring_schedule_created)
      return "border-green-200 bg-green-50/30"
    
    const daysUntil = getDaysUntil(recurrence.next_training_creation_date)
    
    if (daysUntil === null) return "border-gray-200 bg-surface"
    if (daysUntil <= 0) return "border-red-200 bg-red-50/30"
    if (daysUntil <= 7) return "border-yellow-200 bg-yellow-50/30"
  ...
  const getStatusIcon = (recurrence: RecurrenceInfo) => {
    if (recurrence.recurring_schedule_created)
      return <CheckCircle2 className="w-5 h-5 text-green-600" />
    const daysUntil = getDaysUntil(recurrence.next_training_creation_date)
    if (daysUntil === null) return <AlertCircle className="w-5 h-5 text-muted" />
    if (daysUntil <= 0) return <XCircle className="w-5 h-5 text-red-600" />
    return <Clock className="w-5 h-5 text-blue-600" />
  }
      ...
      <div className="max-w-1920px mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-surface p-6 rounded-2xl border border-border shadow-lg">
            ...
            <div className="text-sm text-muted font-bold uppercase tracking-wider">Total Recurring</div>
          </div>
          ...
          <div className="bg-surface p-6 rounded-2xl border border-border shadow-lg">
            ...
            <div className="text-sm text-muted font-bold uppercase tracking-wider">Pending</div>
          </div>
          ...
          <div className="bg-surface p-6 rounded-2xl border border-border shadow-lg">
            ...
            <div className="text-sm text-muted font-bold uppercase tracking-wider">Created</div>
          </div>
          ...
          <div className="bg-surface p-6 rounded-2xl border border-border shadow-lg">
            ...
            <div className="text-sm text-muted font-bold uppercase tracking-wider">Overdue</div>
          </div>
        </div>

        {/* Filter and Search */}
        <div className="bg-surface rounded-3xl shadow-xl border border-border mb-8 overflow-hidden">
          <div className="bg-gradient-to-r from-purple-600 via-purple-500 to-pink-500 px-8 py-6">
          ...
          <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div id="statusFilter" className="flex space-x-3">
                {["all", "pending", "created"].map((status) => (
                  <button
                    key={status}
                    onClick={() => setFilterStatus(status as any)}
                    className={`flex-1 px-5 py-4 rounded-2xl text-base font-bold transition-all shadow-sm focus:outline-none border ${
                      filterStatus === status
                        ? "bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg border-transparent"
                        : "bg-background text-muted border-border hover:bg-gray-50 hover:text-text"
                    }`}
                  >
                    {status.charAt(0).toUpperCase() + status.slice(1)}
                  </button>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setSearchTerm(e.target.value)}
                placeholder="Search by category, training, or trainer..."
                className="w-full px-5 py-4 border-2 border-border rounded-2xl focus:outline-none focus:ring-4 focus:ring-purple-100 focus:border-purple-500 bg-background text-text shadow-sm transition-all text-base font-medium"
              />
            </div>
          </div>
        ...
        <div className="bg-surface rounded-3xl shadow-xl border border-border overflow-hidden">
          <div className="bg-surface px-8 py-6 border-b border-border">
            <div className="flex items-center justify-between">
              ...
              <p className="text-base text-muted mt-1 font-medium">All recurring trainings with their next scheduled dates</p>
            </div>
            <span className="bg-purple-100 text-purple-800 px-6 py-3 rounded-2xl text-base font-bold mt-[20px] inline-block">
              {filteredRecurrences.length} schedule{filteredRecurrences.length !== 1 ? "s" : ""}
            </span>
          </div>
          ...
          <div className="p-8 bg-background">
            {loading ? (
              ...
              <div className="text-center py-16">
                <Calendar className="w-20 h-20 text-muted mx-auto mb-6" />
                <h3 className="text-2xl font-bold text-text mb-3">
                  No Recurring Schedules Found
                </h3>
                <p className="text-muted font-medium text-lg mb-6">No recurring training schedules match your filters.</p>
                <button
                  onClick={() => {
                    setFilterStatus("all")
                    setSearchTerm("")
              ...
              <div className="space-y-6">
                {filteredRecurrences.map((recurrence) => {
                  ...
                  return (
                    <div 
                      key={recurrence.id} 
                      className={`bg-surface rounded-3xl shadow-md border-2 p-6 hover:shadow-xl transition-all duration-200 ${getStatusStyles(recurrence)}`}
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            {getStatusIcon(recurrence)}
                            <h3 className="text-lg font-bold text-text">{recurrence.training_name}</h3>
                          </div>
                          <p className="text-base text-muted font-medium">{recurrence.training_category}</p>
                        </div>
                        <div>
                          <span
                      ...
                      <div className="space-y-3 text-base text-text font-medium">
                        <div className="flex items-center justify-between">
                          <span className="text-muted">Recurrence Interval</span>
                          <span>
                            Every{" "}
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-bold text-text">{formatDate(recurrence.completed_date)}</span>
                        </div>

                        <div className="border-t border-border pt-4 mt-4">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-text font-bold">Next Training</span>
                            <span className="font-bold text-purple-700 text-lg">
                              {editingId === recurrence.id ? (
                                <input
                                  type="date"
                                  value={newNextTrainingDate}
                                  onChange={(e) => setNewNextTrainingDate(e.target.value)}
                                  className="border border-border bg-background px-3 py-2 rounded-xl text-text focus:ring-2 focus:ring-purple-200 outline-none"
                                />
                              ) : recurrence.next_training_date_override ? (
                                <>
                                  {formatDate(recurrence.next_training_date_override)}{" "}
                                  <small className="text-sm text-muted">(Overridden)</small>
                                </>
                              ) : (
                                formatDate(recurrence.next_training_date)
                              )}
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-sm text-right font-medium text-muted">
                              {daysUntilTraining !== null &&
                                (daysUntilTraining === 0
                                  ? "In 0 days Today"
                                  : `In ${daysUntilTraining} day${daysUntilTraining !== 1 ? "s" : ""}`)}
                          ...
                          {editingId === recurrence.id ? (
                            <div className="mt-4 flex gap-3 justify-end">
                              <button
                                onClick={() => handleSaveNextTrainingDate(recurrence.id)}
                                className="bg-purple-600 text-white px-6 py-2 rounded-xl font-bold hover:bg-purple-700 transition shadow-md"
                              >
                                Save
                              </button>
                              <button
                                onClick={handleCancelEdit}
                                className="bg-background border border-border text-text px-6 py-2 rounded-xl font-bold hover:bg-gray-50 transition"
                              >
                                Cancel
                              </button>
                            </div>
                            ...
                            <div className="mt-4 flex justify-end">
                              <button
                                onClick={() => handleEditClick(recurrence.id, recurrence.next_training_date)}
                                className="bg-background border border-border text-purple-600 px-6 py-2 rounded-xl font-bold hover:bg-purple-50 hover:border-purple-200 transition"
                              >
                                Change Date
                              </button>
                            </div>
                        ...
                        <div className="flex items-center justify-between text-muted font-medium mt-6 pt-3 border-t border-border">
                          <span>Schedule Creates</span>
                        ...
                        <div className="flex items-center justify-between text-muted font-medium mt-2">
                          <span>Trainers</span>
                          <span className="font-bold text-text">{recurrence.trainer}</span>
                        </div>

                        <div className="flex items-center justify-between text-muted font-medium mt-2">
                          <span>Venues</span>
                          <span className="font-bold text-text">{recurrence.venue}</span>
                        </div>

                        <div className="flex items-center justify-between text-muted font-medium mt-2">
                          <span>Employees</span>
                          <span className="font-bold text-text">{recurrence.employee_count}</span>
                        </div>
                      </div>
          ...
          {/* Legend */}
          <div className="mt-8 bg-surface rounded-3xl shadow-xl border border-border overflow-hidden">
            <div className="bg-surface px-8 py-6 border-b border-border">
              <h3 className="text-xl font-bold text-text">
                Status Legend
              </h3>
            </div>
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-background">
              <div className="flex items-center space-x-3">
                <div className="w-4 h-4 bg-blue-50 border border-blue-200 rounded-full"></div>
                <span className="text-muted font-medium">Scheduled - Waiting for creation date</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-4 h-4 bg-yellow-50 border border-yellow-200 rounded-full"></div>
                <span className="text-muted font-medium">Due Soon - Creates within 7 days</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-4 h-4 bg-red-50 border border-red-200 rounded-full"></div>
                <span className="text-muted font-medium">Overdue - Past creation date</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-4 h-4 bg-green-50 border border-green-200 rounded-full"></div>
                <span className="text-muted font-medium">Completed - Schedule created successfully</span>
              </div>
            </div>
          </div>
        </div>
