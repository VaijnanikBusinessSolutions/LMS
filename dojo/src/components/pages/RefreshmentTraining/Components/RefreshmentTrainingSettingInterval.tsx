    ...
    <div className="space-y-6 p-6 bg-background text-text">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted mt-2">Configure automatic recurring training schedules</p>
        </div>
        <button
          onClick={() => setShowAddForm(!showAddForm)}
          className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors flex items-center space-x-2"
      ...
      {showAddForm && (
        <div className="bg-surface rounded-lg shadow-md p-6 border border-border">
          <h3 className="text-lg font-semibold text-text mb-4">Add New Recurrence Interval</h3>
          <form onSubmit={handleAddInterval} className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewInterval(e.target.value)}
                placeholder="Enter number of months (e.g., 3, 6)"
                className="w-full px-4 py-2 border border-border bg-background text-text rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-muted"
                required
              />
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                className="flex-1 bg-background border border-border text-text py-2 rounded-md hover:bg-surface"
              >
                Cancel
              </button>
            </div>
      ...
      {/* Current Active Interval */}
      {activeInterval && (
        <div className="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-950/30 dark:to-blue-900/20 rounded-lg shadow-md p-6 border-2 border-blue-300 dark:border-blue-500/30">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-blue-600 dark:text-blue-200/80 mt-2">
                Trainings will automatically reschedule every {activeInterval.interval_months} month
                {activeInterval.interval_months !== 1 ? 's' : ''}
              </p>
            </div>
      ...
      <div className="bg-surface rounded-lg shadow-md overflow-hidden border border-border">
        <div className="px-6 py-4 border-b border-border">
          <h3 className="text-lg font-semibold text-text">All Recurrence Intervals</h3>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="bg-surface divide-y divide-border">
              {intervals.length === 0 ? (
                ...
                intervals.map((interval) => (
                  <tr key={interval.id} className="hover:bg-background transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {interval.is_active ? (
                        ...
                        <span className="px-3 py-1 text-xs font-medium rounded-full bg-background text-text border border-border">
                          Inactive
                        </span>
                      )}
                    </td>
