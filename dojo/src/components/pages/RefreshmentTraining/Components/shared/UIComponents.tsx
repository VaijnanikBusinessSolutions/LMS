...
// --- COLOR UTILITY FOR AVATARS ---
// These remain hardcoded as they provide specific visual identification
const AVATAR_COLORS = [
  'bg-purple-500', 'bg-blue-500', 'bg-green-500', 'bg-orange-500',
  'bg-pink-500', 'bg-indigo-500', 'bg-teal-500', 'bg-rose-500',
  'bg-cyan-500', 'bg-amber-500', 'bg-violet-500', 'bg-emerald-500'
...
export const Avatar: React.FC<AvatarProps> = ({ name, size = 'md', className = '' }) => {
  ...
  return (
    <div className={`${sizeClasses[size]} ${getAvatarColor(name)} rounded-full flex items-center justify-center text-white font-bold shadow-md ring-2 ring-surface ${className}`}>
      {getInitials(name)}
    </div>
  );
};
    ...
    <div className={`${config.container} relative`}>
      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
        ...
        <circle
          ...
          r={radius}
          fill="none"
          // Using a subtle border color for the track
          className={isWarning ? 'stroke-red-100' : 'stroke-border'}
          strokeWidth={config.stroke}
        />
        {/* Progress circle */}
        <circle
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      {stages.map((stage, idx) => {
        ...
        return (
          <React.Fragment key={stepNum}>
            <div className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all border ${
              isActive 
            ...
            {idx < stages.length - 1 && (
              <div className={`w-8 h-1 rounded-full transition-all ${
                currentStage > stepNum ? 'bg-green-300' : 'bg-border'
              }`} />
            )}
          </React.Fragment>
        );
...
export const AttendanceChips: React.FC<AttendanceChipsProps> = ({ value, onChange, disabled }) => {
  ...
  const getChipClasses = (status: string, color: string, isSelected: boolean) => {
    ...
    if (isSelected) {
      const selectedColors: Record<string, string> = {
        green: 'bg-green-500 text-white border-green-500 shadow-md shadow-green-200',
        yellow: 'bg-yellow-500 text-white border-yellow-500 shadow-md shadow-yellow-200',
        red: 'bg-red-500 text-white border-red-500 shadow-md shadow-red-200'
      };
      return `${baseClasses} ${selectedColors[color]}`;
    }
    
    const unselectedColors: Record<string, string> = {
      green: 'bg-surface text-green-600 border-green-200 hover:bg-green-50 hover:border-green-300',
      yellow: 'bg-surface text-yellow-600 border-yellow-200 hover:bg-yellow-50 hover:border-yellow-300',
      red: 'bg-surface text-red-600 border-red-200 hover:bg-red-50 hover:border-red-300'
    };
    return `${baseClasses} ${unselectedColors[color]}`;
  };
...
export const StickyHeader: React.FC<StickyHeaderProps> = ({ children, className = '' }) => {
  return (
    <div className={`sticky top-0 z-40 bg-surface/95 backdrop-blur-sm border-b border-border shadow-sm ${className}`}>
      {children}
    </div>
  );
};