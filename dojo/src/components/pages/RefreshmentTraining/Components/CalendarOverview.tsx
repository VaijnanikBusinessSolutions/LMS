  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push(
        <div
          key={`empty-${i}`}
          className="h-28 border border-border bg-background/50"
        ></div>
      );
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          key={day}
          onClick={() => handleDateClick(day)}
          className={`h-28 border border-border p-2 cursor-pointer hover:bg-surface transition-colors ${
            isSelected
              ? 'bg-blue-50/50 border-blue-300 ring-1 ring-inset ring-blue-300'
              : 'bg-background'
          } ${isToday ? 'bg-purple-50/50 border-purple-300' : ''}`}
        >
          <div
            className={`text-sm font-bold mb-1 w-7 h-7 flex items-center justify-center rounded-full ${
              isToday ? 'bg-purple-600 text-white' : 'text-text'
          ...
          <div className="space-y-1 overflow-hidden">
            {dayTrainings.slice(0, 2).map(session => {
              ...
              return (
                <div
                  key={session.id}
                  className={`text-xs px-2 py-1 rounded-md font-semibold truncate border ${getStatusColor(
                    session.status
                  )}`}
                  title={`${title} - ${session.trainer?.name ?? ''}`}
                >
            ...
            {dayTrainings.length > 2 && (
              <div className="text-xs text-muted font-bold px-2">
                +{dayTrainings.length - 2} more
              </div>
            )}
          </div>
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center space-x-4">
              <button
                onClick={() => navigateMonth('prev')}
                className="p-3 rounded-xl border-2 border-border bg-surface hover:bg-background text-text transition-colors shadow-sm"
              >
                <ChevronLeft className="w-6 h-6" />
              </button>
              <h3
                className="text-2xl font-bold text-text min-w-64 text-center"
              >
                {currentMonth}
              </h3>
              <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-surface rounded-3xl shadow-xl border border-border overflow-hidden">
              <div className="grid grid-cols-7 bg-gradient-to-r from-purple-50 to-pink-50 border-b border-border">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(
                  day => (
                    <div
                      key={day}
                      className="px-4 py-4 text-center text-sm font-bold text-purple-900 uppercase tracking-wider border-r border-border last:border-r-0"
                    >
                      {day}
                    </div>
                  )
          ...
          <div className="space-y-6">
            <div
              className="bg-surface rounded-3xl shadow-xl border border-border overflow-hidden flex flex-col h-full max-h-[800px]"
            >
              <div className="bg-gradient-to-r from-purple-600 via-purple-500 to-pink-500 px-6 py-5 flex-shrink-0">
                <h4
                  className="text-xl font-bold text-white flex items-center"
              ...
              {selectedDate ? (
                <div className="flex-1 overflow-y-auto p-6 bg-background">
                  {getSelectedDateTrainings().length > 0 ? (
                    <div className="space-y-4">
                      {getSelectedDateTrainings().map(session => {
                        ...
                        return (
                          <div
                            key={session.id}
                            className="bg-surface border border-border rounded-2xl p-5 hover:shadow-lg transition-all hover:-translate-y-0.5"
                          >
                            <div className="flex items-center justify-between mb-3">
                              <h5
                            ...
                            {session.training_category?.name && (
                              <div className="flex items-center space-x-2 mb-3 text-sm text-blue-700 bg-blue-50 px-3 py-2 rounded-lg border border-blue-100">
                                <Building2 className="w-4 h-4 text-blue-600" />
                                <span className="font-semibold">
                                  {session.training_category.name}
                                </span>
                            ...
                            <div className="space-y-3 text-sm text-text">
                              <div className="flex items-center space-x-3 bg-background px-3 py-2 rounded-lg border border-border">
                                <Clock className="w-5 h-5 text-purple-600 flex-shrink-0" />
                                <span className="font-semibold">
                                  {session.time}
                                </span>
                    ...
                    <div className="text-center py-16">
                      <div className="w-20 h-20 bg-surface border border-border rounded-3xl mx-auto mb-4 flex items-center justify-center">
                        <CalendarIcon className="w-10 h-10 text-muted" />
                      </div>
                      <p className="text-text font-semibold text-lg">
                        No trainings scheduled
                      </p>
                      <p className="text-muted mt-2">for this date</p>
                    </div>
                  )}
                </div>
              ) : (
            ...
            {/* Status Legend */}
            <div className="bg-surface rounded-3xl shadow-xl border border-border p-6">
              <h4
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center space-x-3">
                  <div className="w-5 h-5 bg-yellow-100 border-2 border-yellow-200 rounded-lg"></div>
                  <span className="text-sm font-semibold text-muted">
                    Pending
                  </span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-5 h-5 bg-red-100 border-2 border-red-200 rounded-lg"></div>
                  <span className="text-sm font-semibold text-muted">
                    Cancelled
                  </span>
                </div>
