...
import React, { useState, useEffect } from 'react';
import { 
  ArrowLeft, CheckCircle, Lock, Monitor, 
  MousePointer
} from 'lucide-react';
import { FileText, Image as ImageIcon, Link as LinkIcon } from "lucide-react";
...
const RefresherBatchView: React.FC<Props> = ({ 
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center justify-between">
        <button onClick={onBack} className="flex items-center text-muted hover:text-purple-600 font-bold transition-colors">
          <ArrowLeft className="w-5 h-5 mr-2" /> Back to Staging
        </button>
        <div className="text-right">
          <h2 className="text-2xl font-bold text-text">{batch.name}</h2>
          <p className="text-muted text-sm">{stats.total_employees} Employees</p>
        </div>
      </div>

      {/* STEP 1: PRE-TEST */}
      <div 
        className={`rounded-3xl shadow-lg border p-8 transition-all duration-300
          ${isPreTestDone 
            ? 'border-green-200 bg-green-50/10' 
            : 'border-purple-200 bg-surface'
          }
        `}
      >
        <div className="flex justify-between items-start mb-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold shadow-sm
                ${isPreTestDone ? 'bg-green-600 text-white' : 'bg-purple-600 text-white'}
              `}>1</span>
              <h3 className="text-xl font-bold text-text">Pre-Test Assessment</h3>
            </div>
            <p className="text-muted ml-11">Assess initial knowledge before training.</p>
          </div>
          <div className="text-right">
            <span className={`text-2xl font-bold ${isPreTestDone ? 'text-green-600' : 'text-purple-600'}`}>
              {stats.pre_completed}/{stats.total_employees}
            </span>
            <p className="text-xs uppercase font-bold text-muted">Completed</p>
          </div>
        </div>
        ...
        {!isPreTestDone ? (
          <div className="ml-11 grid grid-cols-1 md:grid-cols-2 gap-4">
            <button 
              onClick={() => onLaunchTest('pre', 'individual')}
              className="p-4 border-2 border-dashed border-border rounded-xl hover:border-purple-500 hover:bg-purple-50/50 transition-all flex items-center justify-center gap-3 group bg-background"
            >
              <MousePointer className="w-6 h-6 text-muted group-hover:text-purple-600" />
              <span className="font-bold text-text group-hover:text-purple-700">Individual Mode</span>
            </button>
            <button 
              onClick={() => onLaunchTest('pre', 'remote')}
              className="p-4 border-2 border-dashed border-border rounded-xl hover:border-purple-500 hover:bg-purple-50/50 transition-all flex items-center justify-center gap-3 group bg-background"
            >
              <Monitor className="w-6 h-6 text-muted group-hover:text-purple-600" />
              <span className="font-bold text-text group-hover:text-purple-700">Remote Group Mode</span>
            </button>
          </div>
        ) : (
          <div className="ml-11 flex items-center gap-2 text-green-600 font-bold bg-green-100 px-4 py-2 rounded-xl w-fit">
      ...
      <div 
        className={`rounded-3xl shadow-lg border p-8 transition-all duration-300
          ${!isPreTestDone 
            ? 'opacity-50 grayscale bg-background border-border' 
      ...
      >
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-center gap-3">
             <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold shadow-sm
               ${isPreTestDone ? 'bg-blue-600 text-white' : 'bg-background border border-border text-muted'}
             `}>2</span>
             <h3 className="text-xl font-bold text-text">Training Content</h3>
          </div>
          {!isPreTestDone && <Lock className="w-6 h-6 text-muted" />}
        </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
               {topicFiles.length > 0 ? topicFiles.map(file => (
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer"
                    className="flex items-center gap-3 p-3 border border-border rounded-xl hover:shadow-md hover:border-blue-300 transition-all bg-background"
                  >
                    <div className="p-2 bg-surface border border-border rounded-lg">{getFileIcon(file.content_type)}</div>
                    <span className="font-bold text-text truncate">{file.content_name}</span>
                  </a>
               )) : <p className="text-muted italic">No content uploaded.</p>}
             </div>
           ) : (
             <p className="text-muted">Complete Pre-Test to unlock training materials.</p>
           )}
        </div>
      </div>
      ...
      >
        <div className="flex justify-between items-start mb-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold shadow-sm
                ${isPreTestDone ? 'bg-orange-500 text-white' : 'bg-background border border-border text-muted'}
              `}>3</span>
              <h3 className="text-xl font-bold text-text">Post-Test Assessment</h3>
            </div>
            <p className="text-muted ml-11">Verify knowledge retention.</p>
          </div>
          {!isPreTestDone ? (
        ...
        {isPreTestDone ? (
           !isPostTestDone ? (
            <div className="ml-11 grid grid-cols-1 md:grid-cols-2 gap-4">
              <button 
                onClick={() => onLaunchTest('post', 'individual')}
                className="p-4 border-2 border-dashed border-border rounded-xl hover:border-orange-500 hover:bg-orange-50/50 transition-all flex items-center justify-center gap-3 group bg-background"
              >
                <MousePointer className="w-6 h-6 text-muted group-hover:text-orange-600" />
                <span className="font-bold text-text group-hover:text-orange-700">Individual Mode</span>
              </button>
              <button 
                onClick={() => onLaunchTest('post', 'remote')}
                className="p-4 border-2 border-dashed border-border rounded-xl hover:border-orange-500 hover:bg-orange-50/50 transition-all flex items-center justify-center gap-3 group bg-background"
              >
                <Monitor className="w-6 h-6 text-muted group-hover:text-orange-600" />
                <span className="font-bold text-text group-hover:text-orange-700">Remote Group Mode</span>
              </button>
            </div>
           ) : (
            <div className="ml-11 flex items-center gap-2 text-green-600 font-bold bg-green-100 px-4 py-2 rounded-xl w-fit">
          ...
          <div className="ml-11 p-4 bg-background border border-border rounded-xl text-muted text-sm font-medium">
             Locked until Pre-Test is completed.
          </div>
        )}
      </div>
