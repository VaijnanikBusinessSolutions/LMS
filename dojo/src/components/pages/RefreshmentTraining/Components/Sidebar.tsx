...
import React from 'react';
import { Calendar, Bell, BookOpen, Users, BarChart3, RefreshCw, UserX, Clock } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeModule, setActiveModule }) => {
  ...
  return (
    <div className="sticky top-0 h-screen w-72 bg-surface text-text shadow-xl border-r border-border flex flex-col">
      {/* Header */}
      <div className="p-8 border-b border-border flex-shrink-0 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-950/30 dark:to-pink-950/20">
        <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
          Training Module
        </h1>
        <p className="text-sm text-muted mt-2 font-medium">Organize your content</p>
      </div>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center space-x-4 px-5 py-4 rounded-2xl transition-all duration-200 font-semibold text-base group ${
                isActive
                  ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg shadow-purple-500/30 transform scale-[1.02]'
                  : 'text-text hover:bg-purple-50/70 dark:hover:bg-purple-500/10 hover:text-purple-700'
              }`}
            >
              <div
                className={`p-2 rounded-xl transition-colors ${
                  isActive ? 'bg-white/20' : 'bg-background border border-border group-hover:bg-purple-100/60 dark:group-hover:bg-purple-500/10'
                }`}
              >
                <Icon className={`w-5 h-5 ${isActive ? 'text-white' : 'text-purple-600'}`} />
              </div>
