...
//       try {
//         const response = await fetch("http://127.0.0.1:8000/multiskilling/");
...
// --- Icons (kept intentional stroke colors as-is) ---
const NotificationIcon = () => (
  <svg width="24" height="24" fill="none" stroke="#275080" strokeWidth="1.5" viewBox="0 0 24 24">
    <path d="M12 22a2 2 0 1 0 0-4 2 2 0 0 0 0 4ZM5 18h14M18 7a6 6 0 1 0-12 0v7a2 2 0 0 1-2 2h16a2 2 0 0 1-2-2V7Z" />
  </svg>
...
function Accordion({ children, open, onClick, header, unread, date, status }: AccordionProps) {
  // Keep status colors as-is (intentional)
  const getStatusColor = (status: string) => {
    switch (status?.toLowerCase()) {
      case "completed":
        return "border-green-400 bg-gradient-to-r from-green-50 to-white";
      case "in-progress":
        return "border-yellow-400 bg-gradient-to-r from-yellow-50 to-white";
      case "scheduled":
        return "border-blue-400 bg-gradient-to-r from-blue-50 to-white";
      default:
  ...
  return (
    <div
      ...
      style={{
        ...
        fontSize: "1.09rem",
        overflow: "hidden",
        // keep these as-is for unread emphasis (intentional)
        borderLeft: unread ? "5px solid #3478b8" : "3px solid #c6d0da",
      }}
    >
      <button
        onClick={onClick}
        className={`w-full flex items-center justify-between px-6 py-5 text-left font-medium ${
          // swap only the neutral text colors
          unread ? "text-text" : "text-text/90"
        }`}
        type="button"
      >
        <span className="flex items-center gap-3">
        ...
        <span className="flex items-center gap-4">
          ...
          <span
            className={`inline-block ml-2 transform transition-transform ${open ? "rotate-180" : ""}`}
            style={{ fontSize: "1.3em", color: "#87a4c2" }} // kept as-is
          >
            ▼
          </span>
        </span>
...
const MultiNotification: React.FC = () => {
  ...
  useEffect(() => {
    ...
    list = [...list.filter((s) => !readIds.includes(s.id)), ...list.filter((s) => readIds.includes(s.id))];
    setFiltered(list);
  }, [filter, multiSkills, readIds]);
  ...
  const SkillDetail: React.FC<{ skill: DisplayMultiSkilling }> = ({ skill }) => (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <User />
        <span className="font-semibold text-text">{skill.employee_name}</span>
        <span className="text-xs text-muted bg-background px-2 py-1 rounded border border-border">
          ID: {skill.emp_id}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-text">{skill.department_name}</span>
          </div>

          <div className="flex items-center gap-2">
            <Target />
            <span className="text-sm text-muted">Station:</span>
            <span className="font-medium text-text">{skill.station_name}</span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-text">{formatDate(skill.start_date)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        <div
          className={`px-3 py-2 inline-block rounded-lg font-medium text-sm ${
            skill.current_status?.toLowerCase() === "completed"
              ? "bg-green-100 text-green-800 border border-green-200"
              : skill.current_status?.toLowerCase() === "in-progress"
              ? "bg-yellow-100 text-yellow-800 border border-yellow-200"
              : skill.current_status?.toLowerCase() === "scheduled"
      ...
      {skill.remarks && skill.remarks !== "No remarks provided" && (
        <div className="mt-4 p-3 bg-surface border border-border rounded-lg">
          <span className="text-sm text-muted block mb-1">Remarks:</span>
          <span className="text-text">{skill.remarks}</span>
        </div>
      )}
    </div>
  );
  ...
  if (loading) {
    return (
      <div className="w-full min-h-screen bg-background text-text flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500 mx-auto mb-4"></div>
          <p className="text-muted">Loading multiskilling notifications...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="w-full min-h-screen bg-background text-text flex items-center justify-center">
        <div className="text-center bg-surface p-8 rounded-xl shadow-lg border border-border">
          <h3 className="text-xl font-bold mb-2 text-text">Error Loading Data</h3>
    ...
    <div className="w-full min-h-screen bg-background text-text">
      ...
      <div className="max-w-full px-4 sm:px-6 lg:px-8 pt-9 pb-5 mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-3">
            <NotificationIcon />
            <h2 className="text-3xl font-bold text-text">MultiSkilling Notifications</h2>
            {unreadCount > 0 && (
              <span className="bg-red-500 text-white px-3 py-1 rounded-full text-sm font-medium">
                {unreadCount} new
              </span>
          ...
          <select
            value={filter}
            onChange={(e) => setFilter(e.target.value)}
            className="border-2 border-border rounded-lg px-4 py-2 bg-surface text-text focus:border-blue-500 focus:outline-none"
          >
            {FILTER_OPTIONS.map((opt) => (
        ...
        {recentSkill && (
          <div className="bg-green-50 border-2 border-green-200 rounded-xl p-4 mb-6 flex justify-between items-center">
            <div className="flex items-center gap-3 text-green-800 font-semibold">
              <NotificationIcon />
              <span>
                New multiskill assignment: <strong>{recentSkill.station_name}</strong> for {recentSkill.employee_name}
              </span>
            </div>
            <button
              onClick={() => markAsRead(recentSkill.id)}
        ...
        {filtered.length === 0 ? (
          <div className="text-center text-muted py-12 bg-surface rounded-xl shadow-lg border border-border">
            <NotificationIcon />
            <h3 className="text-xl font-semibold mt-4 mb-2 text-text">No notifications found</h3>
            <p>
              {filter === "all"
                ? "No multiskilling assignments have been created yet."
                : `No multiskilling assignments found for the selected time period.`}
