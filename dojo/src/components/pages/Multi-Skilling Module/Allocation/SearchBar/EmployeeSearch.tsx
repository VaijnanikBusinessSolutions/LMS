...
// // Interface for skills from SkillMatrix (comes with /employee-skill-search/)
// interface Skill {
//   skill_id: number;
//   station: string;
//   department: string;
...
// // Interface for the completed skills from /multiskilling/ (for tracking/assignments if needed)
...
//                             skill.current_status === 'completed' ? 'bg-green-100 text-green-800' :
  ...
  return (
    <div className="min-h-screen bg-background text-text p-6">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-text mb-2">Employee Skill Management</h1>
          <p className="text-muted text-lg">Search and manage employee skills</p>
        </div>

        {/* Search Bar (keep intentional red styling) */}
        <div className="relative mb-8">
          <div className="relative">
            <input
              type="text"
              placeholder="Search by employee name or ID..."
              className="w-full p-4 pl-12 bg-surface border-2 border-red-200 rounded-2xl shadow-lg focus:outline-none focus:ring-4 focus:ring-red-300 focus:border-red-500 transition-all duration-300 text-xl text-text placeholder:text-muted"
              value={searchTerm}
              onChange={handleSearch}
            />
            <svg
              className="absolute left-4 top-1/2 transform -translate-y-1/2 text-red-400 w-8 h-8"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          {isLoading && (
            <div className="absolute z-20 w-full mt-2 bg-surface border-2 border-border rounded-2xl shadow-2xl overflow-hidden p-4">
              <div className="flex justify-center">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
              </div>
            </div>
          ...
          {error && !isLoading && (
            <div className="absolute z-20 w-full mt-2 bg-surface border-2 border-border rounded-2xl shadow-2xl overflow-hidden p-4 text-red-500">
              {error}
            </div>
          )}

          {filteredEmployees.length > 0 && !isLoading && (
            <ul className="absolute z-20 w-full mt-2 bg-surface border-2 border-border rounded-2xl shadow-2xl overflow-hidden">
              {filteredEmployees.map((emp) => (
                <li
                  key={emp.emp_id}
                  className="p-4 hover:bg-background cursor-pointer transition-all duration-200 border-b border-border last:border-0"
                  onClick={() => handleEmployeeSelect(emp)}
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-muted bg-background px-3 py-1 rounded-full border border-border">
                      {emp.department || 'No department'}
                    </span>
                  </div>
                  <div className="text-sm text-muted mt-1">ID: {emp.emp_id}</div>
                </li>
              ))}
            </ul>
          )}
        ...
        {selectedEmployee && (
          <div className="space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Employee Information Card (neutral theming only) */}
              <div className="bg-surface p-8 rounded-3xl shadow-xl border border-border hover:shadow-2xl transition-all duration-300">
                <div className="flex items-center mb-6">
                  <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-3 rounded-full mr-4">
                    <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  ...
                  <h3 className="text-2xl font-bold text-text">Employee Information</h3>
                </div>

                <div className="space-y-4">
                  <div className="flex items-center p-3 bg-background rounded-xl border border-border">
                    <span className="font-semibold text-text/80 w-32">Name :</span>
                  ...
                  <div className="flex items-center p-3 bg-background rounded-xl border border-border">
                    <span className="font-semibold text-text/80 w-32">Employee ID :</span>
                    <span className="text-text font-medium">{selectedEmployee.emp_id}</span>
                  </div>

                  <div className="flex items-center p-3 bg-background rounded-xl border border-border">
                    <span className="font-semibold text-text/80 w-32">Department :</span>
                    <span className="text-text font-medium">{selectedEmployee.department || 'Not specified'}</span>
                  </div>

                  <div className="flex items-center p-3 bg-background rounded-xl border border-border">
                    <span className="font-semibold text-text/80 w-32">Join Date :</span>
                    <span className="text-text font-medium">{selectedEmployee.date_of_joining}</span>
                  </div>
                </div>
              </div>

              {/* Current Skills Card (neutral theming only; keep green styling inside) */}
              <div className="bg-surface p-8 rounded-3xl shadow-xl border border-border hover:shadow-2xl transition-all duration-300">
                <div className="flex items-center mb-6">
                  <div className="bg-gradient-to-r from-green-500 to-emerald-600 p-3 rounded-full mr-4">
                    <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-text">Current Skills</h3>
                    <p className="text-sm text-muted">Skills mastered by this employee</p>
                  </div>
                </div>
                ...
                <div className="space-y-4 max-h-96 overflow-y-auto">
                  {selectedEmployee.skills.length > 0 ? (
                    selectedEmployee.skills.map((s) => (
                      <div
                        key={s.skill_id}
                        className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-xl border-l-4 border-green-500 hover:shadow-md transition-all duration-200"
                      >
                        <div className="flex items-center justify-between mb-2">
                          <p className="font-bold text-gray-800 text-lg">{s.station}</p>
                          <span className="bg-green-100 text-green-800 text-xs font-semibold px-2 py-1 rounded-full">
                    ...
                    <div className="text-center py-8">
                      <div className="mx-auto w-16 h-16 bg-background border border-border rounded-full flex items-center justify-center mb-4">
                        <svg className="w-8 h-8 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                      </div>
                      <p className="text-muted font-medium">No current skills found</p>
                      <p className="text-muted/80 text-sm mt-1">This employee hasn't completed any skills yet.</p>
                    </div>
                  )}
                </div>
                ...
                {selectedEmployee.skills.length > 0 && (
                  <div className="mt-6 pt-4 border-t border-border">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-muted">Total Current Skills:</span>
            ...
            {/* Multiskilling Assignments Card (neutral theming only; keep orange styling inside) */}
            {multiskillingSkills.length > 0 && (
              <div className="bg-surface p-8 rounded-3xl shadow-xl border border-border">
                <div className="flex items-center mb-6">
                  <div className="bg-gradient-to-r from-orange-500 to-yellow-600 p-3 rounded-full mr-4">
                    <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-text">Assigned Skills</h3>
                    <p className="text-sm text-muted">Skills assigned for training/development</p>
                  </div>
                </div>
                ...
                <div className="space-y-4 max-h-96 overflow-y-auto">
                  {multiskillingSkills.map((skill) => (
                    <div
                      key={skill.id}
                      className="bg-gradient-to-r from-orange-50 to-yellow-50 p-4 rounded-xl border-l-4 border-orange-500 hover:shadow-md transition-all duration-200"
                    >
                      <div className="flex items-center justify-between mb-2">
                        <p className="font-bold text-gray-800 text-lg">{skill.station_name}</p>
                        <div className="flex items-center space-x-2">
                          ...
                          <span
                            className={`text-xs font-semibold px-2 py-1 rounded-full ${
                              skill.current_status === 'completed'
                                ? 'bg-green-100 text-green-800'
                                : skill.current_status === 'in-progress'
                      ...
                      <p className="text-sm text-gray-600">Start Date: {skill.start_date}</p>
                      {skill.remarks && <p className="text-sm text-gray-600 mt-1">Remarks: {skill.remarks}</p>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* AddSkill container (neutral only; keep purple icon block) */}
            <div className="bg-surface p-8 rounded-3xl shadow-xl border border-border">
              <div className="flex items-center mb-6">
                <div className="bg-gradient-to-r from-purple-500 to-red-600 p-3 rounded-full mr-4">
                  <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <h3 className="text-2xl font-bold text-text">Add New Skill</h3>
              </div>

              <AddSkill employeeID={selectedEmployee.emp_id} employee={selectedEmployee} />
            </div>
          </div>
        )}
      </div>
