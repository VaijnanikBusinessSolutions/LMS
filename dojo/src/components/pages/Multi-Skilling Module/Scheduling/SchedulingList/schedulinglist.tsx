...
// // line ~4
// const formatSkillLevel = (level: any): string => {
//   if (typeof level === "string") return level.replace(/_/g, " ").trim();
//   if (typeof level === "number" && Number.isFinite(level)) return `L${level}`;
//   if (level && typeof level === "object") {
//     const v = (level as any).name ?? (level as any).label ?? (level as any).value;
//     return v != null ? formatSkillLevel(v) : "—";
//   }
//   return "—";
// };
...
//                               {skill.status && (
//                                 <span
//                                   className={`text-xs px-3 py-1 rounded-full font-medium ${skill.status.toLowerCase() === 'completed'
...
const MultiSkillingList: React.FC<MultiSkillingListProps> = ({
    ...
    const filtered = employees
      .map(emp => {
        ...
        const filteredSkills = skills.filter(skill => {
          const statusOK =
            statusFilter === 'all' ||
            (skill.status && skill.status.toLowerCase() === statusFilter.toLowerCase());
          ...
          if (dateFilter !== 'all-time' && skill.start_date && skill.start_date !== 'N/A') {
            ...
            switch (dateFilter) {
              ...
              case 'this-month':
                dateOK =
                  skillStartDate.getFullYear() === today.getFullYear() &&
                  skillStartDate.getMonth() === today.getMonth();
                break;
              default:
                dateOK = true;
            }
  ...
  if (!employees || employees.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background text-text">
        <div className="text-center">
          <div className="mx-auto w-24 h-24 bg-surface border border-border rounded-full flex items-center justify-center mb-4">
            <Award className="w-12 h-12 text-muted" />
          </div>
          <h3 className="text-xl font-semibold text-text mb-2">No Multi-Skilling Data Found</h3>
          <p className="text-muted">No skill assignments have been created yet.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background text-text">
        <div className="text-center">
          ...
          <h3 className="text-xl font-semibold text-text mb-2">No Matching Skills</h3>
          <p className="text-muted">No skills match the selected filters.</p>
          <p className="text-sm text-muted/80 mt-2">Try adjusting your filters to see more results.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background p-6 text-text">
      <div className="max-w-7xl mx-auto">
        {/* Header (keep your blue icon block and gradient title intact) */}
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-3 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl shadow-lg">
              <Award className="w-6 h-6 text-white" />
            </div>
            <h1 className="text-3xl font-bold bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent dark:from-white dark:to-slate-300">
              Multi-Skilling Allocations
            </h1>
          </div>
          <p className="text-muted ml-14">
            Total employees: {filteredEmployees.length} | Total skills:{' '}
            {filteredEmployees.reduce((sum, emp) => sum + emp.skills.length, 0)}
          </p>
        </div>
        ...
        <div className="space-y-6">
          {filteredEmployees.map(employee => (
            <div
              ...
              "
            >
              {/* Employee Header (keep gradient; just make border theme-aware) */}
              <div className="bg-gradient-to-r from-slate-50 to-white dark:from-slate-900/40 dark:to-slate-900/10 border-b border-border p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="flex items-start gap-4">
                    <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg">
                      <User className="w-8 h-8 text-white" />
                    ...
                    <div>
                      <h3 className="text-xl font-semibold text-text mb-1">{employee.full_name}</h3>

                      <div className="flex items-center gap-4 mb-1">
                        <p className="text-sm text-muted">ID: {employee.card_no || employee.employee_id}</p>
                      </div>
                      ...
                      <div className="flex items-center gap-2 text-sm mb-2">
                        ...
                        {employee.section && employee.section !== employee.department && (
                          <>
                            ...
                            <span className="text-muted">{employee.section}</span>
                          </>
                        )}
                        {employee.department_code && (
                          <>
                            <span className="text-muted/60">•</span>
                            <span className="text-muted">Code: {employee.department_code}</span>
                          </>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-4 text-sm mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-muted">Joined: {employee.joining_date}</span>
                        </div>
                        {employee.employment_pattern && (
                          <>
                            <span className="text-muted/40">|</span>
                            <span className="text-muted">Pattern: {employee.employment_pattern}</span>
                          </>
                        )}
                      </div>
                    </div>
              ...
              <div className="p-6 space-y-4">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h4 className="font-semibold text-text">Multi-Skills</h4>
                  <span className="text-sm text-muted">({employee.skills.length} skills)</span>
                ...
                {employee.skills.length ? (
                  <div className="grid grid-cols-1 gap-4">
                    {employee.skills.map((skill: any, index: number) => {
                      // Keep your intentional status colors as-is
                      let cardBgColor = 'bg-gray-50';
                      let cardBorderColor = 'border-gray-200';
        ...
        {/* Footer */}
        <div className="mt-12 text-center">
          <p className="text-sm text-muted">Last updated: {formatDate(new Date().toISOString())}</p>
        </div>
      </div>
    </div>
  );
