...
const Overview = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        console.error("❌ Error fetching stats:", err);
        setError(err.message || 'Failed to fetch statistics');
        setStats({ scheduled: 0, in_progress: 0, completed: 0, total: 0 });
        setAllEmployees([]);
      } finally {
        setIsLoading(false);
      }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-background p-6 flex items-center justify-center">
        <div className="bg-surface p-8 rounded-xl shadow-lg max-w-md text-center border border-border">
          <h3 className="text-xl font-bold text-text mb-2">Error Loading Statistics</h3>
          <p className="text-muted mb-4">{error}</p>
          <button
            onClick={() => window.location.reload()}
            className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors focus:outline-none focus:ring-2 focus:ring-accent/50"
          >
            Try Again
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-text mb-2">Skill Scheduling Overview</h1>
          <p className="text-muted text-lg">Monitor and track skill development across your organization</p>
        </div>

        <div className="bg-surface p-8 rounded-3xl shadow-xl border border-border mb-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            <label htmlFor="status-filter" className="block text-sm font-semibold mb-2 text-text">
              Filter by Status
            </label>
            <select
              id="status-filter"
          ...
          <div>
            <label htmlFor="date-filter" className="block text-sm font-semibold mb-2 text-text">
        ...
        {/* Stats Cards (only neutral bg/border changed; your status colors preserved) */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          <div className="bg-surface p-8 rounded-3xl shadow-xl border border-border">
            ...
            <h3 className="text-xl font-bold text-text mb-2">Scheduled</h3>
            <p className="text-4xl font-bold text-blue-600 mb-3">{stats.scheduled || 0}</p>
            <p className="text-sm text-muted">Skills waiting to be started</p>
            <div className="mt-4 h-2 bg-border/60 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-blue-500 to-blue-600 rounded-full transition-all duration-500" style={{ width: stats.total > 0 ? `${(stats.scheduled / stats.total) * 100}%` : '0%' }}></div>
            </div>
            <p className="text-xs text-muted mt-2">{stats.total > 0 ? `${Math.round((stats.scheduled / stats.total) * 100)}%` : '0%'} of total</p>
          </div>
          ...
          <div className="bg-surface p-8 rounded-3xl shadow-xl border border-border">
            ...
            <h3 className="text-xl font-bold text-text mb-2">In Progress</h3>
            <p className="text-4xl font-bold text-yellow-600 mb-3">{stats.in_progress || 0}</p>
            <p className="text-sm text-muted">Skills currently in progress</p>
            <div className="mt-4 h-2 bg-border/60 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-yellow-500 to-orange-500 rounded-full transition-all duration-500" style={{ width: stats.total > 0 ? `${(stats.in_progress / stats.total) * 100}%` : '0%' }}></div>
            </div>
            <p className="text-xs text-muted mt-2">{stats.total > 0 ? `${Math.round((stats.in_progress / stats.total) * 100)}%` : '0%'} of total</p>
          </div>
          ...
          <div className="bg-surface p-8 rounded-3xl shadow-xl border border-border">
            ...
            <h3 className="text-xl font-bold text-text mb-2">Completed</h3>
            <p className="text-4xl font-bold text-green-600 mb-3">{stats.completed || 0}</p>
            <p className="text-sm text-muted">Successfully finished skills</p>
            <div className="mt-4 h-2 bg-border/60 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-green-500 to-emerald-600 rounded-full transition-all duration-500" style={{ width: stats.total > 0 ? `${(stats.completed / stats.total) * 100}%` : '0%' }}></div>
            </div>
            <p className="text-xs text-muted mt-2">{stats.total > 0 ? `${Math.round((stats.completed / stats.total) * 100)}%` : '0%'} of total</p>
          </div>
          ...
          <div className="bg-surface p-8 rounded-3xl shadow-xl border border-border">
            ...
            <h3 className="text-xl font-bold text-text mb-2">Total Skills</h3>
            <p className="text-4xl font-bold text-purple-600 mb-3">{stats.total || 0}</p>
            <p className="text-sm text-muted">All skills in the system</p>
            <div className="mt-4 h-2 bg-border/60 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-purple-500 to-indigo-600 rounded-full transition-all duration-500" style={{ width: '100%' }}></div>
            </div>
            <p className="text-xs text-muted mt-2">{stats.scheduled + stats.in_progress + stats.completed} allocated</p>
          </div>
        </div>

        <MultiSkillingList employees={allEmployees} statusFilter={statusFilter} dateFilter={dateFilter} />
      </div>
    </div>
  );
};
