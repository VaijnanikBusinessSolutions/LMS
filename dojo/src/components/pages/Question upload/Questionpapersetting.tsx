...
// src/components/pages/QuestionPaperManager.tsx

import React, { useEffect, useState } from 'react';
import { Toaster, toast } from 'react-hot-toast';
import {
  Loader2, File, X, ClipboardList, Type,
  Eye, Edit, Trash2, Plus, Search, RefreshCw,
  Building2, Factory, Gauge, MapPin, Trophy,
  Calendar, Clock, FileText, Sparkles, FileQuestion
} from 'lucide-react';
...
const Spinner = ({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) => {
  const sizeClasses = { sm: 'h-4 w-4', md: 'h-6 w-6', lg: 'h-8 w-8' };
  return <Loader2 className={`animate-spin text-indigo-600 dark:text-indigo-400 ${sizeClasses[size]}`} />;
};
...
const TextInputWithIcon = ({
  ...
  <div className="group">
    <label htmlFor={id} className="block text-sm font-semibold text-text mb-2 capitalize">
      {name.replace(/_/g, ' ')}
    </label>
    <div className="relative">
      <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
        <Icon className="h-5 w-5 text-muted group-focus-within:text-indigo-600 dark:group-focus-within:text-indigo-400 transition-colors" />
      </div>
      <input
        type="text"
        id={id}
        ...
        onChange={onChange}
        disabled={disabled}
        className={`w-full p-3 pl-10 border rounded-lg shadow-sm transition-all duration-200 bg-background text-text
          ${disabled 
            ? 'bg-muted/10 border-border cursor-not-allowed opacity-60' 
            : 'border-border focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900/20 hover:border-gray-400 dark:hover:border-gray-500'
          }
        `}
        placeholder={placeholder}
      />
  ...
  <div className="group">
    ...
    <div className="relative">
      ...
      <select
        ...
        onChange={onChange}
        disabled={loading || disabled}
        className={`w-full p-3 ${Icon ? 'pl-10' : ''} border rounded-lg shadow-sm transition-all duration-200 appearance-none cursor-pointer text-text
          ${loading || disabled 
            ? 'bg-muted/10 border-border cursor-not-allowed opacity-60' 
            : 'bg-background border-border hover:border-gray-400 dark:hover:border-gray-500 focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900/20'
          }`}
      >
        <option value="" disabled className="bg-surface text-text">{`Select a ${label}`}</option>
        {options.map((option) => (
          <option key={`opt-${name}-${option.id}`} value={String(option.id)} className="bg-surface text-text">
            {option.name}
          </option>
        ))}
      </select>
...
const QuestionPaperCard = ({ 
  ...
  <div className="bg-surface rounded-xl shadow-sm border border-border hover:shadow-xl hover:border-indigo-200 dark:hover:border-indigo-700/50 transition-all duration-300 overflow-hidden group">
    <div className="p-6">
      <div className="flex items-start justify-between mb-4">
        <h3 className="text-lg font-bold text-text truncate flex-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
          {paper.question_paper_name}
        </h3>
        <div className="flex gap-1 ml-4 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={() => onEdit(paper)}
            className="p-2 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-all duration-200 hover:scale-110"
            title="Edit"
          >
            <Edit className="h-4 w-4" />
          </button>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3 text-sm text-muted">
          ...
          <span className="font-medium text-text">{paper.department?.name || 'N/A'}</span>
        </div>
        ...
        <div className="flex items-center gap-3 text-sm text-muted">
          ...
          <span className="font-medium text-text">{paper.station?.name || 'N/A'}</span>
        </div>
        ...
        <div className="flex items-center gap-3 text-sm text-muted">
          ...
          <span className="font-medium text-text">{paper.level?.name || 'N/A'}</span>
        </div>
...
const QuestionPaperManager: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* FULL WIDTH WRAPPER */}
      <div className="bg-background text-text min-h-screen w-full transition-colors duration-300">
        {/* FULL WIDTH CONTAINER WITH PADDING */}
        <div className="w-full px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-4xl py-3 font-bold text-text">
                  Question Paper Manager
                </h1>
            ...
            {/* TABS */}
            <div className="flex bg-surface rounded-xl p-1.5 shadow-lg border border-border gap-1 overflow-x-auto">
              <TabButton id="create" label="Create Paper" icon={Plus} />
              <TabButton id="list" label="View Papers" icon={Eye} />
              <TabButton id="upload" label="Create Questions" icon={FileQuestion} />
            </div>
          ...
          {activeTab === 'list' && (
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="bg-surface rounded-2xl shadow-lg border border-border p-6">
                <div className="flex flex-col sm:flex-row gap-4 items-center">
                  <div className="w-full sm:flex-1 relative group">
                    <Search className="h-5 w-5 absolute left-4 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-indigo-600 dark:group-focus-within:text-indigo-400 transition-colors" />
                    <input
                      type="text"
                      placeholder="Search papers, departments..."
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
                      className="w-full pl-12 pr-4 py-3 border border-border rounded-xl focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900/20 focus:border-indigo-500 dark:focus:border-indigo-400 bg-background text-text transition-all duration-200"
                    />
                  </div>
                  <div className="w-full sm:w-64">
                    <select
                      value={filterDepartment}
                      onChange={(e) => setFilterDepartment(e.target.value)}
                      className="w-full p-3 border border-border rounded-xl focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900/20 focus:border-indigo-500 dark:focus:border-indigo-400 bg-background text-text transition-all duration-200 appearance-none cursor-pointer"
                    >
                      <option value="" className="bg-surface text-text">All Departments (Filter)</option>
                      <option value={ALL_OPTION_ID} className="bg-surface text-text">Global Papers Only</option> 
                      {departments.map((dept) => (
                        <option key={`filter-dept-${dept.department_id}`} value={String(dept.department_id)} className="bg-surface text-text">
                          {dept.department_name}
                        </option>
                      ))}
                    </select>
                  </div>
                  <button onClick={refetch} className="p-3 text-muted border border-border rounded-xl hover:bg-muted/10 transition-all duration-200 group" title="Refresh">
                    <RefreshCw className="h-5 w-5 group-hover:rotate-180 transition-transform duration-500" />
                  </button>
                </div>
              </div>
              ...
              {/* Responsive Grid */}
              {isListLoading ? (
                <div className="flex flex-col items-center justify-center py-20 bg-surface rounded-2xl shadow-lg border border-border">
                  <Spinner size="lg" />
                  <p className="mt-4 text-muted font-medium">Loading question papers...</p>
                </div>
              ) : filteredPapers.length === 0 ? (
                <div className="text-center py-20 bg-surface rounded-2xl shadow-lg border border-border">
                  ...
                  <h3 className="text-xl font-bold text-text mb-2">No Question Papers Found</h3>
                  <p className="text-muted mb-8 max-w-md mx-auto">{searchTerm || filterDepartment ? 'Try adjusting your search or filters' : 'Create your first question paper to get started'}</p>
                  <button onClick={() => { setActiveTab('create'); resetForm(); }} className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl hover:shadow-lg transform hover:scale-105 transition-all duration-200 font-semibold">
                    <Plus className="h-5 w-5 mr-2" />
                    Create Your First Paper
                  </button>
            ...
            <div className="max-w-5xl mx-auto animate-fade-in">
              <div className="bg-surface rounded-2xl shadow-xl border border-border overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="p-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="md:col-span-2">
                      <TextInputWithIcon 
                        ...
                        disabled={!editingPaper}
                      />
                      {!editingPaper && <p className="mt-1 text-sm text-muted">Auto-generated name based on selection.</p>}
                    </div>

                    {/* === TIMER SETTINGS CONTROLS === */}
                    <div className="md:col-span-2 bg-muted/5 p-4 rounded-xl border border-border">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                                <div className={`p-2 rounded-lg ${formData.is_timer_enabled ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400' : 'bg-gray-200 dark:bg-gray-700 text-gray-500'}`}>
                                    <Clock className="h-5 w-5" />
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-text">Exam Timer</label>
                                    <p className="text-xs text-muted">Enable countdown for each question</p>
                                </div>
                                <button
                                    type="button"
                                    onClick={() => setFormData(prev => ({ ...prev, is_timer_enabled: !prev.is_timer_enabled }))}
                                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${formData.is_timer_enabled ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-gray-700'}`}
                                >
                                    <span className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${formData.is_timer_enabled ? 'translate-x-5' : 'translate-x-0'}`} />
                                </button>
                            </div>
                            ...
                            {formData.is_timer_enabled && (
                                <div className="flex items-center gap-3 animate-fade-in">
                                    <label htmlFor="question_duration" className="text-sm font-semibold text-text whitespace-nowrap">
                                        Seconds per Question:
                                    </label>
                                    <div className="relative w-24">
                                        <input
                                            ...
                                            min="10"
                                            max="300"
                                            className="w-full p-2 pl-3 pr-8 border border-border bg-background text-text rounded-lg focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900/20 transition-all text-center font-medium"
                                        />
                                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted font-medium">s</span>
                                    </div>
                                </div>
                            )}
                        </div>
                  ...
                  <div className="flex items-center justify-between gap-4 mt-10 pt-8 border-t border-border">
                    <div className="text-sm text-muted">
                      <p className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-yellow-500" />Select level and location as needed</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <button type="button" onClick={() => setActiveTab('list')} className="py-3 px-6 text-sm font-semibold text-text bg-surface rounded-xl border border-border hover:bg-muted/10 transition-all duration-200">Cancel</button>
                      <button type="submit" disabled={isSubmitting || optionsLoading} className="inline-flex justify-center items-center py-3 px-8 border border-transparent rounded-xl shadow-sm text-sm font-semibold text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed transition-all duration-200 hover:shadow-lg transform hover:scale-105 disabled:transform-none">
                        {isSubmitting && <Spinner size="sm" />}
                        <span className={isSubmitting ? 'ml-2' : ''}>{isSubmitting ? (editingPaper ? 'Updating...' : 'Creating...') : (editingPaper ? 'Update Paper' : 'Create Paper')}</span>
                      </button>
