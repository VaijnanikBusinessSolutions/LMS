...
// --- THEME CONSTANTS ---
// Using CSS variables to ensure reactivity to Dark/Light mode switches
const THEME = {
  planned: "rgb(var(--brand-primary))",    // Main Brand Color
  actual: "rgb(var(--header-primary))",    // Secondary/Header Color for contrast
...
const AttritionTrendChart: React.FC<Props> = ({
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        ...
        <div className="bg-surface p-4 border border-border rounded-lg shadow-soft text-sm">
  ...
  const CustomLegend = () => (
    ...
    <div className="flex justify-center flex-wrap gap-x-6 gap-y-2 text-sm text-muted mt-4">
      <span className="inline-flex items-center gap-2">
        <div className="w-4 h-4 rounded-sm" style={{ backgroundColor: THEME.planned }} />
        <span>Planned Operators</span>
      </span>
      <span className="inline-flex items-center gap-2">
        <div className="w-4 h-4 rounded-sm" style={{ backgroundColor: THEME.actual }} />
        <span>Actual Operators</span>
      </span>
    </div>
  );
      ...
      return (
        <div className="h-full flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted">Loading operators required data...</p>
          </div>
        </div>
      );
    }
    ...
    if (data.length > 0 && data.some(d => d.planned > 0 || d.actual > 0)) {
      return (
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            <defs>
              {/* Updated: Using Theme Colors for Gradients */}
              <linearGradient id="plannedGradient" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={THEME.planned} stopOpacity={0.8} />
                <stop offset="100%" stopColor={THEME.planned} stopOpacity={0.1} />
              </linearGradient>
              <linearGradient id="actualGradient" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={THEME.actual} stopOpacity={0.8} />
                <stop offset="100%" stopColor={THEME.actual} stopOpacity={0.1} />
              </linearGradient>
            </defs>
            {/* Updated: Grid stroke */}
            ...
            <Area
              type="monotone"
              dataKey="planned"
              stroke={THEME.planned}
              strokeWidth={3}
              fill="url(#plannedGradient)"
              dot={{ stroke: THEME.planned, strokeWidth: 2, fill: '#fff', r: 5 }}
              activeDot={{ r: 7, stroke: THEME.planned, strokeWidth: 2, fill: '#fff' }}
            >
              <LabelList
                dataKey="planned"
                position="top"
            ...
            <Area
              ...
              strokeWidth={3}
              fill="url(#actualGradient)"
              dot={{ stroke: THEME.actual, strokeWidth: 2, fill: '#fff', r: 5 }}
              activeDot={{ r: 7, stroke: THEME.actual, strokeWidth: 2, fill: '#fff' }}
            >
              <LabelList
                dataKey="actual"
                position="bottom"
  ...
  return (
    // Updated: bg-surface, border-border, text-text
    <div className="relative w-full h-[550px] bg-surface border border-border rounded-xl p-6 flex flex-col transition-colors duration-300">
      <div className="flex items-center justify-center mb-4 sm:mb-6">
