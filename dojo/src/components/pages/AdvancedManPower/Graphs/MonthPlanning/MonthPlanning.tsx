...
// --- THEME CONSTANTS ---
const THEME = {
  bar: "rgb(var(--brand-primary))",    // Main Brand Color for bar
  ...
  tooltipLabel: "rgb(var(--text-main))",
...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      ...
      <div className="bg-surface p-3 border border-border rounded-lg shadow-soft text-sm">
        <p className="font-semibold text-text mb-1">{data.label}</p>
        <p style={{ color: payload[0].fill }}>
          Productivity: {payload[0].value.toFixed(2)}
        </p>
...
const MonthPlanning: React.FC<Props> = ({ 
  ...
  if (loading) {
    return (
      // Updated: bg-surface, text-muted
      <div className="w-full h-[400px] bg-surface border border-border rounded-lg shadow-soft p-4 flex items-center justify-center text-muted">
  ...
  if (error) {
    return (
      <div className="w-full h-[400px] bg-surface border border-border rounded-lg shadow-soft p-4 flex flex-col items-center justify-center text-red-600 dark:text-red-400 text-center">
        <div className="text-lg font-medium mb-1">Error Loading Data</div>
        <div className="text-sm">{error}</div>
      </div>
    );
  ...
  if (!apiData || chartData.length === 0) {
    return (
       // Updated: bg-surface, text-muted
       <div className="w-full h-[400px] bg-surface border border-border rounded-lg shadow-soft p-4 flex flex-col items-center justify-center text-muted text-center">
        <svg className="w-12 h-12 text-muted/30 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V7a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
        <p className="font-semibold text-text">No Data to Display</p>
        <p className="text-sm">No productivity data found for the selected criteria.</p>
      </div>
    );
  }
  ...
  return (
    ...
    <div className="w-full bg-surface border border-border rounded-lg shadow-soft p-4 transition-colors duration-300">
        <div className="flex items-center justify-center mb-4">
            {/* Updated: bg-primary/10 */}
            <div className="bg-primary/10 rounded-lg px-6 py-3">
              <h2 className="text-lg font-bold text-text flex items-center gap-2">
                {/* Updated: text-primary */}
                <svg className="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                Productivity
              </h2>
            </div>
        </div>
        ...
        <div className="w-full h-[450px]">
            ...
            <BarChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                />
                <YAxis 
                  label={{ value: 'Units per Operator', angle: -90, position: 'insideLeft', fill: THEME.axis, dy: 40 }} 
                  tick={{ fontSize: 12, fill: THEME.axis }} 
                  axisLine={false} 
                />
                <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(var(--text-muted), 0.1)' }} />
                <Legend verticalAlign="top" height={36} wrapperStyle={{ color: THEME.muted }}/>
                {/* Updated: Bar fill uses theme variable */}
                <Bar dataKey="productivity" name="Productivity" fill={THEME.bar} radius={[4, 4, 0, 0]} />
            </BarChart>
            </ResponsiveContainer>
        </div>
    </div>
