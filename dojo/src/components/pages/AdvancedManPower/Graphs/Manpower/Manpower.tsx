...
// Keep specific data colors as they indicate status (Past/Current/Future)
const COLORS = {
  planned: { past: '#A855F7', current: '#F97316', future: '#84CC16' },
  actual: { past: '#EC4899', current: '#FACC15', future: '#16A34A' },
};

// Theme Helper for Recharts SVG elements
const THEME = {
...
const ManpowerTrendChart: React.FC<Props> = ({
        ...
        <div className="bg-surface p-4 border border-border rounded-lg shadow-soft text-sm">
          ...
          <div className="space-y-1">
            {payload.map((entry: any, index: number) => (
              <p key={index} className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-sm" style={{ backgroundColor: entry.color }} />
                <span className="text-muted">{entry.dataKey === 'planned' ? 'Planned:' : 'Actual:'}</span>
                <span className="font-medium text-text">{entry.value.toLocaleString()} Units</span>
              </p>
            ))}
          </div>
        </div>
  ...
  const CustomLegend = () => {
    return (
      // Updated: text-muted
      <div className="flex justify-center items-center flex-wrap gap-x-4 gap-y-1 text-sm text-muted mt-2">
        <span className="inline-flex items-center gap-1.5"><div className="w-2.5 h-2.5" style={{ backgroundColor: COLORS.planned.past }} /><span>Past Planned</span></span>
        <span className="inline-flex items-center gap-1.5"><div className="w-2.5 h-2.5" style={{ backgroundColor: COLORS.actual.past }} /><span>Past Actual</span></span>
        <span className="inline-flex items-center gap-1.5"><div className="w-2.5 h-2.5" style={{ backgroundColor: COLORS.planned.current }} /><span>Current Planned</span></span>
        <span className="inline-flex items-center gap-1.5"><div className="w-2.5 h-2.5" style={{ backgroundColor: COLORS.actual.current }} /><span>Current Actual</span></span>
  ...
  const renderContent = () => {
    ...
    if (error) {
      return (
        <div className="h-full flex items-center justify-center">
          {/* Updated: bg-red-500/10 text-red-600 dark:text-red-400 */}
    ...
    if (chartData.length > 0) {
      return (
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 40 }}>
            {/* Updated: Stroke uses theme variable */}
            <CartesianGrid strokeDasharray="3 3" stroke={THEME.grid} vertical={false} />
            <XAxis 
              dataKey="name" 
              tick={{ fill: THEME.axis, fontSize: 12, fontWeight: 'bold' }} 
  ...
  return (
    // Updated: bg-surface, border-border, text-text
    <div className="relative w-full h-[600px] bg-surface border border-border rounded-xl shadow-soft p-6 flex flex-col manpower-trend-chart transition-colors duration-300">
      <div className="flex items-center justify-center mb-4 sm:mb-6">
        {/* Updated: bg-primary/10 (light purple/blue tint) */}
        <div className="bg-primary/10 rounded-lg px-4 sm:px-6 py-2 sm:py-3">
          <h2 className="text-base sm:text-lg font-bold text-text flex items-center gap-2">
            ...
            <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" /></svg>
            Production Plan vs Actual
          </h2>
        </div>
      </div>
