...
//     <>
//       {/* <Nav /> */}
//       <div className="bg-gradient-to-br from-purple-600 via-purple-700 to-indigo-800 shadow-xl"><h1 className="text-3xl md:text-4xl font-bold text-white text-center py-8">Production Planning Dashboard</h1></div>
...
const Advanced = () => {
    ...
    return (
      <div className="relative group">
        <label className="flex items-center gap-2 text-xs font-semibold text-muted mb-2 uppercase tracking-wider">
            <span className="text-lg">{icons[type]}</span>{label}
        </label>
        <div className="relative">
          <select 
            className="w-full appearance-none rounded-xl border border-border bg-background py-3 pl-4 pr-10 text-sm font-medium text-text shadow-sm transition-all hover:border-primary/50 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 disabled:bg-surface disabled:text-muted disabled:cursor-not-allowed" 
          ...
          <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
            <svg className="h-5 w-5 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
          </div>
        </div>
      </div>
    );
  ...
  return (
    // Replaced main background with bg-background and text-text
    <div className="min-h-screen bg-background text-text transition-colors duration-300">
      {/* <Nav /> */}
      {/* Header: Replaced hardcoded gradient with Theme Header Tokens */}
      <div className="bg-header-surface border-b border-header-border shadow-md">
        <h1 className="text-3xl md:text-4xl font-bold text-header-text text-center py-8">
            Production Planning Dashboard
        </h1>
      </div>

      {/* Filter Section: bg-surface, border-border */}
      <div className="bg-surface shadow-soft border-b border-border p-4 sm:p-6 transition-colors duration-300">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-9 gap-3 sm:gap-4">
          <div className="xl:col-span-1">{renderSelect("hq", "HQ")}</div>
          <div className="xl:col-span-1">{renderSelect("factory", "Factory")}</div>
          <div className="xl:col-span-1">{renderSelect("department", "Department")}</div>
          ...
          <div className="xl:col-span-1 flex items-end">
            <div className="flex flex-col w-full mt-7">
              ...
              <div className="p-1 bg-background border border-border rounded-xl flex w-full">
                <button 
                    onClick={() => setTimeView('Monthly')} 
                    disabled={isCheckingLock} 
                    className={`w-1/2 py-2.5 rounded-lg text-sm font-semibold transition-colors ${timeView === 'Monthly' ? 'bg-primary text-white shadow-sm' : 'text-muted hover:text-text'} disabled:opacity-50 disabled:cursor-not-allowed`}
                >
                    Monthly
                </button>
                <button 
                    onClick={() => setTimeView('Weekly')} 
                    disabled={isCheckingLock || monthLockMode === 'monthly'} 
                    className={`w-1/2 py-2.5 rounded-lg text-sm font-semibold transition-colors ${timeView === 'Weekly' ? 'bg-primary text-white shadow-sm' : 'text-muted hover:text-text'} disabled:opacity-50 disabled:cursor-not-allowed`}
                >
                    Weekly
                </button>
              </div>
              {monthLockMode === 'monthly' && (<p className="text-xs text-primary mt-2 text-center">Weekly view is disabled for this month.</p>)}
            </div>
          </div>
          <div className="xl:col-span-1">{renderSelect("year", "Year")}</div>
          <div className="xl:col-span-1">{renderSelect("month", "Month")}</div>
      ...
      {loading.factories ? (
        ...
        <div className="p-6">
            <div className="mb-6">
              {/* Top Card: bg-surface, border-border */}
              <div className="bg-surface rounded-2xl shadow-soft border border-border p-6 hover:shadow-medium transition-all">
                <CardProps subtopics={bifurcationSubtopics} data={planData as Record<string, number>} loading={loading.planData} cardColors={bifurcationCardColors} />
              </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Charts Containers: bg-surface, border-border */}
              <div className="bg-surface rounded-2xl shadow-soft border border-border overflow-hidden hover:shadow-medium transition-all min-h-[400px]">
                <ManpowerTrendChart selectedHqId={selectedHq} selectedFactoryId={selectedFactory} selectedDepartmentId={selectedDepartment} selectedLineId={selectedLine} selectedSublineId={selectedSubline} selectedStationId={selectedStation} startDate={dateRange.startDate} endDate={dateRange.endDate} timeView={timeView} selectedMonth={selectedMonth} selectedYear={selectedYear} />
              </div>
