...
export default function ArVrComponent() {
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (uploadType === 'file' && newMaterial.file) {
        formData.append('arvr_file', newMaterial.file);
      } else if (uploadType === 'link' && newMaterial.url) {
        formData.append('url_link', newMaterial.url);
      }
  ...
  const handleDeleteContent = async (contentId: string) => {
    ...
    try {
      const response = await axios.delete(`http://127.0.0.1:8000/arvr-content/${contentId}/`);
  ...
  const renderGridView = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {trainingContents.map((content) => (
        <div
          key={content.id}
          className="group flex flex-col p-4 border border-border rounded-lg bg-surface hover:bg-background cursor-pointer transition-all duration-200 hover:shadow-md dark:hover:shadow-lg dark:hover:shadow-primary/5"
          onClick={() => handleMaterialClick(content)}
        >
          <div className="flex items-center mb-3">
            <div className="mr-3 flex-shrink-0">
            ...
            <div className="flex-1 min-w-0">
              <span className="text-text hover:text-primary block font-medium truncate transition-colors">
                {content.description}
              </span>
            </div>
            <button
              ...
              className="text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 p-1 rounded-full hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors"
              title="Delete this material"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
  ...
  const renderListView = () => (
    <div className="space-y-2">
      {trainingContents.map((content) => (
        <div
          key={content.id}
          className="group flex items-center p-3 border border-border rounded-lg bg-surface hover:bg-background cursor-pointer transition-all duration-200 hover:shadow-md dark:hover:shadow-lg dark:hover:shadow-primary/5"
        >
          <div className="mr-3 flex-shrink-0">
            {getFileIcon(content)}
          </div>
          ...
          >
            ...
            <div className="flex items-center space-x-2 mt-1">
              ...
              {content.arvr_file && (
                <span className="text-xs text-muted/70" title={content.arvr_file}>
                  • {content.arvr_file.split('/').pop()}
                </span>
              )}
              {content.url_link && (
                <span className="text-xs text-muted/70" title={content.url_link}>
                  • {content.url_link.length > 30 ? content.url_link.substring(0, 30) + '...' : content.url_link}
                </span>
              )}
            </div>
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="flex items-center space-x-3 text-muted">
          ...
          <span>Loading AR/VR contents...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-text mb-2">AR/VR Training Materials</h1>
        <p className="text-muted">Upload and manage AR/VR training resources including 3D models, applications, and documentation</p>
      </div>

      {/* Content Section */}
      <div className="bg-surface p-6 rounded-xl shadow-soft border border-border/50 transition-colors duration-300">
        {/* Training Materials Section */}
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-medium text-text">AR/VR Materials</h3>
          <div className="flex space-x-2">
            <button
              onClick={() => setShowUploadModal(true)}
              className="px-4 py-2 bg-primary text-white rounded-lg hover:opacity-90 flex items-center space-x-2 transition-all duration-200 shadow-sm"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
            ...
            <div className="flex border border-border rounded-lg overflow-hidden">
              <button
                onClick={() => setViewType('grid')}
                className={`p-2 transition-colors duration-200 ${viewType === 'grid' ? 'bg-primary text-white' : 'bg-surface text-muted hover:text-text hover:bg-background'}`}
                title="Grid view"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
              ...
              <button
                onClick={() => setViewType('list')}
                className={`p-2 transition-colors duration-200 ${viewType === 'list' ? 'bg-primary text-white' : 'bg-surface text-muted hover:text-text hover:bg-background'}`}
                title="List view"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
        ...
        {trainingContents.length > 0 ? (
          ...
          <div className="text-center py-12 text-muted border-2 border-dashed border-border rounded-lg bg-background/50">
            <svg className="w-16 h-16 mx-auto mb-4 text-muted/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
            <p className="text-lg mb-2 text-text">No AR/VR materials uploaded yet</p>
            <p>Click "Upload AR/VR Material" to add 3D models, applications, and other resources</p>
          </div>
        )}
      </div>
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-surface rounded-xl shadow-hard border border-border w-full max-w-md animate-in fade-in slide-in-from-top-2">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-text">Upload AR/VR Material</h2>
                <button
                  onClick={() => setShowUploadModal(false)}
                  className="text-muted hover:text-text p-1 rounded-full hover:bg-background transition-colors"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <form onSubmit={handleUploadSubmit}>
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      value={newMaterial.description}
                      onChange={(e) => setNewMaterial({ ...newMaterial, description: e.target.value })}
                      className="w-full p-3 bg-background border border-border text-text rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/30 transition-all duration-200 placeholder:text-muted"
                      placeholder="e.g., 3D Engine Model, AR Assembly Guide"
                      required
                    />
                  </div>
                      ...
                      <button
                        ...
                        className={`flex-1 px-4 py-2.5 rounded-lg font-medium transition-all duration-200 ${
                          uploadType === 'file' 
                            ...
                            : 'bg-background text-muted border border-border hover:text-text hover:border-primary/50'
                        }`}
                      >
                  ...
                  {uploadType === 'file' ? (
                    <div>
                      ...
                      <div 
                        onClick={triggerFileInput}
                        className="flex items-center border border-border rounded-lg overflow-hidden cursor-pointer hover:border-primary/50 transition-colors group"
                      >
                        <div className="px-4 py-2.5 bg-background text-muted group-hover:text-text transition-colors border-r border-border">
                          Choose File
                        </div>
                        <div className="flex-1 p-2.5 bg-surface text-sm text-muted truncate">
                          {newMaterial.file ? newMaterial.file.name : 'No file selected'}
                        </div>
                      </div>
                ...
                <div className="mt-6 flex justify-end space-x-3">
                  <button
                    type="button"
                    onClick={() => setShowUploadModal(false)}
                    className="px-4 py-2.5 bg-background text-text border border-border rounded-lg hover:bg-surface transition-colors duration-200"
                    disabled={uploadLoading}
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-4 py-2.5 bg-primary text-white rounded-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center space-x-2"
                    disabled={uploadLoading}
                  >
                    {uploadLoading ? (
                      <>
