...
// import BioUserDashboard from '../BiometricSystem/biometric';
// import GlobalMatrixControls from '../../organisms/GlobalMatrixControls/GlobalMatrixControls';
// import MatrixConfiguration from '../../organisms/MatrixConfiguration/MatrixConfiguration';
// // import RecurrenceSettings from './RefreshementTrainingSettinginterval'
// import CriteriaManagement from './SkillEvaluationCriteria';
// import MinimumLevelConfig from './MinimumLevelConfig'
// const tabs = [
...
//   { key: 'Level-Settings', label: 'Level Settings', icon: '🏷️' },
//   // { key: 'Biometic-System', label: 'Biometric System', icon: '🖐️' },
//   { key: 'Global-Matrix', label: 'Global Matrix Configuration', icon: '🎓' },
//   { key: 'Matrix-Configuration', label: 'Matrix Configuration', icon: '🎓' },
//   // { key: 'Refreshment-interval-setting', label: 'Refreshment interval setting', icon: '🎓' },
//   { key: 'Skill-Evaluation-Criteria', label: 'Skill Evaluation Criteria', icon: '📝' },
//   { key: 'MinimumLevelConfig', label: 'MinimumLevelConfig', icon: '📝' },
// ];

// const MethodPage: React.FC = () => {
//   // Use search params to manage state via URL
//   const [searchParams, setSearchParams] = useSearchParams();

//   // Get current tab from URL, fallback to first tab if missing or invalid
//   const tabParam = searchParams.get('tab');
//   const activeTab = tabs.find((t) => t.key === tabParam) ? tabParam! : tabs[0].key;

//   // Handler to update URL when clicking a tab
...
//                   className={`group px-6 py-3.5 text-sm font-semibold rounded-2xl transition-all duration-500 ease-out transform hover:scale-105 hover:-translate-y-1 relative overflow-hidden shadow-lg ${
//                     activeTab === t.key
...
//                         activeTab === t.key ? 'scale-110' : 'group-hover:scale-110'
//                       }`}
//                     >
//                       {t.icon}
//                     </span>
...
//               {/* {activeTab === 'Biometic-System' && <BioUserDashboard/>} */}
//               {activeTab === 'Global-Matrix' && <GlobalMatrixControls />}
//               {activeTab === 'Matrix-Configuration' && <MatrixConfiguration />}
//               {/* {activeTab === 'Refreshment-interval-setting' && <RecurrenceSettings />} */}
//               {activeTab === 'Skill-Evaluation-Criteria' && <CriteriaManagement />}
//               {activeTab === 'MinimumLevelConfig' && <MinimumLevelConfig />}
//             </div>
//           </div>
//         </div>
//       </div>
...
// src/components/pages/MethodPage.tsx
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950 p-4 md:p-8 transition-colors duration-300">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-gradient-to-tr from-indigo-400/20 to-pink-400/20 dark:from-indigo-600/10 dark:to-pink-600/10 rounded-full blur-3xl animate-pulse delay-1000"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-60 h-60 bg-gradient-to-r from-cyan-400/10 to-blue-400/10 dark:from-cyan-600/10 dark:to-blue-600/10 rounded-full blur-3xl animate-pulse delay-500"></div>
      </div>
      ...
      <div className=" mx-auto relative z-10">
        ...
        <div className="mb-10">
          <div className="flex items-center space-x-4 mb-4">
            ...
            <div>
              <h1 className="text-4xl md:text-5xl font-black bg-gradient-to-r from-slate-800 via-blue-700 to-indigo-700 dark:from-white dark:via-blue-300 dark:to-indigo-300 bg-clip-text text-transparent">
                Method Settings
              </h1>
              <div className="h-1 w-32 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full mt-2"></div>
            </div>
          </div>
          <p className="text-slate-600 dark:text-slate-400 text-lg font-medium ml-16 opacity-80">
            Configure and manage various system settings with advanced controls
          </p>
        </div>

        {/* Main Content Card with Glass Morphism */}
        <div className="bg-white/80 dark:bg-gray-900/60 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/50 dark:border-white/10 overflow-hidden relative transition-colors duration-300">
          {/* Decorative top border */}
          <div className="h-1 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500"></div>

          {/* Enhanced Tab Navigation */}
          <div className="bg-gradient-to-r from-white/90 via-slate-50/90 to-white/90 dark:from-gray-900/90 dark:via-gray-800/90 dark:to-gray-900/90 backdrop-blur-sm px-4 py-6 border-b border-slate-200/50 dark:border-gray-700/50 transition-colors duration-300">
            <div className="flex flex-wrap gap-2">
              {tabs.map((t, index) => (
                <button
                  key={t.key}
                  ...
                  className={`group px-6 py-3.5 text-sm font-semibold rounded-2xl transition-all duration-500 ease-out transform hover:scale-105 hover:-translate-y-1 relative overflow-hidden shadow-lg ${
                    activeTab === t.key
                      ? 'bg-gradient-to-r from-blue-500 via-purple-500 to-indigo-600 text-white shadow-2xl shadow-blue-500/30'
                      : 'bg-white/70 dark:bg-gray-800/40 text-slate-700 dark:text-slate-200 hover:text-blue-700 dark:hover:text-blue-300 hover:bg-white dark:hover:bg-gray-700 hover:shadow-xl border border-slate-200/50 dark:border-gray-700/50 hover:border-blue-300/50'
                  }`}
                  style={{
                    animationDelay: `${index * 100}ms`,
                  }}
          ...
          {/* Content Area with Advanced Styling */}
          <div className="p-8 md:p-12 bg-gradient-to-br from-white/50 via-slate-50/30 to-blue-50/40 dark:from-gray-900/50 dark:via-gray-900/30 dark:to-blue-900/20 min-h-[700px] relative transition-colors duration-300">
            {/* Content background pattern */}
            <div
              className="absolute inset-0 opacity-5 dark:opacity-10"
              style={{
                backgroundImage: `radial-gradient(circle at 25px 25px, currentColor 2px, transparent 0), radial-gradient(circle at 75px 75px, #3b82f6 2px, transparent 0)`,
                backgroundSize: '100px 100px',
              }}
            ></div>
