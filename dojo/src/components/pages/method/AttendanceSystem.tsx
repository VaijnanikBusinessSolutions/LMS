...
// const CLEAR_DATE_ENDPOINT = 'biometric-attendance/clear-date/';
// const CLEAR_MONTH_ENDPOINT = 'biometric-attendance/clear-month/';
...
//   // --- DELETE MODAL STATES ---
//   const [isDeleteDailyModalOpen, setIsDeleteDailyModalOpen] = useState(false);
...
//       try {
//           const response = await axios.delete(`${API_BASE_URL}${CLEAR_DATE_ENDPOINT}`, {
//               params: { date: selectedDate }
//           });
          
//           // Clear local data
//           setDailyData([]);
//           setIsDeleteDailyModalOpen(false);
//           setDeleteConfirmText("");
          
//           // Show success message
//           alert(response.data.message || `Successfully cleared all attendance records for ${selectedDate}`);
//       } catch (err: any) {
//           console.error(err);
//           const errorMessage = err.response?.data?.error || err.message || "Failed to delete records";
...
//       setDeleteLoading(true);
//       try {
//           const response = await axios.delete(`${API_BASE_URL}${CLEAR_MONTH_ENDPOINT}`, {
...
//           // Show success message
//           alert(response.data.message || `Successfully cleared all attendance records`);
//       } catch (err: any) {
//           console.error(err);
//           const errorMessage = err.response?.data?.error || err.message || "Failed to delete records";
//           alert("Failed to delete: " + errorMessage);
...
//   const openDeleteDailyModal = () => {
//       setDeleteConfirmText("");
//       setIsDeleteDailyModalOpen(true);
//   };

//   const openDeleteMonthlyModal = () => {
//       setDeleteConfirmText("");
//       setIsDeleteMonthlyModalOpen(true);
//   };
...
//                   <div className={`mt-8 px-8 py-3 rounded-full text-lg font-bold shadow-xl ${
//                       status?.toLowerCase().includes('present') || status?.toLowerCase().startsWith('p')
...
//   const renderDeleteConfirmation = (type: 'daily' | 'monthly') => {
...
//       const recordCount = isDaily ? dailyData.length : summaryData.length;
...
//                       Delete All {isDaily ? 'Daily' : 'Monthly'} Records?
//                   </h3>
//                   <p className="text-gray-600 mb-4">
//                       You are about to permanently delete <strong className="text-red-600">{recordCount}</strong> attendance records for:
//                   </p>
...
//                       <span className="font-semibold text-red-700">{dateDisplay}</span>
//                   </div>
//               </div>
...
//                               <li>All attendance records for this {isDaily ? 'date' : 'month'} will be permanently removed</li>
//                               <li>Employee attendance history will be affected</li>
//                               <li>You will need to re-upload the data if needed</li>
//                           </ul>
//                       </div>
//                   </div>
//               </div>
...
//                       className={`w-full px-4 py-3 border-2 rounded-xl text-center text-lg font-mono uppercase tracking-widest transition-all ${
...
//                       <p className="text-xs text-red-500 mt-1 text-center">Please type exactly: DELETE</p>
//                   )}
//                   {deleteConfirmText === "DELETE" && (
//                       <p className="text-xs text-green-600 mt-1 text-center flex items-center justify-center gap-1">
//                           <CheckCircle2 className="w-3 h-3" /> Ready to delete
//                       </p>
//                   )}
//               </div>
...
//                   <button
//                       onClick={() => {
//                           isDaily ? setIsDeleteDailyModalOpen(false) : setIsDeleteMonthlyModalOpen(false);
//                           setDeleteConfirmText("");
//                       }}
//                       disabled={deleteLoading}
//                       className="flex-1 px-6 py-3 border-2 border-gray-300 text-gray-700 font-semibold rounded-xl hover:bg-gray-100 transition-colors disabled:opacity-50"
//                   >
//                       Cancel
//                   </button>
//                   <button
//                       onClick={isDaily ? handleDeleteDailyData : handleDeleteMonthlyData}
...
//                               disabled={processedDailyData.length === 0}
...
//                           </button>
//                           {/* DELETE BUTTON FOR DAILY */}
//                           {dailyData.length > 0 && selectedDate && (
//                               <button 
//                                   onClick={openDeleteDailyModal}
...
//                               disabled={processedMonthlyData.length === 0}
...
//                               <Download className="w-4 h-4"/> Export
//                           </button>
//                           {/* DELETE BUTTON FOR MONTHLY */}
//                           {summaryData.length > 0 && (
//                               <button 
//                                   onClick={openDeleteMonthlyModal}
//                                   className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors text-sm font-medium"
//                               >
...
//       {/* DELETE DAILY CONFIRMATION MODAL */}
//       <Modal
//           isOpen={isDeleteDailyModalOpen}
//           onClose={() => { setIsDeleteDailyModalOpen(false); setDeleteConfirmText(""); }}
//           size="md"
...
//                       <h2 className="text-xl font-bold">Clear Daily Records</h2>
//                   </div>
//                   <button 
//                       onClick={() => { setIsDeleteDailyModalOpen(false); setDeleteConfirmText(""); }}
//                       className="bg-white/20 hover:bg-white/30 p-2 rounded-full transition-colors"
//                   >
//                       <X className="w-5 h-5"/>
//                   </button>
...
//           }
//       >
//           {renderDeleteConfirmation('daily')}
//       </Modal>

//       {/* DELETE MONTHLY CONFIRMATION MODAL */}
//       <Modal
//           isOpen={isDeleteMonthlyModalOpen}
//           onClose={() => { setIsDeleteMonthlyModalOpen(false); setDeleteConfirmText(""); }}
//           size="md"
//           headerColor="from-red-600 to-rose-600"
//           headerContent={
//               <div className="flex justify-between items-center w-full">
//                   <div className="flex items-center gap-3">
//                       <Trash2 className="w-6 h-6" />
//                       <h2 className="text-xl font-bold">Clear Monthly Records</h2>
//                   </div>
//                   <button 
//                       onClick={() => { setIsDeleteMonthlyModalOpen(false); setDeleteConfirmText(""); }}
//                       className="bg-white/20 hover:bg-white/30 p-2 rounded-full transition-colors"
//                   >
//                       <X className="w-5 h-5"/>
//                   </button>
...
import SetAutoFetchTime from './SetAttendanceTaskTime';
import Modal from './Modal'; // Assuming Modal component is theme-aware or simple enough
...
const AttendanceSystem: React.FC = () => {
  ...
  const PaginationControls: React.FC<{
      ...
      return (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 bg-muted/5 border-t border-border">
              <div className="flex items-center gap-4 text-sm text-muted">
                  ...
                  <div className="flex items-center gap-2">
                      ...
                      <select 
                          value={rowsPerPage} 
                          onChange={(e) => onRowsPerPageChange(Number(e.target.value))}
                          className="px-2 py-1 border border-border bg-background text-text rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20"
                      >
                          <option value={10}>10</option>
                          <option value={25}>25</option>
                          <option value={50}>50</option>
              ...
              <div className="flex items-center gap-1">
                  <button 
                      onClick={() => onPageChange(1)} 
                      disabled={currentPage === 1}
                      className="px-3 py-2 text-sm border border-border bg-background text-text rounded-lg hover:bg-muted/10 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                      First
                  </button>
                  <button 
                      onClick={() => onPageChange(currentPage - 1)} 
                      disabled={currentPage === 1}
                      className="p-2 border border-border bg-background text-text rounded-lg hover:bg-muted/10 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                      <ChevronLeft className="w-4 h-4" />
                  </button>
                      ...
                      <button
                          ...
                          className={`px-3 py-2 text-sm border rounded-lg transition-colors ${
                              currentPage === page 
                                  ...
                                  : 'border-border bg-background text-text hover:bg-muted/10'
                          }`}
                      >
                          {page}
                      </button>
  ...
  const renderCalendar = () => {
      ...
      for (let i = 0; i < firstDay; i++) {
          days.push(
              <div key={`empty-${i}`} className="h-20 bg-muted/5 border-b border-r border-border"></div>
          );
      }
      ...
      for (let d = 1; d <= daysInMonth; d++) {
          const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
          const log = employeeLogs.find(l => l.attendance_date === dateStr);
          let bgColor = "bg-surface hover:bg-muted/5", statusColor = "text-muted", statusText = "-", borderClass = "";
          ...
          if (log) {
              if (log.status.toLowerCase().startsWith('p')) { 
                  ...
                  statusText = "P"; 
                  borderClass = "border-l-4 border-l-green-500 dark:border-l-green-600";
              }
              else if (log.status.toLowerCase().startsWith('a')) { 
                  ...
                  statusText = log.status.substring(0, 2); 
                  borderClass = "border-l-4 border-l-yellow-500 dark:border-l-yellow-600";
              }
          }
          
          days.push(
              <div key={d} className={`h-20 p-2 border-b border-r border-border relative ${bgColor} ${borderClass} transition-colors cursor-pointer`}>
                  <span className="text-xs font-bold text-muted">{d}</span>
                  {log ? (
                    <div className="mt-1 flex flex-col items-center">
                        <span className={`text-xl font-black ${statusColor}`}>{statusText}</span>
      ...
      return (
          <div className="grid grid-cols-7 border-t border-l border-border rounded-xl overflow-hidden shadow-inner">
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
                  <div key={d} className="h-10 flex items-center justify-center bg-muted/5 text-xs font-bold text-muted border-b border-r border-border">
                      {d}
                  </div>
              ))}
              {days}
      ...
      return (
          <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="flex flex-col items-center justify-center">
                  <div className="relative w-64 h-64">
                      <svg className="w-full h-full transform -rotate-90">
                          <circle cx="128" cy="128" r="116" stroke="rgb(var(--border-main))" strokeWidth="20" fill="transparent" />
                          <circle
                              cx="128" cy="128" r="116"
                              stroke="currentColor"
                              strokeWidth="20"
                              ...
                              strokeDashoffset={728 - (728 * progress) / 100}
                              strokeLinecap="round"
                              className={`${hours >= 8 ? 'text-emerald-500 dark:text-emerald-400' : hours >= 6 ? 'text-blue-500 dark:text-blue-400' : 'text-orange-500 dark:text-orange-400'} transition-all duration-1000`}
                          />
                      </svg>
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <span className="text-6xl font-black text-text">{hours.toFixed(1)}</span>
                          <span className="text-lg text-muted font-medium">Hours Worked</span>
                      </div>
                  </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                      {isLate && (
                          <div className="flex items-center justify-between p-5 rounded-2xl bg-gradient-to-r from-orange-50 to-red-50 dark:from-orange-900/20 dark:to-red-900/20 border-2 border-red-200 dark:border-red-800">
                              <div className="flex items-center">
                                  <AlertCircle className="w-8 h-8 text-red-600 dark:text-red-400 mr-4"/>
                                  <div>
                                      <p className="font-bold text-red-700 dark:text-red-300">Late Arrival</p>
                                      <p className="text-sm text-red-600 dark:text-red-400">You were late today</p>
                                  </div>
                              </div>
                              <span className="text-2xl font-black text-red-700 dark:text-red-300">+{late_arrival} min</span>
                          </div>
                      )}

                      {!isLate && (status?.toLowerCase().includes('present') || status?.toLowerCase().startsWith('p')) && (
                          <div className="flex items-center justify-between p-5 rounded-2xl bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-900/20 dark:to-green-900/20 border-2 border-green-200 dark:border-green-800">
                              <div className="flex items-center">
                                  <CheckCircle2 className="w-8 h-8 text-green-600 dark:text-green-400 mr-4"/>
                                  <div>
                                      <p className="font-bold text-green-700 dark:text-green-300">On Time!</p>
                                      <p className="text-sm text-green-600 dark:text-green-400">Great job arriving early</p>
                                  </div>
                              </div>
                              <span className="text-3xl">✨</span>
                          </div>
                      ...
                      {isOT && (
                          <div className="flex items-center justify-between p-5 rounded-2xl bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 border-2 border-purple-200 dark:border-purple-800">
                              <div className="flex items-center">
                                  <Timer className="w-8 h-8 text-purple-600 dark:text-purple-400 mr-4"/>
                                  <div>
                                      <p className="font-bold text-purple-700 dark:text-purple-300">Overtime Bonus</p>
                                      <p className="text-sm text-purple-600 dark:text-purple-400">Extra effort recognized</p>
                                  </div>
                              </div>
                              <span className="text-2xl font-black text-purple-700 dark:text-purple-300">+{ot} hrs</span>
                          </div>
                      )}
                  </div>
              </div>
          ...
          <div className="p-6">
              ...
              <div className="text-center mb-6">
                  ...
                  <p className="text-muted mb-4">
                      You are about to permanently delete <strong className="text-red-600 dark:text-red-400">{recordCount}</strong> attendance records for:
                  </p>
                  <div className="inline-flex items-center px-4 py-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                      ...
                      <span className="font-semibold text-red-700 dark:text-red-300">{dateDisplay}</span>
                  </div>
              </div>
              ...
              <div className="mb-6">
                  <label className="block text-sm font-medium text-text mb-2">
                      Type <span className="font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded">DELETE</span> to confirm:
                  </label>
                  <input
                      type="text"
                      value={deleteConfirmText}
                      onChange={(e) => setDeleteConfirmText(e.target.value.toUpperCase())}
                      placeholder="Type DELETE here"
                      className={`w-full px-4 py-3 border-2 border-border bg-background text-text rounded-xl text-center text-lg font-mono uppercase tracking-widest transition-all ${
                          deleteConfirmText === "DELETE" 
                              ? 'border-green-500 bg-green-50 dark:bg-green-900/20 focus:ring-4 focus:ring-green-100 dark:focus:ring-green-900/30' 
                              : 'border-border focus:border-red-500 focus:ring-4 focus:ring-red-100 dark:focus:ring-red-900/30'
                      }`}
                  />
                  {deleteConfirmText && deleteConfirmText !== "DELETE" && (
                      <p className="text-xs text-red-500 dark:text-red-400 mt-1 text-center">Please type exactly: DELETE</p>
                  )}
                  {deleteConfirmText === "DELETE" && (
              ...
              <div className="flex gap-4">
                  <button
                      ...
                      disabled={deleteLoading}
                      className="flex-1 px-6 py-3 border-2 border-border text-text bg-surface hover:bg-muted/10 font-semibold rounded-xl transition-colors disabled:opacity-50"
                  >
                      Cancel
                  </button>
                  <button
  ...
  const renderDailyOps = () => (
      <div className="space-y-6">
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                  <div className="w-full lg:w-1/3">
                      ...
                      <input 
                          ...
                          value={selectedDate} 
                          onChange={(e) => setSelectedDate(e.target.value)} 
                          className="w-full px-4 py-3 border border-border rounded-xl bg-background text-text font-medium focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/20 focus:border-blue-400 transition-all"
                      />
                  </div>
                  <form onSubmit={handleUpload} className="w-full lg:w-2/3 flex flex-col sm:flex-row gap-4">
                      <div className="flex-1">
                          ...
                          <div className="relative">
                              <input 
                                  ...
                                  accept=".xlsx,.xls" 
                                  onChange={(e) => setUploadFile(e.target.files?.[0] || null)} 
                                  className="block w-full text-sm text-muted border-2 border-dashed border-border rounded-xl p-3 hover:border-green-400 dark:hover:border-green-600 transition-colors file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-green-50 dark:file:bg-green-900/30 file:text-green-700 dark:file:text-green-400 hover:file:bg-green-100 dark:hover:file:bg-green-900/50"
                              />
                          </div>
                      </div>
                      <div className="flex items-end">
          ...
          {dailyData.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                  <StatCard icon={<Users className="w-6 h-6"/>} label="Total" value={dailyStats.total} color="text-blue-600 dark:text-blue-400" bgColor="bg-blue-50 dark:bg-blue-900/20" />
                  <StatCard icon={<UserCheck className="w-6 h-6"/>} label="Present" value={dailyStats.present} subValue={`${((dailyStats.present / dailyStats.total) * 100).toFixed(1)}%`} color="text-green-600 dark:text-green-400" bgColor="bg-green-50 dark:bg-green-900/20" />
                  <StatCard icon={<UserX className="w-6 h-6"/>} label="Absent" value={dailyStats.absent} subValue={`${((dailyStats.absent / dailyStats.total) * 100).toFixed(1)}%`} color="text-red-600 dark:text-red-400" bgColor="bg-red-50 dark:bg-red-900/20" />
                  <StatCard icon={<AlarmClock className="w-6 h-6"/>} label="Late" value={dailyStats.late} color="text-orange-600 dark:text-orange-400" bgColor="bg-orange-50 dark:bg-orange-900/20" />
                  <StatCard icon={<CheckCircle2 className="w-6 h-6"/>} label="On Time" value={dailyStats.onTime} color="text-emerald-600 dark:text-emerald-400" bgColor="bg-emerald-50 dark:bg-emerald-900/20" />
                  <StatCard icon={<Timer className="w-6 h-6"/>} label="OT Workers" value={dailyStats.otWorkers} subValue={`${dailyStats.totalOtHours.toFixed(1)} hrs total`} color="text-purple-600 dark:text-purple-400" bgColor="bg-purple-50 dark:bg-purple-900/20" />
              </div>
          )}
          ...
          <div className="bg-surface rounded-2xl shadow-sm border border-border overflow-hidden">
              <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border-b border-border">
                  <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                      <div className="flex items-center gap-3">
                          <h3 className="font-bold text-text text-lg">Daily Logs</h3>
                          <span className="bg-blue-600 dark:bg-blue-700 text-white text-xs font-bold px-3 py-1 rounded-full">
                              {processedDailyData.length} entries
                          </span>
                          {selectedDate && (
                              <span className="bg-muted/10 text-muted text-xs font-medium px-3 py-1 rounded-full">
                                  {new Date(selectedDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                              </span>
                          )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                          <div className="relative">
                              ...
                              <input 
                                  ...
                                  value={dailySearchTerm}
                                  onChange={(e) => { setDailySearchTerm(e.target.value); setDailyCurrentPage(1); }}
                                  className="pl-10 pr-4 py-2 border border-border bg-background text-text rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 w-64"
                              />
                          </div>
                          <button 
                              onClick={() => setShowDailyFilters(!showDailyFilters)}
                              className={`p-2 rounded-lg border border-border transition-colors ${showDailyFilters ? 'bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400' : 'text-muted hover:bg-muted/10'}`}
                          >
                              <SlidersHorizontal className="w-5 h-5"/>
                          </button>
                          <button 
                              onClick={exportDailyData}
                              disabled={processedDailyData.length === 0}
                              className="flex items-center gap-2 px-4 py-2 bg-blue-600 dark:bg-blue-700 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                              <Download className="w-4 h-4"/> Export
                          </button>
                          {/* DELETE BUTTON FOR DAILY */}
                  ...
                  {showDailyFilters && (
                      <div className="mt-4 pt-4 border-t border-border flex flex-wrap gap-4 items-center p-4">
                          <div>
                              <label className="block text-xs font-medium text-muted mb-1">Status</label>
                              <select 
                                  value={dailyStatusFilter}
                                  onChange={(e) => { setDailyStatusFilter(e.target.value); setDailyCurrentPage(1); }}
                                  className="px-3 py-2 border border-border bg-background text-text rounded-lg text-sm"
                              >
                                  <option value="all">All Status</option>
                                  <option value="present">Present</option>
                                  <option value="absent">Absent</option>
                          ...
                          <button 
                              onClick={resetDailyFilters}
                              className="flex items-center gap-1 px-3 py-2 text-sm text-muted hover:text-text hover:bg-muted/10 rounded-lg transition-colors mt-5"
                          >
                              <RefreshCw className="w-4 h-4"/> Reset
                          </button>
                      </div>
              ...
              <div className="overflow-x-auto">
                  {paginatedDailyData.length > 0 ? (
                              ...
                              <tr>
                                  {dailyFields.map(f => (
                                      <th 
                                          key={f.key} 
                                          onClick={() => f.sortable && handleDailySort(f.key)}
                                          className={`px-6 py-4 text-left text-xs font-bold text-muted uppercase tracking-wider ${f.sortable ? 'cursor-pointer hover:bg-muted/10 select-none' : ''}`}
                                      >
                                          <div className="flex items-center gap-1">
                                              {f.label}
                                              {f.sortable && dailySortConfig?.key === f.key && (
                          ...
                          <tbody className="bg-surface divide-y divide-border">
                              {paginatedDailyData.map((row, idx) => {
                                  ...
                                  return (
                                      ...
                                      >
                                          <td className="px-6 py-4 whitespace-nowrap text-sm text-muted">{row.sr_no || '-'}</td>
                                          <td className="px-6 py-4 whitespace-nowrap text-sm font-mono font-medium text-text">{row.card_no}</td>
                                          <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-text group-hover:text-blue-700 dark:group-hover:text-blue-400">{row.employee_name}</td>
                                          <td className="px-6 py-4 whitespace-nowrap text-sm text-muted">{row.department}</td>
                                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-600 dark:text-blue-400">{row.in_time?.substring(0,5) || '-'}</td>
                                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-purple-600 dark:text-purple-400">{row.out_time?.substring(0,5) || '-'}</td>
                                          <td className="px-6 py-4 whitespace-nowrap">
                                              <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${
                                                  isPresent ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400'
                                              }`}>
                                                  {isPresent ? '✓' : '✗'} {row.status}
                                              </span>
                                          </td>
                                          <td className="px-6 py-4 whitespace-nowrap">
                                              {isLate ? (
                                                  <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-bold bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400">
                                                      <TrendingDown className="w-3 h-3 mr-1"/> {row.late_arrival}
                                                  </span>
                                          ...
                                          <td className="px-6 py-4 whitespace-nowrap">
                                              {hasOT ? (
                                                  <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-bold bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400">
                                                      <TrendingUp className="w-3 h-3 mr-1"/> +{row.ot}
                                                  </span>
                                              ) : <span className="text-muted">-</span>}
                                          </td>
                      ...
                      <div className="p-16 text-center">
                          <div className="w-20 h-20 mx-auto mb-4 bg-muted/10 rounded-full flex items-center justify-center">
                              <Calendar className="w-10 h-10 text-muted"/>
                          </div>
                          <h3 className="text-lg font-semibold text-text mb-2">No Records Found</h3>
                          <p className="text-muted mb-4">
                              {selectedDate ? "No attendance data for this date" : "Select a date to view attendance logs"}
                          </p>
                          {!selectedDate && (
  ...
  const renderMonthlyLedger = () => (
      <div className="space-y-6">
          ...
          <div className="bg-surface p-6 rounded-2xl shadow-sm border border-border">
              <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-end">
                  <div>
                      <label className="block text-sm font-bold text-text mb-2">Month</label>
                      <select 
                          value={selectedMonth} 
                          onChange={(e) => setSelectedMonth(e.target.value)} 
                          className="px-4 py-3 border border-border rounded-xl bg-background text-text font-medium focus:ring-4 focus:ring-purple-100 dark:focus:ring-purple-900/20 focus:border-purple-400"
                      >
                          {Array.from({length: 12}, (_, i) => (
                              <option key={i+1} value={i+1}>
                                  {new Date(0, i).toLocaleString('default', {month: 'long'})}
                  ...
                  <div>
                      <label className="block text-sm font-bold text-text mb-2">Year</label>
                      <select 
                          value={selectedYear} 
                          onChange={(e) => setSelectedYear(e.target.value)} 
                          className="px-4 py-3 border border-border rounded-xl bg-background text-text font-medium focus:ring-4 focus:ring-purple-100 dark:focus:ring-purple-900/20 focus:border-purple-400"
          ...
          {summaryData.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                  <StatCard icon={<Users className="w-6 h-6"/>} label="Employees" value={monthlyStats.totalEmployees} color="text-purple-600 dark:text-purple-400" bgColor="bg-purple-50 dark:bg-purple-900/20" />
                  <StatCard icon={<UserCheck className="w-6 h-6"/>} label="Avg Present" value={monthlyStats.avgPresent.toFixed(1)} subValue="days/employee" color="text-green-600 dark:text-green-400" bgColor="bg-green-50 dark:bg-green-900/20" />
                  <StatCard icon={<UserX className="w-6 h-6"/>} label="Avg Absent" value={monthlyStats.avgAbsent.toFixed(1)} subValue="days/employee" color="text-red-600 dark:text-red-400" bgColor="bg-red-50 dark:bg-red-900/20" />
                  <StatCard icon={<Timer className="w-6 h-6"/>} label="Total OT" value={monthlyStats.totalOtHours.toFixed(1)} subValue="hours" color="text-blue-600 dark:text-blue-400" bgColor="bg-blue-50 dark:bg-blue-900/20" />
                  <StatCard icon={<CheckCircle2 className="w-6 h-6"/>} label="Perfect" value={monthlyStats.perfectAttendance} subValue="0 absents" color="text-emerald-600 dark:text-emerald-400" bgColor="bg-emerald-50 dark:bg-emerald-900/20" />
                  <StatCard icon={<AlertCircle className="w-6 h-6"/>} label="Critical" value={monthlyStats.criticalAttendance} subValue="<50% attendance" color="text-orange-600 dark:text-orange-400" bgColor="bg-orange-50 dark:bg-orange-900/20" />
              </div>
          )}

          {/* Search, Filters & Quick Sort */}
          <div className="bg-surface rounded-2xl shadow-sm border border-border overflow-hidden">
              <div className="p-4 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 border-b border-border">
                  <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                      <div className="flex items-center gap-3">
                          <h3 className="font-bold text-text text-lg">Monthly Summary</h3>
                          <span className="bg-purple-600 dark:bg-purple-700 text-white text-xs font-bold px-3 py-1 rounded-full">
                              {processedMonthlyData.length} employees
                          </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                          <div className="relative">
                              ...
                              <input 
                                  ...
                                  value={monthlySearchTerm}
                                  onChange={(e) => { setMonthlySearchTerm(e.target.value); setMonthlyCurrentPage(1); }}
                                  className="pl-10 pr-4 py-2 border border-border bg-background text-text rounded-lg text-sm focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 w-48"
                              />
                          </div>
                          <button 
                              onClick={() => setShowMonthlyFilters(!showMonthlyFilters)}
                              className={`p-2 rounded-lg border border-border transition-colors ${showMonthlyFilters ? 'bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400' : 'text-muted hover:bg-muted/10'}`}
                          >
                              <SlidersHorizontal className="w-5 h-5"/>
                          </button>
                          <button 
                              onClick={exportMonthlyData}
                              disabled={processedMonthlyData.length === 0}
                              className="flex items-center gap-2 px-4 py-2 bg-purple-600 dark:bg-purple-700 text-white rounded-lg hover:bg-purple-700 dark:hover:bg-purple-600 transition-colors text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                              <Download className="w-4 h-4"/> Export
                          </button>
                          {/* DELETE BUTTON FOR MONTHLY */}
                  ...
                  {showMonthlyFilters && (
                      <div className="mt-4 pt-4 border-t border-border flex flex-wrap gap-4 items-center p-4">
                          ...
                          <div>
                              <label className="block text-xs font-medium text-muted mb-1">Quick Sort</label>
                              <select 
                                  value={quickSortOption}
                                  onChange={(e) => { setQuickSortOption(e.target.value); setMonthlyCurrentPage(1); }}
                                  className="px-3 py-2 border border-border bg-background text-text rounded-lg text-sm"
                          ...
                          <button 
                              onClick={resetMonthlyFilters}
                              className="flex items-center gap-1 px-3 py-2 text-sm text-muted hover:text-text hover:bg-muted/10 rounded-lg transition-colors"
                          >
                              <RefreshCw className="w-4 h-4"/> Reset
                          </button>
                      </div>
              ...
              <div className="overflow-x-auto">
                  {paginatedMonthlyData.length > 0 ? (
                      <table className="min-w-full divide-y divide-border">
                          <thead className="bg-muted/5">
                              <tr>
                                  ...
                                  <th className="px-6 py-4 text-left text-xs font-bold text-muted uppercase">Rate</th>
                              </tr>
                          </thead>
                          <tbody className="bg-surface divide-y divide-border">
                              {paginatedMonthlyData.map((row, idx) => {
                                  const attendanceRate = row.total_present / (row.total_present + row.total_absent) * 100 || 0;
                                  let rateColor = 'text-green-600 bg-green-100 dark:bg-green-900/30 dark:text-green-400';
                                  if (attendanceRate < 80) rateColor = 'text-yellow-600 bg-yellow-100 dark:bg-yellow-900/30 dark:text-yellow-400';
                                  if (attendanceRate < 60) rateColor = 'text-red-600 bg-red-100 dark:bg-red-900/30 dark:text-red-400';
                                  
                                  return (
                                      <tr 
                                          key={idx} 
                                          onClick={() => openCalendarModal(row)} 
                                          className="hover:bg-purple-50 dark:hover:bg-purple-900/20 cursor-pointer transition-colors group"
                                      >
                                          <td className="px-6 py-4 whitespace-nowrap text-sm font-mono font-medium text-text">{row.card_no}</td>
                                          <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-text group-hover:text-purple-700 dark:group-hover:text-purple-400">{row.employee_name}</td>
                                          <td className="px-6 py-4 whitespace-nowrap text-sm text-muted">{row.department}</td>
                                          <td className="px-6 py-4 whitespace-nowrap">
                                          ...
                                          <td className="px-6 py-4 whitespace-nowrap">
                                              <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${row.total_absent > 0 ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' : 'bg-muted/10 text-muted'}`}>
                                                  {row.total_absent}
                                              </span>
                                          </td>
                                          <td className="px-6 py-4 whitespace-nowrap text-sm text-orange-600 dark:text-orange-400 font-medium">{(row.total_early||0).toFixed(2)}</td>
                                          <td className="px-6 py-4 whitespace-nowrap text-sm text-red-500 dark:text-red-400 font-medium">{(row.total_late||0).toFixed(2)}</td>
                                          <td className="px-6 py-4 whitespace-nowrap">
                                              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400">
                                                  {(row.total_ot_hours||0).toFixed(2)}
                                              </span>
                                          </td>
                                          <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="p-16 text-center">
                          ...
                          <h3 className="text-lg font-semibold text-text mb-2">No Summary Data</h3>
                          <p className="text-muted mb-4">Select a month and year, then click "Generate Report"</p>
                      </div>
                  )}
              </div>
    ...
    <div className="min-h-screen bg-background text-text transition-colors duration-300">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
            ...
            <p className="text-muted mt-1 ml-14">Track, analyze, and manage employee attendance</p>
        </div>
        
        {/* Navigation Tabs */}
        <div className="bg-surface rounded-2xl shadow-sm border border-border mb-8 p-2">
            <nav className="flex space-x-2">
                <button 
                    onClick={() => setActiveTab('daily-ops')} 
                    className={`flex items-center px-6 py-3 rounded-xl font-semibold transition-all ${
                ...
                <button 
                    ...
                    className={`flex items-center px-6 py-3 rounded-xl font-semibold transition-all ${
                        activeTab === 'monthly-ledger' 
                            ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-lg shadow-purple-200 dark:shadow-purple-900/30' 
                            : 'text-muted hover:bg-muted/10'
                    }`}
                >
                    <BarChart3 className="w-5 h-5 mr-2"/> Monthly Ledger
                ...
                <button 
                    ...
                    className={`flex items-center px-6 py-3 rounded-xl font-semibold transition-all ${
                        activeTab === 'settings' 
                            ? 'bg-gradient-to-r from-gray-700 to-gray-800 text-white shadow-lg shadow-gray-200 dark:shadow-gray-900/30' 
                            : 'text-muted hover:bg-muted/10'
                    }`}
                >
                    <Fingerprint className="w-5 h-5 mr-2"/> Settings
      ...
      >
          <div className="p-6">
              {modalLoading ? (
                  <div className="flex items-center justify-center py-20">
                      <Loader2 className="animate-spin w-10 h-10 text-purple-600 dark:text-purple-400"/>
                  </div>
              ) : (
                  <>
                      <div className="flex items-center gap-6 mb-4 pb-4 border-b border-border">
                          <span className="text-sm font-medium text-text">Legend:</span>
                          <div className="flex items-center gap-2">
                              <div className="w-4 h-4 bg-green-500 rounded"></div>
                          ...
                          <div className="flex items-center gap-2">
                              <div className="w-4 h-4 bg-yellow-500 rounded"></div>
                              <span className="text-sm text-muted">Half Day / Other</span>
                          </div>
                          <div className="flex items-center gap-2">
