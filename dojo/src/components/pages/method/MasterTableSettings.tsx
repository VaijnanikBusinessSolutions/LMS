...
//   // --- Add these lines after the [formData, setFormData] state ---
...
//     }
//   };
//   // --- Add these functions after handleDownloadTemplate ---

//   const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
//     if (e.target.checked) {
//       const allIds = employeeData.map(emp => emp.emp_id);
...
//     if (window.confirm(`Are you sure you want to delete ${selectedEmpIds.length} selected employee(s)?`)) {
//       setIsDeleting(true);
//       try {
//         const deletePromises = selectedEmpIds.map(empId =>
//           fetch(`${API_BASE_URL}/mastertable/${empId}/`, { method: 'DELETE' })
//         );
//         await Promise.all(deletePromises);

//         // Refresh state locally for immediate feedback
//         setEmployeeData(prev => prev.filter(emp => !selectedEmpIds.includes(emp.emp_id)));
//         setSelectedEmpIds([]); // Clear selection
//         alert(`${selectedEmpIds.length} employee(s) deleted successfully.`);
//       } catch (error) {
...
//     if (window.confirm('DANGER: Are you sure you want to delete ALL employees? This action cannot be undone.')) {
//       setIsDeleting(true);
//       const allIds = employeeData.map(emp => emp.emp_id);
//       try {
//         // Note: A dedicated backend endpoint for bulk delete would be more efficient.
//         const deletePromises = allIds.map(empId =>
//           fetch(`${API_BASE_URL}/mastertable/${empId}/`, { method: 'DELETE' })
//         );
//         await Promise.all(deletePromises);
...
//       // --- Replace the entire `case 'employee-list':` block with this ---

//       case 'employee-list':
//         const isAllSelected = employeeData.length > 0 && selectedEmpIds.length === employeeData.length;
...
//                       disabled={isDeleting}
//                       className="inline-flex items-center px-3 py-1.5 border border-transparent text-sm font-medium rounded-lg text-white bg-red-600 hover:bg-red-700 disabled:bg-red-400"
//                     >
//                       {isDeleting ? (
...
//                   disabled={isDeleting || employeeData.length === 0}
//                   className="inline-flex items-center px-3 py-1.5 border border-transparent text-sm font-medium rounded-lg text-white bg-red-800 hover:bg-red-900 disabled:bg-gray-400"
//                 >
//                   <Trash2 className="h-4 w-4 mr-2" />
//                   Delete All
//                 </button>
...
//                       <tr 
//                         key={employee.emp_id} 
//                         className={`transition-colors duration-150 ${selectedEmpIds.includes(employee.emp_id) ? 'bg-blue-50' : 'hover:bg-gray-50'}`}
//                       >
//                         <td className="px-4 py-4">
//                           <input
//                             type="checkbox"
//                             className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
//                             checked={selectedEmpIds.includes(employee.emp_id)}
...
//                             className="inline-flex items-center px-3 py-1 border border-red-300 text-sm leading-4 font-medium rounded-lg text-red-700 bg-red-50 hover:bg-red-100 disabled:opacity-50"
//                           >
...
// src/components/pages/MasterTableSettings.tsx
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {getOverviewStats().map((stat, index) => {
                const IconComponent = stat.icon;
                return (
                  <div key={index} className="bg-surface rounded-2xl shadow-lg p-6 border border-border hover:shadow-xl transition-shadow duration-300">
                    <div className="flex items-center justify-between">
                      <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-surface rounded-2xl shadow-lg p-6 border border-border">
                <h3 className="text-xl font-semibold text-text mb-4">Department Distribution</h3>
                <div className="space-y-3">
                  {getDepartmentStats().map((dept, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-muted/5 rounded-xl border border-border">
                      <div className="flex items-center">
                        <div className="bg-blue-100 dark:bg-blue-900/20 p-2 rounded-lg mr-3">
                          <Building2 className="h-4 w-4 text-blue-600 dark:text-blue-400" />
                        </div>
                        <span className="font-medium text-text">{dept.department}</span>
                      </div>
                      <span className="bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 text-sm font-medium px-2 py-1 rounded-full">
                        {dept.count} employees
                      </span>
                    </div>
                  ))}
              ...
              <div className="bg-surface rounded-2xl shadow-lg p-6 border border-border">
                <h3 className="text-xl font-semibold text-text mb-4">Recent Additions</h3>
                <div className="space-y-4">
                  {employeeData.slice(-3).map((employee, index) => (
                    <div key={index} className="flex items-center p-4 bg-muted/5 rounded-xl border border-border">
                      <div className="bg-green-100 dark:bg-green-900/20 p-2 rounded-lg mr-4">
                        <User className="h-5 w-5 text-green-600 dark:text-green-400" />
                      </div>
                      <div className="flex-1">
                        <p className="font-medium text-text">{`${employee.first_name} ${employee.last_name}`.trim()}</p>
                        <p className="text-sm text-muted">{employee.department_name || getDepartmentName(employee.department)} • {employee.emp_id}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {formFields.map((field) => {
                  ...
                  return (
                    <div key={field.id} className="space-y-2">
                      <label htmlFor={field.id} className="flex items-center text-sm font-medium text-text mb-2">
                        <IconComponent className="h-4 w-4 mr-2 text-muted" />
                        {field.label}
                        {field.required && <span className="text-red-500 ml-1">*</span>}
                      </label>
                      {field.type === 'select' ? (
                        <select
                          ...
                          value={formData[field.id as keyof EmployeeData] as string || ''}
                          onChange={(e) => handleInputChange(field.id as keyof EmployeeData, e.target.value)}
                          className="w-full px-4 py-3 border border-border bg-background text-text rounded-xl shadow-sm focus:outline-none focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/20 focus:border-blue-500 transition-all duration-200"
                        >
                          <option value="">Select {field.label}</option>
                          {field.id === 'department' 
                            ? departments.map((dept) => (
        ...
        return (
          <div className="bg-surface rounded-2xl shadow-lg p-8 border border-border">
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-text">Upload Employee Excel Data</h2>
            </div>
            
            <div className="space-y-6">
              <div className="border-2 border-dashed border-border rounded-xl p-8 text-center hover:border-green-400 dark:hover:border-green-600 transition-colors duration-200 bg-background hover:bg-muted/5">
                <FileSpreadsheet className="h-16 w-16 text-muted mx-auto mb-4" />
                <div className="space-y-2">
                  <label htmlFor="excel-upload" className="block text-lg font-medium text-text">
                    Choose Excel File
                  </label>
                  <p className="text-sm text-muted">Upload .xlsx or .xls files with employee data</p>
                  <input
                    id="excel-upload"
                    type="file"
                    accept=".xlsx,.xls"
                    onChange={(e) => setUploadFile(e.target.files?.[0] || null)}
                    className="block w-full text-sm text-muted mt-4
                      file:mr-4 file:py-3 file:px-6
                      file:rounded-xl file:border-0
                      file:text-sm file:font-semibold
                      file:bg-green-50 dark:file:bg-green-900/30 file:text-green-700 dark:file:text-green-400
                      hover:file:bg-green-100 dark:hover:file:bg-green-900/50 file:transition-colors file:duration-200"
                  />
                </div>
              </div>
              ...
              {uploadFile && (
                <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4 mt-4">
                  <div className="flex items-center">
                    ...
                    <span className="text-green-800 dark:text-green-300 font-medium">{uploadFile.name}</span>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="bg-surface rounded-2xl shadow-lg border border-border overflow-hidden">
            ...
            <div className="p-4 flex justify-between items-center border-b border-border bg-muted/5 min-h-[68px]">
              <div>
                {selectedEmpIds.length > 0 ? (
                  ...
                  <div className="flex items-center">
                    <FileSpreadsheet className="h-6 w-6 text-purple-600 dark:text-purple-400 mr-3" />
                    <h2 className="text-2xl font-bold text-text">Employee Records</h2>
                    <span className="ml-3 bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 text-sm font-medium px-3 py-1 rounded-full">
                      {employeeData.length} employees
                    </span>
                  </div>
                )}
            ...
            {loading ? (
                ...
                <table className="min-w-full divide-y divide-border">
                  <thead className="bg-muted/5">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-muted uppercase tracking-wider">Employee ID</th>
                      <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-muted uppercase tracking-wider">Name</th>
                      <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-muted uppercase tracking-wider">Department</th>
                      <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-muted uppercase tracking-wider">Join Date</th>
                      <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-muted uppercase tracking-wider">Gender</th>
                      <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-muted uppercase tracking-wider">Contact</th>
                      <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-muted uppercase tracking-wider">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="bg-surface divide-y divide-border">
                    {employeeData.map((employee) => (
                      <tr 
                        key={employee.emp_id} 
                        className={`transition-colors duration-150 ${selectedEmpIds.includes(employee.emp_id) ? 'bg-blue-50 dark:bg-blue-900/20' : 'hover:bg-muted/5'}`}
                      >
                        <td className="px-4 py-4">
                          <input
                            type="checkbox"
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm font-medium text-text">{employee.emp_id}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-text">{`${employee.first_name} ${employee.last_name}`.trim()}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-muted">{employee.department_name || getDepartmentName(employee.department)}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-muted">{formatDate(employee.date_of_joining)}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-muted">{getGenderDisplay(employee.sex)}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-text">{employee.email}</div>
                          <div className="text-sm text-muted">{employee.phone || 'N/A'}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <button
                            onClick={() => handleDelete(employee.emp_id)}
                            disabled={loading || isDeleting}
                            className="inline-flex items-center px-3 py-1 border border-red-300 dark:border-red-800 text-sm leading-4 font-medium rounded-lg text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            <Trash2 className="h-4 w-4" />
                          </button>
                        </td>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-xl font-medium text-text mb-2">No employees found</p>
                <p className="text-muted mb-6">Start by adding your first employee or uploading an Excel file</p>
                <div className="space-x-4">
                  <button
                    onClick={() => setActiveTab('add-data')}
                    className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-blue-600 hover:bg-blue-700"
                  ...
                  <button
                    onClick={() => setActiveTab('upload')}
                    className="inline-flex items-center px-4 py-2 border border-border text-sm font-medium rounded-lg text-text bg-surface hover:bg-muted/5"
                  >
                    <Upload className="h-4 w-4 mr-2" />
                    Upload Excel
                  </button>
  ...
  return (
    <div className="min-h-screen bg-background text-text transition-colors duration-300">
      <div className="container mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-text mb-2">Master Table Settings</h1>
          <p className="text-lg text-muted">Manage employee data and records efficiently</p>
        </div>

        {/* Tab Navigation */}
        <div className="mb-8">
          <div className="border-b border-border bg-surface rounded-t-2xl shadow-sm">
            <nav className="-mb-px flex space-x-8 px-6" aria-label="Tabs">
              {tabs.map((tab) => {
                const IconComponent = tab.icon;
                return (
