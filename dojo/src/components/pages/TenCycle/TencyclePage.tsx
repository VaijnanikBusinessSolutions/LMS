...
//       alert(
//         `Please complete ${
//           sortedDays[selectedIndex - 1].day_name
//         } evaluation first.`
//       );
//     }
//   };
...
const TenCyclePage: React.FC = () => {
  ...
  if (loading) {
    return (
      // Loading State: bg-background, text-muted
  ...
  return (
    // Main Container: bg-background, text-text
    <div className="min-h-screen bg-background text-text p-6 transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        
        {/* Header: Theme Header Tokens */}
        <div className="bg-header-surface border border-header-border rounded-2xl shadow-soft overflow-hidden mb-8">
          <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 text-white px-8 py-6">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <h2 className="text-2xl font-bold mb-1">
                ...
                <p className="text-header-muted font-medium">
                  Level Assessment Form
                </p>
              </div>
              <div className="bg-header-background/20 backdrop-blur-sm rounded-xl p-4 border border-header-border/30">
                <label
                  htmlFor="date"
                  className="block text-sm font-semibold text-header-text mb-2"
                >
                  Evaluation Date
                </label>
                <input
                  ...
                  value={formData.date}
                  onChange={handleInputChange}
                  className="bg-background text-text border border-border rounded-lg px-4 py-2 text-sm font-medium focus:ring-2 focus:ring-primary focus:outline-none shadow-sm"
                  required
                />
              </div>
            </div>
        ...
        {employee && (
          ...
          <div className="bg-surface rounded-2xl shadow-soft overflow-hidden border border-border mb-8 transition-colors duration-300">
            {/* Inner Header: bg-background (slightly diff from surface) */}
            <div className="bg-background/50 border-b border-border p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                  ...
                  <div className="bg-background rounded-lg px-4 py-2 text-sm font-medium text-text border border-border">
                    {(employee.first_name || "") +
                      (employee.last_name ? " " + employee.last_name : "")}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={formData.dateOfJoin}
                    readOnly
                    className="w-full bg-background border border-border rounded-lg px-4 py-2 text-sm text-muted cursor-not-allowed"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                  ...
                  <input
                    ...
                    value={formData.department}
                    onChange={handleInputChange}
                    className="w-full bg-background border border-border rounded-lg px-4 py-2 text-sm text-text focus:ring-2 focus:ring-primary focus:outline-none"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-muted mb-1">
        ...
        {topics.length > 0 && days.length > 0 && subtopics.length > 0 && (
          <>
            {/* Day Selection */}
            <div className="bg-surface rounded-2xl shadow-soft overflow-hidden border border-border mb-8">
              {/* Inner gradient replaced with subtle background color */}
              <div className="px-8 py-6 bg-primary/5 border-b border-border">
                <div className="flex items-center justify-center space-x-6">
                  ...
                  <select
                    value={selectedDay}
                    onChange={(e) => handleDayChange(e.target.value)}
                    className="bg-background border-2 border-border rounded-xl px-6 py-3 text-sm font-semibold text-text focus:ring-2 focus:ring-primary focus:outline-none shadow-sm"
                  >
                    <option value="">Select Day...</option>
                    {days
                      .sort((a, b) => a.sequence_order - b.sequence_order)
                      .map((day) => {
                        ...
                        return (
                          <option
                            ...
                            value={day.day_name}
                            disabled={!isSelectable}
                            // Select options styling is limited, but text color helps
                            className={
            ...
            {selectedDay && (
              <form onSubmit={handleSubmit}>
                <div className="bg-surface rounded-2xl shadow-soft overflow-hidden border border-border mb-8">
                  <div className="p-8">
                    <div className="bg-surface rounded-xl border border-border overflow-hidden shadow-sm overflow-x-auto">
                      <table className="w-full min-w-[1200px] table-fixed">
                        {/* Table Header: bg-background/80 or bg-muted/10 */}
                        <thead className="bg-muted/10 border-b-2 border-border">
                          <tr>
                            <th className="w-16 p-4 border-r border-border text-sm font-bold text-muted">
                              S.No.
                            </th>
                            <th className="w-60 p-4 border-r border-border text-sm font-bold text-muted text-left">
                              Topic
                            </th>
                            <th className="w-80 p-4 border-r border-border text-sm font-bold text-muted text-left">
                              Subtopic Description
                            </th>
                            <th className="w-20 p-4 border-r border-border text-sm font-bold text-muted">
                              Score
                            </th>
                            {[...Array(10)].map((_, i) => (
                              <th
                                key={`header-${i}`}
                                className="w-16 p-4 border-r border-border last:border-r-0 text-sm font-bold text-muted"
                              >
                                {i + 1}
                              </th>
                            ))}
                        ...
                        <tbody className="bg-surface">
                          {topics.map((topic, topicIndex) => {
                            ...
                            return topicSubtopics.map(
                              (subtopic, subtopicIndex) => (
                                <tr
                                  key={subtopic.id}
                                  // Row Alternation: bg-surface vs bg-background/50
                                ...
                                >
                                  {subtopicIndex === 0 && (
                                    <td
                                      rowSpan={topicSubtopics.length}
                                      className="w-16 p-4 border-r border-border text-center align-middle"
                                    >
                                      {/* Icon: Preserved Gradient */}
                                      <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center shadow-sm mx-auto">
                                        <span className="text-sm font-bold text-white">
                                          {topic.slno}
                                        </span>
                                  ...
                                  {subtopicIndex === 0 && (
                                    <td
                                      rowSpan={topicSubtopics.length}
                                      className="w-60 p-4 border-r border-border align-middle"
                                    >
                                      <div className="text-sm font-semibold text-text leading-tight break-words">
                                        {topic.cycle_topics}
                                      </div>
                                    </td>
                                  )}
                                  ...
                                  {/* Subtopic Column  */}
                                  <td className="w-80 p-4 border-r border-border align-top">
                                    <div className="text-sm text-text/80 leading-relaxed break-words hyphens-auto">
                                      {subtopic.sub_topic}
                                    </div>
                                  </td>

                                  {/* Score Column */}
                                  <td className="w-20 p-4 border-r border-border text-center align-top">
                                    <div
                                      className={`px-3 py-2 rounded-full text-sm font-bold shadow-sm inline-block ${
                                        subtopic.score_required === 10
                                  ...
                                  {[...Array(10)].map((_, i) => (
                                    <td
                                      key={`cycle-${i}-subtopic-${subtopic.id}`}
                                      className="w-16 p-4 border-r border-border last:border-r-0 text-center align-top"
                                    >
                                      <input
                                        type="number"
                                        min="0"
                                        ...
                                        className={`w-full h-12 text-center rounded-lg text-sm font-medium shadow-sm transition-all duration-200 ${
                                          isDaySubmitted
                                            ? "bg-muted/10 border-border text-muted cursor-not-allowed"
                                            : "bg-background border-2 border-border text-text focus:ring-2 focus:ring-primary focus:outline-none hover:border-primary/50"
                                        }`}
                                        placeholder="0"
                                      />
                                    </td>
                          ...
                          {/* Total Marks Row: bg-primary/5 */}
                          <tr className="bg-primary/5 border-b-2 border-border">
                            <td className="w-16 p-4 border-r border-border"></td>
                            <td className="w-60 p-4 border-r border-border">
                              <span className="text-sm font-bold text-text">
                                Total Marks
                              </span>
                            </td>
                            <td className="w-80 p-4 border-r border-border">
                            ...
                            <td className="w-20 p-4 border-r border-border"></td>
                            {[...Array(10)].map((_, i) => {
                              const total = calculateTotalForCycle(i);
                              const maxPossible = subtopics.reduce(
                                (sum, subtopic) =>
                              ...
                              return (
                                <td
                                  key={`total-${i}`}
                                  className="w-16 p-4 border-r border-border last:border-r-0 text-center"
                                >
                                  <div
                                    className={`px-3 py-2 rounded-lg text-sm font-bold shadow-sm border inline-block ${
                                      total === 0
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="bg-background rounded-xl p-6 border border-border">
                        ...
                        <input
                          ...
                          value={formData.prepared_by}
                          onChange={handleInputChange}
                          className="w-full rounded-lg px-4 py-3 text-sm border bg-surface border-border text-text focus:ring-2 focus:ring-primary focus:outline-none"
                          placeholder="Enter trainer name"
                        />
                      </div>
                ...
                {/* Submit Button */}
                <div className="bg-surface rounded-2xl shadow-soft border border-border p-8 text-center">
                  {isDaySubmitted ? (
                    <div className="flex flex-col items-center space-y-4">
                    ...
                    <button
                      type="submit"
                      disabled={loading || !formData.date || !selectedDay}
                      // Preserved gradient for main CTA
                      className="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 hover:from-blue-700 hover:via-blue-800 hover:to-indigo-800 disabled:opacity-50 text-white font-bold px-10 py-4 rounded-xl shadow-lg transition-all duration-300 disabled:cursor-not-allowed transform hover:scale-105 disabled:transform-none active:scale-95"
                    >
                      {loading ? (
                        <div className="flex items-center">
                          <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-3"></div>
        ...
        {evaluationData &&
          evaluationData.evaluations &&
          evaluationData.evaluations.length > 0 && (
            <div className="mt-8 bg-surface rounded-2xl shadow-soft overflow-hidden border border-border">
              <div className="px-8 py-6 bg-background/50 border-b border-border">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {/* Daily Results */}
                  <div>
                    <h4 className="text-md font-semibold text-muted mb-3 flex items-center">
                          ...
                          {days
                            ...
                            .map((day) => {
                              ...
                              return (
                                ...
                                >
                                  <td className="px-3 py-2 font-medium text-text flex items-center">
                                    {day.day_name}
                                  </td>
        ...
        {/* Scoring Guidelines */}
        <div className="mt-8 bg-surface rounded-2xl shadow-soft overflow-hidden border border-border">
          <div className="px-8 py-6 bg-background/50 border-t border-border">
            <h3 className="text-lg font-bold text-text mb-6 flex items-center">
              <div className="w-1 h-6 bg-green-500 rounded-full mr-3"></div>
              Evaluation Criteria
            </h3>
            ...
            {topics.length > 0 && (
              <div className="space-y-6">
                {topics.map((topic) => {
                  ...
                  return (
                    // Criteria Card: bg-primary/5
                    <div
                      key={topic.id}
                      className="bg-primary/5 border-2 border-primary/20 rounded-xl p-6 shadow-sm"
                    >
                      <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="space-y-4">
                        {topicSubtopics.map((subtopic) => (
                          ...
                          >
                            <div className="flex justify-between items-start mb-3">
                              <h5 className="font-semibold text-primary flex-1">
                                {subtopic.sub_topic}
                              </h5>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                              <div className="flex items-center p-2 bg-red-500/10 rounded text-sm">
                                <span className="w-4 h-4 bg-red-500 rounded-full mr-2 flex-shrink-0"></span>
                                <span className="text-red-700 dark:text-red-400">
                                  <strong>0:</strong> Not Following Standard
                                </span>
                              </div>
                              <div className="flex items-center p-2 bg-orange-500/10 rounded text-sm">
                                <span className="w-4 h-4 bg-orange-500 rounded-full mr-2 flex-shrink-0"></span>
                              ...
                              <div className="flex items-center p-2 bg-green-500/10 rounded text-sm">
                                <span className="w-4 h-4 bg-green-500 rounded-full mr-2 flex-shrink-0"></span>
            ...
            {/* Overall Passing Criteria */}
            <div className="mt-6 bg-green-500/5 border-2 border-green-500/20 rounded-xl p-6 shadow-sm">
              <h4 className="font-bold text-green-700 dark:text-green-400 text-center mb-4 text-lg">
                Overall Passing Criteria
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex items-center justify-center p-4 bg-surface rounded-lg border border-green-500/20">
                  <span className="w-4 h-4 bg-green-500 rounded-full mr-3"></span>
