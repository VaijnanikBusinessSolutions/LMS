...
// src/components/pages/TenCycle/TenCycleMethodPage.tsx
...
const TenCyclePage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-background p-6 transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="bg-surface rounded-2xl shadow-lg overflow-hidden border border-border mb-8">
          <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 dark:from-blue-800 dark:via-blue-900 dark:to-indigo-900 text-white px-8 py-6">
            <div className="flex items-center">
              <Settings className="w-8 h-8 mr-3" />
              <div>
                <h1 className="text-3xl font-bold mb-1">
          ...
          {/* Selections */}
          <div className="p-8 bg-muted/5 border-b border-border grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Department */}
            <div>
              ...
              <select
                className="w-full p-2 border border-border rounded bg-background text-text focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                value={selectedDepartment ?? ""}
                onChange={(e) => handleDepartmentChange(+e.target.value)}
              >
                <option value="">Select Department</option>
            ...
            <div>
              ...
              <select
                className="w-full p-2 border border-border rounded bg-background text-text focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 disabled:opacity-50"
                disabled={!selectedDepartment}
                value={selectedStation ?? ""}
                onChange={(e) => handleStationChange(+e.target.value)}
              >
        ...
        {selectedLevel && selectedDepartment && selectedStation && (
          <>
            {/* Passing Criteria */}
            <div className="bg-surface rounded p-6 shadow-sm mb-8 border border-border">
              <h3 className="text-xl font-semibold mb-4 text-text">Passing Criteria</h3>
              ...
              <input
                ...
                value={passingPercentage}
                onChange={(e) => setPassingPercentage(+e.target.value)}
                className="border border-border p-2 rounded w-24 bg-background text-text focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
              />
            </div>
            ...
            <div className="bg-surface rounded p-6 shadow-sm mb-8 border border-border">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-semibold text-text">Evaluation Days</h3>
                <button
                  onClick={addDay}
                  className="bg-green-600 hover:bg-green-700 text-white px-4 py-1 rounded inline-flex items-center transition-colors"
                >
              ...
              {days.map((day, idx) => (
                <div
                  key={idx}
                  className="border border-border rounded p-4 mb-3 flex flex-wrap items-center bg-background hover:bg-muted/5 transition-colors"
                >
                  {editingDay === idx ? (
                    <>
                      <input
                        className="p-2 border border-border rounded mr-4 mb-2 bg-background text-text focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                        value={day.day_name}
                        onChange={(e) =>
                          updateDay(idx, "day_name", e.target.value)
                        }
                      ...
                      <input
                        type="number"
                        className="p-2 border border-border rounded mr-4 mb-2 w-20 bg-background text-text focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                        value={day.sequence_order}
                        onChange={(e) =>
                          updateDay(idx, "sequence_order", +e.target.value)
                        }
                      ...
                      <button
                        onClick={() => setEditingDay(null)}
                        className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1 rounded mr-2 inline-flex items-center transition-colors"
                      >
                        <Save className="w-4 h-4 mr-1" /> Save
                      </button>
                      <button
                        onClick={() => deleteDay(idx)}
                        className="bg-red-600 hover:bg-red-700 text-white px-4 py-1 rounded inline-flex items-center transition-colors"
                      >
                        <Trash2 className="w-4 h-4 mr-1" /> Delete
                      </button>
                    </>
                    ...
                    <>
                      ...
                      <button
                        onClick={() => setEditingDay(idx)}
                        className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-1 rounded inline-flex items-center transition-colors"
                      >
                        <Edit className="w-4 h-4 mr-1" /> Edit
                      </button>
                    </>
            ...
            <div className="bg-surface rounded p-6 shadow-sm mb-8 border border-border">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-semibold text-text">Evaluation Topics & Subtopics</h3>
                <button
                  onClick={addTopic}
                  className="bg-green-600 hover:bg-green-700 text-white px-4 py-1 rounded inline-flex items-center transition-colors"
                >
              ...
              {topics.map((topic, idx) => (
                <div key={idx} className="border border-border rounded p-4 mb-4 bg-background">
                  {/* Topic */}
                  <div className="flex flex-wrap items-center mb-4">
                    {editingTopic === idx ? (
                      <>
                        ...
                        <input
                          type="text"
                          className="p-2 border border-border rounded mr-4 mb-2 flex-grow bg-background text-text focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                          value={topic.cycle_topics}
                          onChange={(e) =>
                            updateTopic(idx, "cycle_topics", e.target.value)
                          }
                      ...
                      <>
                        ...
                        <button
                          onClick={() => setEditingTopic(idx)}
                          className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-1 rounded mr-2 inline-flex items-center transition-colors"
                        >
                          <Edit className="w-4 h-4 mr-1" /> Edit Topic
                        </button>
                        <button
                          onClick={() => addSubtopic(idx)}
                          className="bg-green-500 hover:bg-green-600 text-white px-4 py-1 rounded inline-flex items-center transition-colors"
                        >
                          <Plus className="w-4 h-4 mr-1" /> Add Subtopic
                        </button>
                      </>
                  ...
                  <div className="ml-4 border-l-2 border-border pl-4">
                    {(subtopics[idx] || []).map((subtopic, subIdx) => (
                      <div key={subIdx} className="border border-border rounded p-2 mb-2 bg-muted/5">
                        {editingSubtopic?.topicIndex === idx && editingSubtopic?.subtopicIndex === subIdx ? (
                          <div className="flex flex-wrap items-center">
                            <input
                              type="text"
                              className="p-2 border border-border rounded mr-2 mb-2 flex-grow bg-background text-text focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                              value={subtopic.sub_topic}
                              onChange={(e) =>
                                updateSubtopic(idx, subIdx, "sub_topic", e.target.value)
                              }
                            ...
                            <input
                              type="number"
                              min={1}
                              className="p-2 border border-border rounded mr-2 mb-2 w-24 bg-background text-text focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                              value={subtopic.score_required}
                              onChange={(e) =>
                                updateSubtopic(idx, subIdx, "score_required", +e.target.value)
                              }
                            ...
                            <button
                              onClick={() => setEditingSubtopic(null)}
                              className="bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 rounded mr-2 text-sm inline-flex items-center transition-colors"
                            >
                              <Save className="w-3 h-3 mr-1" /> Save
                            </button>
                            <button
                              onClick={() => deleteSubtopic(idx, subIdx)}
                              className="bg-red-600 hover:bg-red-700 text-white px-2 py-1 rounded text-sm inline-flex items-center transition-colors"
                            >
                              <Trash2 className="w-3 h-3 mr-1" /> Delete
                            </button>
                          </div>
                          ...
                          <div className="flex items-center">
                            <div className="flex-1 text-text">
                              <span className="text-sm">
                                {subtopic.sub_topic || "Untitled Subtopic"} 
                                <span className="text-muted ml-2"> (Max Score: {subtopic.score_required})</span>
                              </span>
                            </div>
                            <button
                              onClick={() => setEditingSubtopic({ topicIndex: idx, subtopicIndex: subIdx })}
                              className="bg-yellow-400 hover:bg-yellow-500 text-white px-2 py-1 rounded text-sm inline-flex items-center transition-colors"
                            >
                              <Edit className="w-3 h-3 mr-1" /> Edit
                            </button>
                          </div>
                    ...
                    {(!subtopics[idx] || subtopics[idx].length === 0) && (
                      <p className="text-muted text-sm py-2">No subtopics added yet.</p>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="flex justify-center space-x-6 mb-8">
              <button
                onClick={resetToDefaults}
                disabled={loading}
                className="bg-gray-500 hover:bg-gray-600 text-white px-6 py-3 rounded inline-flex items-center disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                <X className="w-5 h-5 mr-2" /> Reset to Defaults
              </button>
              <button
                ...
                className={`${
                  loading
                    ? "bg-blue-300 cursor-not-allowed"
                    : "bg-blue-600 hover:bg-blue-700"
                } text-white px-6 py-3 rounded inline-flex items-center transition-colors shadow-lg`}
              >
                {loading ? (
                  <span className="animate-spin inline-block w-5 h-5 border-4 border-white border-t-transparent rounded-full mr-2"></span>
                ) : (
