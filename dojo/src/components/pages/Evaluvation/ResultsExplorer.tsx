      ...
      <header className="bg-surface shadow-md border-b border-border sticky top-0 z-30">
        <div className="w-full mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate(-1)}
              className="w-9 h-9 rounded-xl bg-muted/10 hover:bg-muted/20 flex items-center justify-center transition-colors text-muted hover:text-primary"
            >
          ...
          {/* Mode Toggle */}
          <div className="flex items-center gap-2 bg-muted/10 rounded-full p-1 border border-border">
            <button
              onClick={() => handleModeChange("remote")}
              className={`px-4 py-1.5 text-xs font-semibold rounded-full transition-colors ${
                mode === "remote"
      ...
      <main className="w-full mx-auto px-6 py-6 space-y-6">
        ...
        <section className="card p-5">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {/* Level */}
            <div>
              <label className="label text-muted">Level</label>
              <div className="relative">
                <select
                  value={selectedLevelId ?? ""}
                  onChange={(e) =>
                  ...
                  className="input appearance-none pr-8 focus:ring-blue-500"
                  disabled={loadingLevels || levels.length === 0}
                >
                  {levels.length === 0 && <option value="">No levels</option>}
                  {levels.map((lvl) => (
            ...
            {/* Department */}
            <div>
              <label className="label text-muted">Department</label>
              <div className="relative">
                <select
                  value={selectedDepartmentId ?? ""}
                  onChange={(e) =>
                  ...
                  className="input appearance-none pr-8 focus:ring-green-500"
                  disabled={
                    !selectedLevelId ||
                    loadingDepartments ||
                    departments.length === 0
            ...
            {/* Station */}
            <div>
              <label className="label text-muted">Station</label>
              <div className="relative">
                <select
                  value={selectedStationId ?? ""}
                  onChange={(e) =>
                  ...
                  className="input appearance-none pr-8 focus:ring-purple-500"
                  disabled={
                    !selectedLevelId ||
                    !selectedDepartmentId ||
                    loadingStations ||
              ...
              {mode === "remote" ? (
                <div className="relative">
                  <select
                    ...
                    className="input appearance-none pr-8 focus:ring-yellow-500"
                    disabled={
                      !selectedLevelId ||
                      !selectedDepartmentId ||
                      !selectedStationId ||
                ...
                <div className="w-full px-4 py-2 text-xs text-muted bg-muted/10 border border-border rounded-xl">
                  Showing all individual attempts for the selected Level,
                  Department and Station.
                </div>
              )}
          ...
          <div className="lg:col-span-2 card p-5">
            ...
            {currentStation ? (
              <div className="grid grid-cols-3 gap-3 mt-3 text-sm">
                ...
                <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800 rounded-xl px-3 py-2">
                  <p className="text-xs text-purple-600 dark:text-purple-400">Avg %</p>
                  <p className="text-lg font-semibold text-text">
                    {currentStation.avg_percentage.toFixed(1)}%
                  </p>
                </div>
            ...
            {mode === "remote" && currentBatch && (
              <div className="mt-4 border-t border-border pt-3 text-xs text-muted">
                ...
                <p className="mt-1">
                  <span className="font-semibold text-text">Paper:</span>{" "}
                  {currentBatch.question_paper_name}
                </p>
              </div>
            )}
        ...
        <section className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={handleMatrixView}
              disabled={!selectedLevelId || !selectedStationId}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl border border-indigo-500 text-indigo-600 dark:text-indigo-400 bg-surface hover:bg-indigo-50 dark:hover:bg-indigo-900/20 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              <Table className="w-4 h-4" />
              Matrix View
            </button>
            ...
            <button
              onClick={handleMatrixExcel}
              disabled={!selectedLevelId || !selectedStationId}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl border border-green-500 text-green-600 dark:text-green-400 bg-surface hover:bg-green-50 dark:hover:bg-green-900/20 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              <Download className="w-4 h-4" />
              Export Matrix (Excel)
            </button>
        ...
        <section className="card p-0 overflow-hidden">
          ...
          <div className="p-4">
            {loadingScores ? (
              <div className="flex flex-col items-center justify-center py-12 gap-3">
                <div className="w-8 h-8 border-4 border-blue-200 dark:border-blue-900/30 rounded-full animate-spin border-t-blue-600" />
                    ...
                    {scores.map((s, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-center text-sm font-semibold text-text">
                            {s.marks}
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap text-center text-sm font-semibold text-text">
                            {s.percentage.toFixed(1)}%
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-center">
                            <button
                              onClick={() => handleViewSheet(s.id)}
                              className="px-3 py-1 text-xs font-semibold text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/20 dark:hover:bg-indigo-900/40 rounded-lg transition-colors"
                            >
                              View Sheet
                            </button>
                          </td>
