...
const RemoteQuiz: React.FC = () => {
  ...
  // --- REFS (Critical for Event Handlers) ---
  ...
  const wsRef = useRef<WebSocket | null>(null); // Keep track of socket
  ...
  // Keep refs updated for use inside WebSocket callback
  ...
  useEffect(() => {
    ...
    // 2. Singleton Check: If socket exists, do not recreate it.
    if (wsRef.current) {
        // Optional: Check readyState if you want to be extra safe
    ...
    ws.onmessage = (event) => {
      try {
        ...
        // FIX: Handle wrapped payload vs raw data
    ...
    ws.onclose = (e) => {
      ...
      // Only nullify if it matches the current ref (prevents race conditions)
      if (wsRef.current === ws) {
          wsRef.current = null;
      }
    };
    ...
    // 3. Cleanup: Only runs on unmount or if quiz ends
    return () => {
      if (wsRef.current === ws) {
        console.log("🧹 Cleaning up WebSocket");
        ws.onclose = null; // Prevent onclose logic from firing during cleanup
        ws.close();
        wsRef.current = null;
      }
    };
    
    // 4. Dependencies: ONLY run this when these high-level states change.
    // Do NOT include 'questions' or 'isFullScreen' here.
