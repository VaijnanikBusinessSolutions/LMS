...
const InfoField: React.FC<InfoFieldProps> = ({ label, value }) => (
	<div className='flex items-start bg-background border border-border p-4 rounded-xl hover:shadow-md transition-all duration-200'>
		<span className='text-muted font-semibold text-base w-32'>{label}:</span>
		<span className='font-bold text-lg text-text flex-1'>{value || "N/A"}</span>
	</div>
);
