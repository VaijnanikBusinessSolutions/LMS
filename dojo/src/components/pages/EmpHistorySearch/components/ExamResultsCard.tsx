    ...
    return (
        <div className='bg-surface p-8 rounded-3xl shadow-sm border border-border hover:shadow-md transition-shadow duration-300 h-[450px] flex flex-col'>
            <div className='flex items-center mb-6'>
                ...
                <h2 className='text-2xl font-bold text-text'>Hanchou & Shokuchou Exam Results</h2>
            </div>
            <div className='flex-1 overflow-y-auto hide-scrollbar'>
                {results.length > 0 ? (
                    <div className='space-y-4'>
                        {results.map((result) => (
                            <div key={`${result.type}-${result.id}`} className='bg-background p-5 rounded-2xl border-l-4 border-indigo-500 shadow-sm hover:shadow-md transition-all duration-200'>
                                <div className='flex items-center justify-between mb-3'>
                                    <h3 className='font-bold text-lg text-text'>{result.exam_name}</h3>
                                    <span className={`px-3 py-1 text-xs font-bold rounded-full ${result.type === 'hanchou' ? 'bg-indigo-100 text-indigo-800' : 'bg-purple-100 text-purple-800'}`}>{result.type.toUpperCase()}</span>
                                </div>
                                <div className='flex justify-between text-base mb-2'><span className='text-muted font-medium'>Score:</span><span className='font-bold text-purple-600'>{result.score} / {result.total_questions} ({result.percentage.toFixed(1)}%)</span></div>
                                <div className='flex justify-between text-base mb-2'><span className='text-muted font-medium'>Date:</span><span className='font-semibold text-text'>{new Date(result.submitted_at).toLocaleDateString()}</span></div>
                                <div className='flex justify-between text-base mb-2'><span className='text-muted font-medium'>Result:</span><span className={`font-bold text-lg ${result.passed ? "text-green-600" : "text-red-600"}`}>{result.passed ? "PASS" : "FAIL"}</span></div>
                                {result.passed && (
                                    <div className='mt-4 text-right'>
                                        {/* <button
                                            onClick={() => onDownload(result)}
                    ...
                    <div className='flex items-center justify-center h-full text-muted text-lg'>No Hanchou or Shokuchou exam results found</div>
                )}
            </div>
        </div>
    );
