...
//                                 <div className='flex justify-between text-base mb-2'><span className='text-gray-600 font-medium'>Date:</span><span className='font-semibold'>{new Date(score.created_at).toLocaleDateString()}</span></div>
    ...
    return (
        <div className='bg-surface p-8 rounded-3xl shadow-sm border border-border hover:shadow-md transition-shadow duration-300 h-[450px] flex flex-col'>
            <div className='flex items-center mb-6'>
                ...
                <h2 className='text-2xl font-bold text-text'>Scores & Assessments</h2>
            </div>
            <div className='flex-1 overflow-y-auto hide-scrollbar'>
                {scores.length > 0 ? (
                    <div className='space-y-4'>
                        {scores.map((score) => (
                            <div key={score.id} className='bg-background p-5 rounded-2xl border-l-4 border-yellow-500 shadow-sm hover:shadow-md transition-all duration-200'>
                                <h3 className='font-bold text-lg text-text mb-3'>{score.test_name || "Assessment"}</h3>
                                <div className='flex justify-between text-base mb-2'><span className='text-muted font-medium'>Score:</span><span className='font-bold text-purple-600'>{score.marks} ({score.percentage.toFixed(1)}%)</span></div>
                                <div className='flex justify-between text-base mb-2'><span className='text-muted font-medium'>Date:</span><span className='font-semibold text-text'>{new Date(score.created_at).toLocaleDateString()}</span></div>
                                <div className='flex justify-between text-base mb-2'><span className='text-muted font-medium'>Result:</span><span className={`font-bold text-lg ${isPassed(score) ? "text-green-600" : "text-red-600"}`}>{isPassed(score) ? "PASS" : "FAIL"}</span></div>
                                {isPassed(score) && (
                                    <div className='mt-4 text-right'>
                                        {/* <button
                                            onClick={() => onDownload(score)}
                    ...
                    <div className='flex items-center justify-center h-full text-muted text-lg'>No assessment scores available</div>
                )}
            </div>
        </div>
    );
