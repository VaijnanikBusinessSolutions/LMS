        ...
        <div className="bg-surface p-8 rounded-3xl shadow-sm border border-border hover:shadow-md transition-shadow duration-300 h-[450px] flex flex-col">
            <div className="flex justify-between items-start gap-4 mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-2xl font-bold text-text">Skill Level Visualizer</h2>
                </div>
                {skills.length > 0 && selectedSkill && (
                    <select
                        id="skill-selector"
                        value={selectedSkill.id}
                        onChange={handleSkillChange}
                        className="px-4 py-2.5 bg-background border border-border rounded-xl shadow-sm text-base font-medium text-text focus:ring-2 focus:ring-purple-500 focus:border-transparent max-w-xs transition-all duration-200"
                    >
                        {skills.map((skill) => <option key={skill.id} value={skill.id}>{skill.station_name}</option>)}
                    </select>
                )}
            ...
            <div className="flex-grow">
                {skills.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center h-full text-muted text-lg">No skills available to visualize</div>
                 )}
            </div>
        </div>
    );
