...
const TrainingCard: React.FC<TrainingCardProps> = ({ trainings }) => {
    ...
    return (
        <div className='bg-surface p-8 rounded-3xl shadow-sm border border-border hover:shadow-md transition-shadow duration-300 h-[450px] flex flex-col'>
            <div className='flex items-center mb-6'>
                <div className='bg-gradient-to-r from-red-500 to-pink-500 p-3 rounded-2xl mr-4 shadow-lg'>
                    <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                </div>
                <h2 className='text-2xl font-bold text-text'>Scheduled Trainings</h2>
            </div>
            <div className='flex-1 overflow-y-auto hide-scrollbar'>
                {trainings.length > 0 ? (
                    <div className='space-y-5'>
                        {trainings.map((training) => (
                            <div key={training.id} className='bg-background p-6 rounded-2xl border-l-4 border-red-500 shadow-sm hover:shadow-md transition-all duration-200'>
                                <h3 className='font-bold text-xl text-text mb-4'>{training.topic}</h3>
                                <div className='space-y-3 text-base'>
                                    <div className='flex justify-between items-center'><span className='text-muted font-medium'>Category:</span><span className='font-semibold text-purple-700 bg-purple-100 border border-purple-200 px-3 py-1 rounded-full text-sm'>{training.category_name}</span></div>
                                    <div className='flex justify-between items-center'><span className='text-muted font-medium'>Venue:</span><span className='font-semibold text-text text-right'>{training.venue_name || 'N/A'}</span></div>
                                    <div className='flex justify-between items-center'><span className='text-muted font-medium'>Date:</span><span className='font-semibold text-text'>{new Date(training.date).toLocaleDateString()}</span></div>
                                    <div className='flex justify-between items-center'><span className='text-muted font-medium'>Status:</span><span className={`px-3 py-1.5 text-sm font-bold rounded-full capitalize border-2 ${statusStyles[training.status.toLowerCase()] || 'bg-gray-100 text-gray-800 border-gray-300'}`}>{training.status}</span></div>
                                </div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className='flex items-center justify-center h-full text-muted text-lg'>No trainings scheduled</div>
                )}
            </div>
        </div>
    );
