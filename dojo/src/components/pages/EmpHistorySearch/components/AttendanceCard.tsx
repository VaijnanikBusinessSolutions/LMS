...
const AttendanceCard: React.FC<AttendanceCardProps> = ({
  ...
  return (
    <div className='bg-surface p-8 rounded-3xl shadow-sm border border-border hover:shadow-md transition-shadow duration-300 h-[520px] flex flex-col'>
      {/* Header */}
      <div className='flex items-center mb-6'>
        <div className='bg-gradient-to-r from-cyan-500 to-blue-500 p-3 rounded-2xl mr-4 shadow-lg'>
          <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <h2 className='text-2xl font-bold text-text'>Attendance Record</h2>
      </div>
          ...
          Object.entries(attendanceByBatch).map(([batchId, records]) => {
            ...
            return (
              <div key={batchId} className="space-y-10">
                ...
                {/* ORIGINAL ATTENDANCE */}
                <div>
                  <h4 className="font-bold text-lg text-text mb-3">Batch: {batchId}</h4>
                  <div className="overflow-hidden rounded-2xl border-2 border-border shadow-md">
                    <div className="grid" style={{ gridTemplateColumns: `repeat(${records.length}, minmax(0, 1fr))` }}>
                      ...
                      {/* Show ORIGINAL status only */}
                      {records.map(record => (
                        <div
                          key={record.id}
                          className={`flex items-center justify-center p-4 border-t-2 border-r border-border last:border-r-0 ${
                            record.status === 'present' ? 'bg-green-50' : 'bg-red-50'
                          }`}
                        >
                          {record.status === 'present' ? (
