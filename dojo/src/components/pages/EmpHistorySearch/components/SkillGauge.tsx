...
const SkillGauge = ({ level }: SkillGaugeProps) => {
  ...
  // We use CSS variables for colors to match the theme tokens
  // Note: react-d3-speedometer requires string color values, so we pass var()
  return (
    <div className="w-full h-full flex items-center justify-center bg-surface rounded-xl">
      <ReactSpeedometer
        ...
        value={getGaugeValue(level)}
        segments={4}
        segmentColors={["#ef4444", "#f59e0b", "#eab308", "#84cc16"]} // Semantic colors kept for meaning
        needleColor="var(--text)" // Updated to use token color
        needleTransitionDuration={2000}
        needleTransition={Transition.easeElastic}
        ringWidth={40}
        textColor="var(--muted)" // Updated to use token color
        customSegmentLabels={[
          { text: "Beginner", position: CustomSegmentLabelPosition.Inside, color: "#fff" },
          { text: "Intermediate", position: CustomSegmentLabelPosition.Inside, color: "#fff" },
          { text: "Advanced", position: CustomSegmentLabelPosition.Inside, color: "#1f2937" },
