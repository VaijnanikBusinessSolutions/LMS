...
// src/components/pages/GeneralSettingsMethod.tsx
...
const CompanyLogoUpload: React.FC = () => {
  ...
  return (
    <div className="bg-surface rounded-2xl border border-border shadow-sm overflow-hidden transition-colors duration-300">
      {/* Header */}
      <div className="bg-muted/5 px-8 py-6 border-b border-border">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted">Manage your organization's brand identity</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-8">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  ...
                  value={formData.name}
                  onChange={handleChange}
                  className="w-full px-4 py-3 bg-surface border border-border text-text rounded-xl shadow-sm focus:outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200"
                  placeholder="Enter your company name"
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    accept="image/*"
                    onChange={handleFileChange}
                    className="w-full px-4 py-3 bg-surface border border-border text-text rounded-xl shadow-sm focus:outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-muted/10 file:text-blue-600 dark:file:text-blue-400 hover:file:bg-muted/20"
                  />
                </div>
              ...
              <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 pt-4">
                {isEditing && (
                  <button
                    type="button"
                    onClick={resetForm}
                    className="px-6 py-3 bg-surface border border-border text-text rounded-xl font-semibold hover:bg-muted/10 transition-all duration-200 flex items-center justify-center space-x-2"
                  >
                    <X className="w-4 h-4" />
                    <span>Cancel</span>
                  </button>
        ...
        {logos.length > 0 && (
          <div>
            <h4 className="text-lg font-semibold mb-4 text-text flex items-center">
              <Building className="w-5 h-5 mr-2 text-muted" />
              Current Company Logo
            </h4>
            <div className="grid grid-cols-1 gap-6">
              {logos.map((logo) => (
                <div key={logo.id} className="bg-background rounded-xl p-6 border border-border hover:shadow-md transition-all duration-200">
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-6">
                      <div className="w-24 h-24 bg-white rounded-xl border border-border flex items-center justify-center p-2 shadow-sm mx-auto sm:mx-0">
                        <img 
                          src={logo.logo}
                          alt={logo.name} 
                          className="max-w-full max-h-full object-contain"
                      ...
                      <div className="text-center sm:text-left">
                        <h5 className="text-lg font-semibold text-text">{logo.name}</h5>
                    ...
                    <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                      <button
                        onClick={() => handleEdit(logo)}
                        className="px-4 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg font-medium hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-all duration-200 flex items-center justify-center space-x-2 border border-transparent hover:border-blue-200 dark:hover:border-blue-800"
                      >
                        <Edit3 className="w-4 h-4" />
                        <span>Edit</span>
                      </button>
                      <button
                        onClick={() => handleDelete(logo.id)}
                        className="px-4 py-2 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg font-medium hover:bg-red-100 dark:hover:bg-red-900/40 transition-all duration-200 flex items-center justify-center space-x-2 border border-transparent hover:border-red-200 dark:hover:border-red-800"
                      >
                        <Trash2 className="w-4 h-4" />
                        <span>Delete</span>
                      </button>
        ...
        {logos.length === 0 && !isEditing && (
          <div className="text-center py-12">
            ...
            <p className="text-muted text-lg">No company logo uploaded yet</p>
            <p className="text-muted/70 text-sm mt-1">Upload your first logo to get started</p>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="min-h-screen bg-background text-text transition-colors duration-300 p-8">
      ...
      <div className="bg-surface border border-border rounded-xl shadow-sm mb-8">
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-text">General Settings</h1>
              <p className="text-muted mt-1">Configure your organization's core settings and preferences</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 gap-8">
          ...
          {/* Future Settings Sections */}
          <div className="bg-surface rounded-2xl border border-border shadow-sm p-8 transition-colors duration-300">
            <div className="text-center py-12">
              <div className="w-16 h-16 bg-muted/10 rounded-full flex items-center justify-center mx-auto mb-4">
                <Building className="w-8 h-8 text-muted" />
              </div>
              <h3 className="text-lg font-semibold text-text mb-2">More Settings Coming Soon</h3>
              <p className="text-muted">Additional configuration options will be available here</p>
            </div>
          </div>
        </div>
      </div>
