...
//             {levels
//       .filter((lvl) => lvl.id !== 1)  // This line blocks Level 1
//       .map((lvl) => (
//         <option key={lvl.id} value={lvl.id}>
//           {lvl.name}
//         </option>
...
// src/components/molecules/OJTSettingsPanel/OJTSettingsPanel.tsx
import React, { useEffect, useState } from "react";
import { getDepartments, getLevels } from "../../hooks/ServiceApis";
import { Settings } from "lucide-react";
import TopicsSection from "../TopicsSection/TopicsSection";
...
const OJTSettingsPanel: React.FC = () => {
  ...
  return (
    <div className="bg-background border-b border-border p-8 transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        {/* Title */}
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-2xl font-bold text-text">Form Customization</h2>
        </div>

        {/* Department & Level Selection */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="p-4 bg-surface rounded-xl shadow-sm border border-border">
            <h4 className="font-semibold text-text mb-2">Select Department</h4>
            <select
              className="w-full px-3 py-2 border border-border bg-background text-text rounded-lg focus:outline-none focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-200"
              value={selectedDepartment || ""}
              onChange={(e) => setSelectedDepartment(Number(e.target.value) || null)}
            >
              <option value="">Choose Department</option>
          ...
          <div className="p-4 bg-surface rounded-xl shadow-sm border border-border">
            <h4 className="font-semibold text-text mb-2">Select Level</h4>
            <select
              className="w-full px-3 py-2 border border-border bg-background text-text rounded-lg focus:outline-none focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-200"
              value={selectedLevel || ""}
              onChange={(e) => setSelectedLevel(Number(e.target.value) || null)}
