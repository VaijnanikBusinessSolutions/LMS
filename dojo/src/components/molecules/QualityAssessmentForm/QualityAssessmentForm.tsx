...
const QualityAssessmentForm: React.FC<QualityAssessmentFormProps> = ({
  ...
  // Updated colors to be theme-compatible (using subtle backgrounds with colored text/border)
  const getCategoryColor = (category: string) => {
    const colors = {
      'Technical Knowledge': 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20',
      'Safety': 'bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20',
      'Process': 'bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20',
      'Quality': 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/20',
      'Production': 'bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/20',
    };
    return (
      colors[category as keyof typeof colors] ||
  ...
  return (
    <div className="mb-10">
      ...
      <div className="bg-surface rounded-2xl shadow-soft border border-border overflow-hidden transition-colors duration-300">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              {/* Header: Uses Header Theme Tokens for consistency */}
              <tr className="bg-header-surface text-header-text">
                <th className="px-6 py-4 text-left text-sm font-bold tracking-wide border-r border-header-border/20">
                  S.NO.
                </th>
                <th className="px-6 py-4 text-left text-sm font-bold tracking-wide min-w-[350px] border-r border-header-border/20">
                  TRAINING TOPIC
                </th>
                <th className="px-6 py-4 text-center text-sm font-bold tracking-wide border-r border-header-border/20">
                  CATEGORY
                </th>
                {days.map((day) => (
                  // Day Columns: Uses Primary Color for distinction
                  <th
                    key={day}
                    className="px-4 py-4 text-center text-sm font-bold tracking-wide min-w-[100px] bg-primary text-white border-r border-white/20"
                  >
                    {day}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-border">
              {currentTopics.map((topic, index) => (
                <tr
                  key={topic.id}
                  // Row: bg-surface/bg-background alternation for stripes
                  className={`hover:bg-primary/5 transition-all duration-200 ${
                    index % 2 === 0 ? 'bg-background/50' : 'bg-surface'
                  }`}
                >
                  <td className="px-6 py-4 text-center text-sm font-bold text-muted border-r border-border">
                    {index + 1}
                  </td>
                  <td className="px-6 py-4 text-sm text-text font-medium border-r border-border">
                    {topic.description}
                  </td>
                  ...
                  {days.map((day, dayIndex) => {
                    ...
                    return (
                      <td
                        key={day}
                        className="px-3 py-4 border-r border-border"
                      >
                        <input
                          type="number"
                          min={scoreRanges?.min_score ?? 0}
                          ...
                          className={`w-full px-3 py-2 text-center border rounded-lg text-sm font-semibold transition-all duration-200
                            ${
                              isEditable
                                ? 'border-border focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent bg-background text-text hover:border-primary/50'
                                : 'bg-muted/10 text-muted cursor-not-allowed border-transparent'
                            }`}
                          placeholder={isEditable ? `${scoreRanges?.min_score ?? 0}-${scoreRanges?.max_score ?? 10}` : '-'}
                        />
                      </td>
                    );
                  })}
