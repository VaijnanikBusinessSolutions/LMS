...
from django.contrib import admin
from .models import SubTopic, SubTopicContent, Evaluation,StationSheetConfig
...
from django.contrib import admin
from .models import Hq, Factory, Department, Line, SubLine, Station, HierarchyStructure

# ... register other models ...

@admin.register(HierarchyStructure)
class HierarchyStructureAdmin(admin.ModelAdmin):
    list_display = ('structure_name', 'hq', 'factory', 'department', 'station', 'station_type')
    list_filter = ('station_type', 'structure_name')
    search_fields = ('structure_name', 'station__station_name')
...
admin.site.register(LevelDayRequirement)
...
from .models import Question, Option,Schedule


# Inline for Options (appears inside Question admin)
class OptionInline(admin.TabularInline):
    model = Option
    extra = 2
    fields = ('option_text', 'option_image', 'image_preview', 'is_correct')
...
class QuestionAdmin(admin.ModelAdmin):
    list_display = ('id', 'short_text', 'question_image_preview', 'subtopiccontent', 'created_at', 'total_options')
    list_display_links = ('id', 'short_text')
    search_fields = ('question_text', 'subtopiccontent__content')
    list_filter = ('subtopiccontent__content', 'created_at')
    inlines = [OptionInline]
    readonly_fields = ('question_image_preview', 'created_at')
    fields = ('subtopiccontent', 'question_text', 'question_image', 'question_image_preview', 'created_at')
    ...
    def question_image_preview(self, obj):
        if obj.question_image:
            return format_html(
                '<img src="{}" style="max-height: 200px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);" />',
                obj.question_image.url
            )
        return mark_safe('<span style="color: #999; font-style: italic;">No image uploaded</span>')
    question_image_preview.short_description = "Question Paper"

    def total_options(self, obj):
        count = obj.options.count()
        correct = obj.options.filter(is_correct=True).count()
        return format_html(
            '<span style="font-weight: bold; color: {};">{} options</span> <small>({} correct)</small>',
            "#10b981" if correct == 1 else "#ef4444",
            count,
            correct
        )
...
# Optional: Register Option separately if you want to edit standalone
@admin.register(Option)
class OptionAdmin(admin.ModelAdmin):
    list_display = ('question', 'option_text_preview', 'image_preview_small', 'is_correct')
    list_filter = ('is_correct', 'question__subtopiccontent__content')
    search_fields = ('option_text', 'question__question_text')

    def option_text_preview(self, obj):
        if obj.option_text:
            return obj.option_text[:50] + ("..." if len(obj.option_text) > 50 else "")
        return "Image Option"
    option_text_preview.short_description = "Text"
...
admin.site.register(StationSheetConfig)

admin.site.register(HumanBodyCheckSession)
...
admin.site.register(AnimationVideo)
...
class StationTypeAdmin(admin.ModelAdmin):
    list_display = ('type_id', 'code', 'name', 'icon_preview')
